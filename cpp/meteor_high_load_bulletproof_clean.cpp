...
public:
    ...
    void process_pending_batch() {
        ...
        // **HIGH-LOAD BULLETPROOF PROCESSING**: Chunked with memory management
        const size_t CHUNK_SIZE = 32;
        size_t total_ops = pending_operations_.size();
        
        for (size_t chunk_start = 0; chunk_start < total_ops; chunk_start += CHUNK_SIZE) {
            size_t chunk_end = std::min(chunk_start + CHUNK_SIZE, total_ops);
            
            for (size_t i = chunk_start; i < chunk_end; ++i) {
                auto& op = pending_operations_[i];
                try {
                    std::string response = execute_single_operation_high_load_safe(op);
                    if (client_responses[op.client_fd].size() < 500) { // Limit per client
                        client_responses[op.client_fd].emplace_back(std::move(response));
                    } else {
                        client_responses[op.client_fd].emplace_back("-ERR too many pending\r\n");
                    }
    ...
    // **HIGH-LOAD BULLETPROOF**: Memory-efficient safe version for extreme loads
    std::string execute_single_operation_high_load_safe(const BatchOperation& op) {
        try {
            if (zero_copy::is_get_command(op.command)) {
                auto value = cache_->get(std::string(op.key));
                if (value) {
                    // **MEMORY EFFICIENT**: Pre-calculate exact size
                    size_t total_size = 1 + std::to_string(value->length()).length() + 2 + value->length() + 2;
                    std::string response;
                    response.reserve(total_size); // **CRITICAL**: Pre-allocate
