...
namespace zero_copy {
    ...
    // **MINIMAL TTL**: Only TTL command detection  
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **MINIMAL TTL**: Static TTL response (key not found)
        if (zero_copy::is_ttl_command(op.command)) {
            static const char* TTL_NOT_FOUND = ":-2\r\n";
            return ResponseInfo(TTL_NOT_FOUND, 5);
        }
