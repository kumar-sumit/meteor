...
private:
    ...
    // **SEGFAULT FIX: Disable work-stealing for pipeline=1 to prevent race conditions**
    void try_distribute_heavy_work() {
        // **ROOT CAUSE FIX**: Work-stealing creates race conditions with pending_operations_ vector
        // Pipeline=1 uses process_pending_batch() which conflicts with extract_work_task()
        // Pipeline>1 uses extract_work_task() which works fine with work-stealing
        // SOLUTION: Only enable work-stealing when we detect pipeline>1 patterns
        
        // **HEURISTIC**: If most recent operations are single responses, likely pipeline=1
        static size_t single_response_count = 0;
        static size_t total_operations = 0;
        static auto last_check = std::chrono::steady_clock::now();
        
        auto now = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_check).count() >= 5) {
            // Check every 5 seconds
            double single_response_ratio = (total_operations > 0) ? 
                (double)single_response_count / total_operations : 0.0;
                
            if (single_response_ratio > 0.8) {
                // **PIPELINE=1 DETECTED**: Disable work-stealing to prevent race conditions
                std::cout << "[SEGFAULT-FIX] Pipeline=1 detected (" << (single_response_ratio * 100) 
                          << "% single responses), disabling work-stealing" << std::endl;
                return; // NO WORK-STEALING
            }
        ...
        // **PIPELINE>1 PATH**: Work-stealing is safe here
        if (processor_->has_heavy_workload()) {
            ...
            if (work_task) {
                ...
                total_operations++; // Track for heuristic
            }
        }
    }
