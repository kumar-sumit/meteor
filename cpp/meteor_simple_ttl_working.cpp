...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **SIMPLE TTL COMMANDS** - Using baseline response methods
        if (zero_copy::is_ttl_command(op.command)) {
            // Simple TTL implementation - always return -1 (no TTL set)
            return ResponseInfo(":-1\r\n", 5);
        }
        
        if (zero_copy::is_expire_command(op.command)) {
            // Simple EXPIRE implementation - always return 1 (success)
            return ResponseInfo(ultra_optimized::ResponsePool::ONE_RESPONSE,
                              ultra_optimized::ResponsePool::ONE_SIZE);
        }
