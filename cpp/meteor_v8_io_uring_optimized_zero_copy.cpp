...
private:
    ...
    bool process_events_zero_syscall() {
        ...
        // **FIX #1.1: ASYNC COMPLETION PROCESSING**: Handle completed async I/O operations first
        ...
        for (const auto& event : events) {
            if (event.readable) {
                // **ASYNC PROCESSING**: Submit async receive operations (no blocking!)
    ...
    // **FIX #1: ASYNC RECEIVE OPERATION STRUCTURE**
    struct AsyncReceiveContext {
        int client_fd;
        char* buffer;
        CoreThread* core_thread;
        
        AsyncReceiveContext(int fd, CoreThread* thread) 
            : client_fd(fd), core_thread(thread) {
    ...
    void process_client_request(int client_fd) {
        // **ASYNC IO_URING RECEIVE**: No more blocking recv() calls!
        if (async_io_ && async_io_->initialized_) {
            // Create async receive context
            auto* ctx = new AsyncReceiveContext(client_fd, this);
            
            // Submit async receive operation
            bool submitted = async_io_->submit_recv(client_fd, ctx->buffer, 4095, ctx);
            if (!submitted) {
                // Fallback to sync if async fails
                delete ctx;
                process_client_request_sync_fallback(client_fd);
            }
            // Completion will be handled in process_async_completions()
        } else {
            // Fallback to sync receive
            process_client_request_sync_fallback(client_fd);
        }
    }
    
    // **FALLBACK**: Synchronous processing only when async fails
    void process_client_request_sync_fallback(int client_fd) {
        char buffer[4096];
        ...
        buffer[bytes_read] = '\0';
        process_received_data(std::string(buffer), client_fd);
        requests_processed_.fetch_add(1);
    }
    
    // **ASYNC COMPLETION PROCESSOR**: Handle completed async operations
    void process_async_completions() {
        if (!async_io_ || !async_io_->initialized_) return;
        
        // Check for completed async operations
        struct io_uring_cqe* cqe;
        while (io_uring_peek_cqe(&async_io_->ring_, &cqe) == 0) {
            auto* ctx = static_cast<AsyncReceiveContext*>(io_uring_cqe_get_data(cqe));
            
            if (cqe->res > 0) {
                // Successful receive
                ctx->buffer[cqe->res] = '\0';
                process_received_data(std::string(ctx->buffer), ctx->client_fd);
                requests_processed_.fetch_add(1);
            } else if (cqe->res == 0 || (cqe->res < 0 && cqe->res != -EAGAIN && cqe->res != -EWOULDBLOCK)) {
                // Connection closed or error
                close_client_connection(ctx->client_fd);
            }
            
            delete ctx;
            io_uring_cqe_seen(&async_io_->ring_, cqe);
        }
    }
    
    // **EXTRACTED COMMAND PROCESSING**: Common logic for sync and async
    void process_received_data(const std::string& data, int client_fd) {
