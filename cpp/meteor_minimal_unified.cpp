...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **MINIMAL UNIFIED APPROACH**: Process all commands using same logic, eliminate hanging
        for (const auto& cmd_data : commands) {
            ...
            if (!parsed_cmd.empty()) {
                ...
                // **SIMPLIFIED CORRECTNESS**: Always process locally first (eliminate cross-core complexity)
                if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                    // **DIRECT LOCAL PROCESSING**: Use working baseline logic (no routing for now)
                    processor_->submit_operation(command, key, value, client_fd);
                } else {
