...
public:
    ...
    void process_connection_pipeline_batch(int client_fd) {
        ...
        // **DRAGONFLY-STYLE: Single send() call for entire pipeline response**
        if (!conn_state->response_buffer.empty()) {
            ssize_t bytes_sent = send(client_fd, conn_state->response_buffer.c_str(), 
            ...
            if (bytes_sent < 0) {
                std::cerr << "❌ Failed to send pipeline response to client " << client_fd << std::endl;
            }
        }
...
// **Connection State - Tracks Pipeline Context**
...
private:
    void process_connection_migrations() {
        ...
        ConnectionMigrationMessage migration(0, 0, "", "", "", ""); // Default constructor with pipeline
        while (incoming_migrations_.dequeue(migration)) {
            ...
            if (!migration.pipeline_data.empty()) {
                // Process the entire pipeline that triggered migration
                parse_and_submit_commands(migration.pipeline_data, migration.client_fd);
