...
private:
    ...
    void run() {
        ...
        while (running_.load()) {
            ...
            // **ZERO-OVERHEAD**: Connection migration eliminated - direct routing used
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    // **ZERO-OVERHEAD PIPELINE CHECK**: Fast character-based command detection
                    std::string command = parsed_cmd[0];
                    std::string key = parsed_cmd.size() > 1 ? parsed_cmd[1] : "";
                    
                    if (!key.empty() && ((command[0] == 'G' || command[0] == 'g') || 
                                         (command[0] == 'S' || command[0] == 's') || 
                                         (command[0] == 'D' || command[0] == 'd'))) {
                        size_t key_target_core = (std::hash<std::string>{}(key) % total_shards_) % num_cores_;
            ...
            // **ZERO-OVERHEAD PIPELINE ROUTING**: Process mixed pipelines without migration
            if (!all_local && target_core != core_id_) {
                // Mixed pipeline - process each command on correct core directly
                for (const auto& cmd_batch : parsed_commands) {
                    if (cmd_batch.size() >= 2) {
                        std::string cmd = cmd_batch[0];
                        std::string key = cmd_batch[1]; 
                        std::string value = cmd_batch.size() > 2 ? cmd_batch[2] : "";
                        
                        if (!key.empty() && ((cmd[0] == 'G' || cmd[0] == 'g') || 
                                             (cmd[0] == 'S' || cmd[0] == 's') || 
                                             (cmd[0] == 'D' || cmd[0] == 'd'))) {
                            size_t key_core = (std::hash<std::string>{}(key) % total_shards_) % num_cores_;
                            if (key_core < all_cores_.size() && all_cores_[key_core] && all_cores_[key_core]->processor_) {
                                all_cores_[key_core]->processor_->submit_operation(cmd, key, value, client_fd);
                            } else {
                                processor_->submit_operation(cmd, key, value, client_fd);
                            }
                        } else {
                            processor_->submit_operation(cmd, key, value, client_fd);
                        }
            ...
            // **SINGLE COMMAND**: Route to correct core based on key hash
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    std::string command = parsed_cmd[0];
                    ...
                    // **ZERO-OVERHEAD ROUTING**: Ultra-fast character-based command detection
                    if (!key.empty() && ((command[0] == 'G' || command[0] == 'g') || 
                                         (command[0] == 'S' || command[0] == 's') || 
                                         (command[0] == 'D' || command[0] == 'd'))) {
                        
                        // **FAST HASH**: Direct computation without temporary variables
                        size_t key_target_core = (std::hash<std::string>{}(key) % total_shards_) % num_cores_;
                        
                        if (key_target_core != core_id_) {
                            // **ZERO-OVERHEAD CROSS-SHARD**: Direct processor access
                            if (key_target_core < all_cores_.size() && all_cores_[key_target_core] && 
                                all_cores_[key_target_core]->processor_) {
                                all_cores_[key_target_core]->processor_->submit_operation(command, key, value, client_fd);
                                return; // Processed on correct core without migration
                            }
                        }
                    }
                    
                    // Process locally (correct core for this key, or non-key command)
...
public:
    ThreadPerCoreServer(const std::string& host, int port, size_t num_cores, size_t num_shards,
        ...
        if (num_shards_ == 0) {
            // **ROUTING CORRECTNESS FIX**: Always use cores = shards for perfect key routing
            // This ensures each core owns exactly one shard, eliminating routing inconsistencies
            // where SET and GET commands for the same key hit different cores/caches
            num_shards_ = num_cores_;  // Perfect 1:1 mapping for consistent key routing
            std::cout << "🔧 ROUTING CORRECTNESS: " << num_cores_ 
