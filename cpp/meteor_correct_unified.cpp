...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **CORRECT UNIFIED APPROACH**: Use working baseline logic for all commands
        // Eliminate pipeline vs single branching while preserving proven routing
        
        if (commands.size() > 1) {
            // **MULTI-COMMAND**: Use pipeline processing (already works correctly)
            ...
            // **SINGLE COMMAND**: Use proven working baseline logic
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    ...
                    // **PROVEN WORKING LOGIC**: Use exact same routing as working baseline
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        ...
                        if (target_core == core_id_) {
                            // **LOCAL PROCESSING**: Use working submit_operation path
                            processor_->submit_operation(command, key, value, client_fd);
                        } else {
                            // **CROSS-CORE ROUTING**: Use working route_single_command_to_target_core
                            route_single_command_to_target_core(target_core, client_fd, command, key, value);
                        }
                    } else {
                        // **NON-KEY COMMANDS**: Process locally
                        processor_->submit_operation(command, key, value, client_fd);
                    }
                }
            }
