// **METEOR DRAGONFLY OPTIMIZED - LINEAR SCALING VERSION**
// 
// TARGET PERFORMANCE (DragonflyDB-Style Architecture):
// - 12C:6S:  8M+ ops/sec (optimal shard count, per-command routing)
// - 16C:8S:  12M+ ops/sec (linear scaling with core count)
...
public:
    ThreadPerCoreServer(const std::string& host, int port, size_t num_cores, size_t num_shards,
        ...
        if (num_shards_ == 0) {
            // **DRAGONFLY OPTIMIZATION**: Use fewer shards for linear scaling
            // Optimal shard count reduces cross-shard probability dramatically
            if (num_cores_ <= 4) {
                num_shards_ = num_cores_;           // 4C:4S
            } else if (num_cores_ <= 8) {
                num_shards_ = 4;                    // 8C:4S 
            } else if (num_cores_ <= 12) {
                num_shards_ = 6;                    // 12C:6S (optimal)
            } else if (num_cores_ <= 16) {
                num_shards_ = 8;                    // 16C:8S (optimal)
            } else {
                num_shards_ = std::min(num_cores_ / 2, size_t(12));  // Cap at 12 shards
            }
            std::cout << "🚀 DragonflyDB-optimized shard count: " << num_cores_ 
                      << " cores → " << num_shards_ << " shards (" 
                      << (100.0 * (num_shards_ - 1) / num_shards_) << "% cross-shard probability)" << std::endl;
        }
