// **PHASE 8 STEP 14: MEMC3 OPTIMISTIC CUCKOO SIMPLE**
// Implementing MemC3-style optimistic cuckoo hashing based on NSDI '13 paper:
// - Replace std::unordered_map with optimistic cuckoo hash table  
// - Concurrent readers with lock-free optimistic reads
// - Version counters for consistency without blocking readers
// - Bucketized design with 4 slots per bucket for >95% load factor
...
// **MEMC3 OPTIMISTIC CUCKOO HASH TABLE** (based on NSDI '13 paper)
namespace storage {
    
    /**
     * MemC3-style Optimistic Cuckoo Hash Table
     * Key features from the paper:
     * - Bucketized design (4 slots per bucket)
     * - Version counters for optimistic locking
     * - Concurrent readers, serialized writers
     * - >95% load factor achievable
     */
    class MemC3OptimisticCuckoo {
    public:
        static constexpr size_t SLOTS_PER_BUCKET = 4;  // MemC3 standard
        static constexpr size_t INITIAL_BUCKET_COUNT = 16384;
        static constexpr double MAX_LOAD_FACTOR = 0.95;
        
        using Tag = uint8_t;
        
        // **SLOT** (cache-aligned for performance)
        struct alignas(64) Slot {
            std::atomic<std::string*> key{nullptr};
            std::atomic<std::string*> value{nullptr};
            std::atomic<Tag> tag{0};
            
            bool is_empty() const {
                return key.load(std::memory_order_acquire) == nullptr;
            }
            
            bool matches_tag(Tag t) const {
                return tag.load(std::memory_order_acquire) == t;
            }
            
            void clear() {
                auto* old_key = key.exchange(nullptr, std::memory_order_acq_rel);
                auto* old_value = value.exchange(nullptr, std::memory_order_acq_rel);
        ...
        // **BUCKET** with version counter for optimistic locking
        struct alignas(64) Bucket {
            std::array<Slot, SLOTS_PER_BUCKET> slots;
            std::atomic<uint64_t> version{0};  // Odd = write in progress, Even = stable
            mutable std::mutex write_lock;     // Only writers use this
            
            // **OPTIMISTIC GET** (lock-free for readers)
            std::optional<std::string> optimistic_get(const std::string& key, Tag target_tag) const {
                uint64_t version_before, version_after;
                std::string result;
                
                do {
                    version_before = version.load(std::memory_order_acquire);
                    if (version_before & 1) continue;  // Writer active, retry
                    
                    // Scan slots for matching tag and key
                    for (const auto& slot : slots) {
                        if (slot.matches_tag(target_tag)) {
                            auto* key_ptr = slot.key.load(std::memory_order_acquire);
                            if (key_ptr && *key_ptr == key) {
                                auto* value_ptr = slot.value.load(std::memory_order_acquire);
                                if (value_ptr) {
                                    result = *value_ptr;
                                    break;
                                }
                    ...
                    version_after = version.load(std::memory_order_acquire);
                } while (version_before != version_after || (version_after & 1));
            ...
            int find_empty_slot() const {
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].is_empty()) return static_cast<int>(i);
                }
                return -1;
            }
            
            int find_key_slot(const std::string& key, Tag target_tag) const {
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].matches_tag(target_tag)) {
                        auto* key_ptr = slots[i].key.load(std::memory_order_acquire);
                        if (key_ptr && *key_ptr == key) {
    ...
    public:
        MemC3OptimisticCuckoo() : bucket_count_(INITIAL_BUCKET_COUNT), item_count_(0) {
        ...
        Tag hash_to_tag(size_t full_hash) const {
            return static_cast<Tag>((full_hash >> 24) & 0xFF);
        }
        
        std::pair<size_t, size_t> get_bucket_indices(const std::string& key) const {
            size_t h1 = hash1_(key);
            size_t h2 = hash2_(key + "_alt");
            size_t count = bucket_count_.load(std::memory_order_acquire);
            return {h1 % count, h2 % count};
        }
        
        // **OPTIMISTIC GET** (concurrent with writers)
        std::optional<std::string> get(const std::string& key) {
            size_t full_hash = hash1_(key);
            Tag target_tag = hash_to_tag(full_hash);
            auto [b1, b2] = get_bucket_indices(key);
            
            // Try both buckets with optimistic reads
            if (auto result = buckets_[b1]->optimistic_get(key, target_tag)) {
                return result;
            }
            if (auto result = buckets_[b2]->optimistic_get(key, target_tag)) {
                return result;
            }
        ...
        // **SET** (serialized writers)
        bool set(const std::string& key, const std::string& value) {
            size_t full_hash = hash1_(key);
            Tag target_tag = hash_to_tag(full_hash);
            auto [b1, b2] = get_bucket_indices(key);
            
            // Try direct insertion in both buckets
            for (size_t bucket_idx : {b1, b2}) {
                Bucket& bucket = *buckets_[bucket_idx];
                std::lock_guard<std::mutex> lock(bucket.write_lock);
                
                // Start write (increment to odd)
                bucket.version.fetch_add(1, std::memory_order_acq_rel);
                
                // Check if key exists (update case)
                int existing_slot = bucket.find_key_slot(key, target_tag);
                if (existing_slot >= 0) {
                    auto& slot = bucket.slots[existing_slot];
                    delete slot.value.exchange(new std::string(value), std::memory_order_acq_rel);
                    bucket.version.fetch_add(1, std::memory_order_acq_rel); // End write
                    return true;
                }
                ...
                if (empty_slot >= 0) {
                    ...
                    slot.tag.store(target_tag, std::memory_order_release);
            ...
            // **SIMPLIFIED CUCKOO** (single displacement only)
            for (size_t bucket_idx : {b1, b2}) {
                Bucket& bucket = *buckets_[bucket_idx];
                std::lock_guard<std::mutex> lock(bucket.write_lock);
                ...
                // Try to displace first item to its alternative bucket
        ...
        bool del(const std::string& key) {
            ...
            for (size_t bucket_idx : {b1, b2}) {
                ...
                int key_slot = bucket.find_key_slot(key, target_tag);
                if (key_slot >= 0) {
                    bucket.slots[key_slot].clear();
        ...
        double load_factor() const {
            size_t total_slots = bucket_count_.load(std::memory_order_acquire) * SLOTS_PER_BUCKET;
            return static_cast<double>(size()) / total_slots;
        }
    };
...
// **PHASE 3 STEP 3: VLL Hash Table (PRESERVED)**
    ...
    private:
        struct Shard {
            std::unique_ptr<MemC3OptimisticCuckoo> data;
            mutable std::shared_mutex mutex;  // Still needed for shard-level coordination
            std::atomic<size_t> operation_count{0};
            std::atomic<size_t> lock_contention{0};
            
            Shard() : data(std::make_unique<MemC3OptimisticCuckoo>()) {}
        };
