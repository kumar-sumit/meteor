    ...
    for (size_t cmd_idx = 0; cmd_idx < commands.size(); ++cmd_idx) {
        ...
        if (cmd_parts.size() >= 2) {
            ...
            if (target_shard == current_shard) {
                // **LOCAL FAST PATH**: Use pipeline mechanism but ensure data persistence
                BatchOperation op(command, key, value, client_fd);
                
                // **DIRECT CACHE OPERATION**: Bypass broken execute_single_operation, do direct cache ops
                std::string response;
                if (command == "SET" || command == "set") {
                    bool success = cache_->set(key, value);
                    response = success ? "+OK\r\n" : "-ERR failed\r\n";
                } else if (command == "GET" || command == "get") {
                    auto value_opt = cache_->get(key);
                    if (value_opt) {
                        response = "$" + std::to_string(value_opt->length()) + "\r\n" + *value_opt + "\r\n";
                    } else {
                        response = "$-1\r\n";
                    }
                } else if (command == "DEL" || command == "del") {
                    bool deleted = cache_->remove(key);
                    response = deleted ? ":1\r\n" : ":0\r\n";
                } else {
                    response = "+OK\r\n";
                }
                ...
                if (dragonfly_cross_shard::global_cross_shard_coordinator) {
                    ...
                    // Fallback: Direct cache operations (no coordinator available)
                    std::string response;
                    if (command == "SET" || command == "set") {
                        bool success = cache_->set(key, value);
                        response = success ? "+OK\r\n" : "-ERR failed\r\n";
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **TRUE UNIFIED APPROACH**: Use pipeline routing with proper data storage
        
        if (commands.size() > 1) {
            // **MULTIPLE COMMANDS**: Use proven pipeline processing
            ...
            // **SINGLE COMMANDS**: Use pipeline routing but preserve data storage mechanism
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        // **PIPELINE ROUTING**: Use same shard calculation as pipeline
                        ...
                        if (target_core == core_id_) {
                            // **PROVEN DATA STORAGE**: Use working submit_operation mechanism
                            processor_->submit_operation(command, key, value, client_fd);
                        } else {
                            // **PROVEN CROSS-CORE**: Use working routing mechanism
                            route_single_command_to_target_core(target_core, client_fd, command, key, value);
                        }
                    } else {
                        // **FALLBACK**: For non-key commands
                        processor_->submit_operation(command, key, value, client_fd);
                    }
                }
            }
