...
namespace cpu_affinity {
    ...
    inline int create_reuseport_socket(const std::string& host, int port, bool& reuseport_success) {
        ...
        // **v8.1 HIGH-LOAD FIX**: Optimize TCP settings for high concurrency
        // Enable TCP_NODELAY for low latency (disable Nagle algorithm)
        if (setsockopt(server_fd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) < 0) {
            std::cerr << "⚠️  TCP_NODELAY failed: " << strerror(errno) << " (continuing)" << std::endl;
        }
        ...
        if (setsockopt(server_fd, SOL_SOCKET, SO_RCVBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            std::cerr << "⚠️  SO_RCVBUF failed: " << strerror(errno) << " (continuing)" << std::endl;
        }
        if (setsockopt(server_fd, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            std::cerr << "⚠️  SO_SNDBUF failed: " << strerror(errno) << " (continuing)" << std::endl;
        }
        ...
        // **v8.1 HIGH-LOAD FIX**: Increase listen backlog for high concurrency
        int backlog = std::max(SOMAXCONN, 4096); // Higher backlog for connection bursts
...
public:
    ...
    void run_dedicated_accept() {
        ...
        while (running_.load()) {
            ...
            // **v8.1 HIGH-LOAD OPTIMIZATION**: Optimize client connection
            // Set non-blocking
            ...
            if (setsockopt(client_fd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) < 0) {
                // Don't log every failure under high load - just continue
            }
            
            // **CRITICAL**: Add directly to this core's event loop (no round-robin needed!)
...
private:
    ...
    bool process_events_zero_syscall() {
        ...
        // **v8.1 HIGH-LOAD OPTIMIZATION**: Adaptive sleep based on activity
        if (events.empty()) {
            // Under high load, reduce sleep time for faster responsiveness
            // Under low load, longer sleep to save CPU
            static thread_local int consecutive_empty = 0;
            consecutive_empty++;
            
            if (consecutive_empty < 10) {
                // High activity recently - short sleep for responsiveness
                std::this_thread::sleep_for(std::chrono::microseconds(1));  // 1μs when active
            } else {
                // Low activity - longer sleep to save CPU
                std::this_thread::sleep_for(std::chrono::microseconds(10)); // 10μs when idle
            }
            return true;
        }
        
        // Reset consecutive empty counter when we have events
        static thread_local int consecutive_empty = 0;
        consecutive_empty = 0;
