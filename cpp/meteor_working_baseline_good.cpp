...
public:
    ...
    void process_pending_batch() {
        ...
        for (auto& op : pending_operations_) {
            // **CRITICAL FIX**: Use string-based responses to avoid thread-local buffer corruption
            // Thread-local buffer gets overwritten between operations causing "response parsing failed"
            std::string response = execute_single_operation(op);  // Safe: separate memory per response
