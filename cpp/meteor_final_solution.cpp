...
namespace storage {
    ...
    private:
        ...
        // **MINIMAL TTL OVERLAY**: Separate table, zero impact on baseline
    ...
    public:
        ...
        std::optional<std::string> get(const std::string& key) {
            // **BASELINE PRESERVED**: Zero overhead for keys without TTL
            auto it = data_.find(key);
            if (it == data_.end()) return std::nullopt;
            
            // **CONDITIONAL TTL CHECK**: Only if key has TTL entry
            auto ttl_it = ttl_expiry_.find(key);
            if (ttl_it != ttl_expiry_.end() && std::chrono::steady_clock::now() >= ttl_it->second) {
                // Key expired
                data_.erase(it);
                ttl_expiry_.erase(ttl_it);
                return std::nullopt;
            }
        ...
        bool del(const std::string& key) {
            // **TRUE SHARED-NOTHING**: Direct access to core's own data (no locking needed!)
            ttl_expiry_.erase(key); // Clean TTL table if key had TTL
            return data_.erase(key) > 0;
        }
        
        // **MINIMAL TTL OVERLAY** - completely separate from baseline
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
            // Only works if key exists in baseline storage
        ...
        std::optional<std::chrono::seconds> ttl(const std::string& key) {
            ...
            if (now >= ttl_it->second) {
                ...
                data_.erase(key);
                ttl_expiry_.erase(ttl_it);
                return std::nullopt; // -2 in Redis (key expired)
            }
...
namespace cache {
    ...
    public:
        ...
        // **MINIMAL TTL OVERLAY** - pass through to VLLHashTable
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **MINIMAL TTL COMMANDS** - overlay on existing keys only
