// **PHASE 5 STEP 4A: SIMD + Lock-Free + Advanced Monitoring Implementation**
// Thread-per-core architecture with connection migration + SIMD vectorization + lock-free structures
...
// **STEP 4A: SIMD and Lock-Free Includes**
#include <immintrin.h>  // AVX2/AVX-512 SIMD instructions
...
// **PHASE 5 STEP 4A: SIMD Vectorized Hash Functions**
namespace simd {
    ...
    // SIMD-optimized hash function for batch processing
...
namespace lockfree {
    ...
    public:
        ...
        void enqueue(T item) {
            ...
            // Try to advance tail
            tail_.compare_exchange_weak(tail_.load(), newNode);
        }
...
// **PHASE 5 STEP 4A: Advanced Monitoring System**
...
public:
    DirectOperationProcessor(size_t memory_shards, const std::string& ssd_path, size_t max_memory_mb = 256, 
        ...
        std::cout << "🔥 STEP 4A Enhanced DirectOperationProcessor initialized:" << std::endl;
...
public:
    ...
    void stop() {
        ...
        migrations_cv_.notify_all(); // Wake up migration processing
        if (thread_.joinable()) {
            thread_.join();
        }
    }
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        for (const auto& cmd_data : commands) {
            ...
            if (!parsed_cmd.empty()) {
                ...
                // Route command to correct core based on key hash (for key-based commands)
                std::string cmd_upper = command;
                std::transform(cmd_upper.begin(), cmd_upper.end(), cmd_upper.begin(), ::toupper);
                
                if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                    // Route to the core that owns this key's shard
                    ...
                    if (target_core == core_id_) {
                        ...
                        // **CONNECTION MIGRATION**: Migrate entire connection to owning core
...
private:
    ...
    std::atomic<size_t> next_core_for_connection_{0};
...
public:
    ThreadPerCoreServer(const std::string& host, int port, size_t num_cores, size_t num_shards,
        ...
        : host_(host), port_(port), num_cores_(num_cores), num_shards_(num_shards),
          memory_mb_(memory_mb), ssd_path_(ssd_path), enable_logging_(enable_logging), server_fd_(-1) {
    ...
    bool start() {
        std::cout << "\n🚀 Starting Thread-Per-Core Meteor Server..." << std::endl;
        ...
        running_.store(true);
        accept_thread_ = std::thread(&ThreadPerCoreServer::accept_connections, this);
...
╚═╝     ╚═╝╚══════╝   ╚═╝   ╚══════╝ ╚═════╝ ╚═╝  ╚═╝
    ...
    bool initialize_server_socket() {
        ...
        if (bind(server_fd_, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
            std::cerr << "Failed to bind to " << host_ << ":" << port_ << ": " << strerror(errno) << std::endl;
            close(server_fd_);
            return false;
        }
    ...
    void start_core_threads() {
        std::cout << "🔧 Starting " << num_cores_ << " core threads..." << std::endl;
        ...
        std::cout << "✅ All core threads started with CPU affinity, connection migration, SIMD, lock-free structures, and monitoring enabled" << std::endl;
    }
    ...
    void accept_connections() {
        ...
        while (running_.load()) {
            ...
            // Round-robin distribution to cores
