// **PHASE 7 STEP 1: Incremental io_uring Integration**
// Building on Phase 6 Step 3 (working baseline) with:
// - Incremental io_uring support (falls back to epoll if needed)
// - Preserves all working functionality (pipeline + non-pipeline)
// - DragonflyDB-style optimizations
// - Linear scaling with cores
// - Performance target: Beat DragonflyDB
// Strategy: Add io_uring incrementally while keeping epoll working
...
// **PHASE 7 STEP 1: io_uring support (incremental)**
#ifdef HAS_IO_URING
#include <liburing.h>
...
private:
    ...
    // **PHASE 7 STEP 1: Incremental io_uring support**
#ifdef HAS_IO_URING
    struct io_uring ring_;
    bool io_uring_enabled_{false};  // Start with epoll, enable io_uring when ready
    std::atomic<uint64_t> io_uring_op_counter_{1};
    
    // Simple operation tracking for incremental implementation
    std::unordered_map<uint64_t, int> pending_io_accepts_;
    std::unordered_map<uint64_t, int> pending_io_reads_;
    std::mutex io_uring_mutex_;
#endif
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        // **PHASE 7 STEP 1: Try to initialize io_uring (incremental)**
#ifdef HAS_IO_URING
        if (io_uring_queue_init(256, &ring_, 0) == 0) {
            ...
            std::cout << "⚠️  Core " << core_id_ << " failed to initialize io_uring, using epoll" << std::endl;
        }
#endif
...
private:
...
#ifdef HAS_IO_URING
    // **PHASE 7 STEP 1: Incremental io_uring event processing**
    void process_events_io_uring() {
        // For now, implement a simple io_uring event loop
        ...
        if (ret == 0) {
            ...
            // Handle completions (simplified for incremental approach)
            if (pending_io_accepts_.count(op_id)) {
                handle_io_uring_accept_completion(op_id, result);
            } else if (pending_io_reads_.count(op_id)) {
                handle_io_uring_read_completion(op_id, result);
            }
    ...
    void handle_io_uring_accept_completion(uint64_t op_id, int result) {
        std::lock_guard<std::mutex> lock(io_uring_mutex_);
        pending_io_accepts_.erase(op_id);
        ...
        if (result > 0) {
            ...
            // Add to connections (will use epoll for now, io_uring for reads later)
            add_client_connection(client_fd);
    ...
    void handle_io_uring_read_completion(uint64_t op_id, int result) {
        std::lock_guard<std::mutex> lock(io_uring_mutex_);
        int client_fd = pending_io_reads_[op_id];
        pending_io_reads_.erase(op_id);
        
        if (result > 0) {
            // For incremental implementation, fall back to epoll-style processing
            // This will be enhanced to use io_uring buffers later
            handle_client_data_epoll_style(client_fd);
        } else if (result == 0) {
            // Client disconnected
    ...
    void handle_client_data_epoll_style(int client_fd) {
        // Temporary bridge: use existing epoll-style data handling
        // This allows us to keep the working pipeline processing while adding io_uring
        char buffer[16384];
        ...
        if (bytes_read > 0) {
            ...
            // Parse commands (handles both pipelined and non-pipelined)
            std::vector<std::vector<std::string>> parsed_commands;
            parse_commands_from_buffer(data, parsed_commands);
            
            if (!parsed_commands.empty()) {
                // Use existing working pipeline processing
    ...
    void parse_commands_from_buffer(const std::string& data, std::vector<std::vector<std::string>>& commands) {
        // Simple command parsing for incremental implementation
