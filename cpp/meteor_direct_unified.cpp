...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **DIRECT UNIFIED APPROACH**: Eliminate branching completely
        // Always use the same processing path regardless of command count
        for (const auto& cmd_data : commands) {
            ...
            if (!parsed_cmd.empty()) {
                ...
                // **SIMPLE UNIFIED PROCESSING**: Always use working submit_operation
                // No routing complexity, no dual paths, just direct processing
                processor_->submit_operation(command, key, value, client_fd);
            }
        }
    }
