// **PHASE 8 STEP 11: TRUE FREECACHE ARCHITECTURE**
// Implementing proper FreeCache architecture based on https://github.com/coocood/freecache:
// - ONE shard per core (not 256 shards per core!)
// - Each shard is a complete FreeCache with ring buffer
// - 256 internal segments within each FreeCache shard
// - Ring buffer for zero GC overhead
// - NO connection migration (DragonflyDB never migrates)
// Target: 1.9M+ RPS + zero GC overhead + unlimited keys
...
// **TRUE FREECACHE IMPLEMENTATION**
// Based on https://github.com/coocood/freecache architecture
namespace storage {
    class TrueFreeCache {
    private:
        // **FREECACHE CONSTANTS** (from original FreeCache)
        static constexpr size_t SEGMENT_COUNT = 256;  // FreeCache uses 256 segments
        static constexpr size_t INITIAL_RING_SIZE = 1024 * 1024;  // 1MB initial ring buffer
        
        // **RING BUFFER ENTRY** (zero GC overhead design)
        struct RingEntry {
            uint32_t key_len;
            uint32_t value_len;
            uint32_t expire_time;
            uint32_t next_offset;  // For collision chaining
            // Key and value data follows immediately after this header
        };
        
        // **FREECACHE SEGMENT** (one per segment for concurrent access)
        struct Segment {
            std::vector<uint8_t> ring_buffer;  // Ring buffer for entries
            std::vector<uint32_t> slot_table;  // Hash table (index into ring buffer)
            mutable std::shared_mutex mutex;   // Per-segment lock
            std::atomic<uint32_t> ring_head{0};
            std::atomic<uint32_t> ring_tail{0};
            std::atomic<size_t> entry_count{0};
            
            Segment() : ring_buffer(INITIAL_RING_SIZE), slot_table(1024, 0) {}
        };
        ...
        // **FREECACHE HASH FUNCTION** (segment selection)
        size_t hash_to_segment(const std::string& key) const {
            return std::hash<std::string>{}(key) % SEGMENT_COUNT;
        }
        
        // **RING BUFFER OPERATIONS**
        uint32_t write_entry_to_ring(Segment& segment, const std::string& key, const std::string& value) {
            size_t entry_size = sizeof(RingEntry) + key.length() + value.length();
            
            // Ensure ring buffer has space (simplified - no eviction for now)
            if (segment.ring_tail + entry_size >= segment.ring_buffer.size()) {
                segment.ring_tail = 0;  // Wrap around (simplified)
            }
            
            uint32_t offset = segment.ring_tail;
            RingEntry* entry = reinterpret_cast<RingEntry*>(&segment.ring_buffer[offset]);
            
            entry->key_len = key.length();
            entry->value_len = value.length();
            entry->expire_time = 0;  // No expiration for now
            entry->next_offset = 0;
            
            // Copy key and value immediately after header
            uint8_t* data_ptr = reinterpret_cast<uint8_t*>(entry + 1);
            std::memcpy(data_ptr, key.data(), key.length());
            std::memcpy(data_ptr + key.length(), value.data(), value.length());
            
            segment.ring_tail += entry_size;
            return offset;
        }
        
    public:
        TrueFreeCache(size_t ignored = 0)  // Ignore shard count - always use 256 segments
        ...
        bool set(const std::string& key, const std::string& value) {
            ...
            // Calculate slot in hash table
            size_t slot = std::hash<std::string>{}(key) % segment.slot_table.size();
            
            // Write entry to ring buffer
            uint32_t offset = write_entry_to_ring(segment, key, value);
            
            // Update hash table slot (simplified - no collision handling for now)
            segment.slot_table[slot] = offset;
            segment.entry_count.fetch_add(1);
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            // Calculate slot in hash table
            size_t slot = std::hash<std::string>{}(key) % segment.slot_table.size();
            uint32_t offset = segment.slot_table[slot];
            ...
            // Compare key
            std::string stored_key(reinterpret_cast<char*>(data_ptr), entry->key_len);
            if (stored_key == key) {
                // Return value
                std::string value(reinterpret_cast<char*>(data_ptr + entry->key_len), entry->value_len);
                return value;
            }
        ...
        bool del(const std::string& key) {
            ...
            // Verify key match before deletion
            RingEntry* entry = reinterpret_cast<RingEntry*>(&segment.ring_buffer[offset]);
            uint8_t* data_ptr = reinterpret_cast<uint8_t*>(entry + 1);
            std::string stored_key(reinterpret_cast<char*>(data_ptr), entry->key_len);
            
            if (stored_key == key) {
                segment.slot_table[slot] = 0;  // Mark as deleted
                segment.entry_count.fetch_sub(1);
                return true;
            }
        ...
        size_t size() const {
            ...
            for (const auto& segment_ptr : segments_) {
                total += segment_ptr->entry_count.load();
            }
            return total;
        }
        ...
        struct Stats {
            ...
            double avg_entries_per_segment;
        };
        
        Stats get_stats() const {
            Stats stats{SEGMENT_COUNT, 0, 0, 0.0};
            for (const auto& segment_ptr : segments_) {
                size_t entries = segment_ptr->entry_count.load();
                stats.total_entries += entries;
                stats.ring_buffer_usage += segment_ptr->ring_tail.load();
            }
            stats.avg_entries_per_segment = static_cast<double>(stats.total_entries) / SEGMENT_COUNT;
            return stats;
        }
    };
}
...
namespace cache {
    class HybridCache {
    private:
        std::unique_ptr<storage::TrueFreeCache> memory_cache_;
    ...
    public:
        HybridCache(size_t memory_shards, const std::string& ssd_path, size_t max_memory_mb = 256)
            ...
            memory_cache_ = std::make_unique<storage::TrueFreeCache>(memory_shards);  // Uses 256 segments internally
        ...
        struct Stats {
            storage::TrueFreeCache::Stats memory_stats;
