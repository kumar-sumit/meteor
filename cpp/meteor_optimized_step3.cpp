...
public:
    ...
    // **STEP 3 OPTIMIZATION**: Combined accept + processing for SO_REUSEPORT performance
    void run_with_accept(int server_fd) {
        std::cout << "🚀 Core " << core_id_ << " accept+process loop started" << std::endl;
        
        // **CRITICAL**: Initialize coordinator in event loop thread
        dragonfly_cross_shard::initialize_cross_shard_coordinator(total_shards_, core_id_);
        ...
        while (running_.load()) {
            ...
            if (client_fd >= 0) {
                ...
                // Add to this core's connection list
                add_client_connection(client_fd);
            }
            
            // **PROCESS EXISTING CONNECTIONS**: Same optimized event loop
            process_cross_shard_commands();
            process_pending_single_futures();
            ...
            // **BOOST.FIBERS**: Yield for cross-shard coordination
            boost::this_fiber::yield();
        }
        
        std::cout << "🛑 Core " << core_id_ << " accept+process loop stopped" << std::endl;
    }
            ...
            if (!parsed_cmd.empty()) {
                ...
                if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                    ...
                    if (target_shard != current_shard) {
                        ...
                        // **STEP 3**: Debug logging removed for performance
...
public:
    ...
    bool start() {
        ...
        // **STEP 3 OPTIMIZATION**: Set socket options with SO_REUSEPORT for multi-accept
        int opt = 1;
        ...
        // **CRITICAL**: SO_REUSEPORT allows multiple threads to accept on same port
        if (setsockopt(server_fd_, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt)) < 0) {
            std::cerr << "⚠️  Failed to set SO_REUSEPORT (falling back to single accept)" << std::endl;
        } else {
            std::cout << "✅ SO_REUSEPORT enabled for multi-accept performance!" << std::endl;
        }
        ...
        // **STEP 3 OPTIMIZATION**: Each core accepts connections directly (SO_REUSEPORT)
        for (size_t i = 0; i < num_cores_; ++i) {
            worker_threads_.emplace_back([this, i]() {
                ...
                std::cout << "🚀 Core " << i << " bound to CPU " << cpu_affinity::get_current_cpu() 
                          << " with dedicated accept+processing" << std::endl;
                
                // Set Boost.Fibers scheduler
                boost::fibers::use_scheduling_algorithm<boost::fibers::algo::round_robin>();
                
                // **BREAKTHROUGH**: Each core handles both accept AND processing (SO_REUSEPORT)
                core_threads_[i]->run_with_accept(server_fd_);
            });
        }
        
        // **KEEP MAIN THREAD ALIVE**: Wait for worker threads (essential for SO_REUSEPORT)
        std::cout << "✅ All cores started with SO_REUSEPORT multi-accept, waiting for shutdown..." << std::endl;
        
        // **CRITICAL**: Main thread must stay alive for worker threads to continue
        while (running_.load()) {
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
        
        std::cout << "🛑 Shutdown signal received, stopping cores..." << std::endl;
        return true;
    }
