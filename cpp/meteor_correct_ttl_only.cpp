...
namespace zero_copy {
    ...
    // **TTL-ONLY COMMAND DETECTION** - separate from baseline commands
...
namespace storage {
    ...
    public:
        struct Entry {
            ...
            // **TTL FIELDS - ONLY USED WHEN EXPLICITLY SET**
            ...
            // **TTL-ONLY CONSTRUCTOR** - separate from baseline
    ...
    public:
        ...
        bool set(const std::string& key, const std::string& value) {
            ...
            // **BASELINE BEHAVIOR UNCHANGED** - uses default Entry constructor (no TTL)
            data_[key] = Entry(key, value);
            return true;
        }
        
        // **TTL-ONLY EXTENSION** - separate method for TTL operations
        bool set_with_ttl(const std::string& key, const std::string& value, std::chrono::seconds ttl_seconds) {
            // **SEPARATE TTL PATH** - uses TTL constructor
            data_[key] = Entry(key, value, ttl_seconds);
            return true;
        }
        
        // **TTL-ONLY COMMAND** - check TTL for key
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            auto it = data_.find(key);
            if (it != data_.end()) {
                // **TTL CHECK - ONLY IF KEY HAS TTL** (zero impact on non-TTL keys)
                if (it->second.has_ttl && std::chrono::steady_clock::now() >= it->second.expiry_time) {
                    // Key expired - return null but don't remove (passive expiry)
                    return std::nullopt;
                }
                // **BASELINE BEHAVIOR UNCHANGED** for all non-TTL keys
...
namespace cache {
    ...
    public:
        ...
        // **TTL-ONLY EXTENSIONS** - pass-through to VLLHashTable TTL methods
        bool set_with_ttl(const std::string& key, const std::string& value, std::chrono::seconds ttl_seconds) {
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **TTL-ONLY COMMAND** - separate processing block
        if (zero_copy::is_ttl_command(op.command)) {
            ...
            return ResponseInfo(ttl_buffer, len, false); // Dynamic response
        }
