// **METEOR: DRAGONFLY VLL-INSPIRED CROSS-PIPELINE CORRECTNESS IMPLEMENTATION**
// Base: PHASE 8 STEP 23 IO_URING FIXED (4.92M QPS baseline)
// + DragonflyDB VLL: Production-proven cross-pipeline correctness
// 
// PRODUCTION APPROACH: Based on DragonflyDB's architecture at scale
...
// - 100% pipeline correctness with baseline performance preservation
// 
// DragonflyDB Key Insights Applied:
...
// Target: 4.9M+ RPS + <1ms P99 + 100% cross-pipeline correctness
...
// **DRAGONFLY VLL IMPLEMENTATION**
// Production-proven Very Lightweight Locking system for cross-pipeline correctness
namespace dragonfly_vll {

// **VLL INTENT LOCK MANAGER**: Based on DragonflyDB's production architecture
class VLLIntentLockManager {
private:
    static constexpr size_t LOCK_TABLE_SIZE = 8192;  // Power of 2 for fast modulo
    
    // **VLL INTENT ENTRY**: Ultra-lightweight atomic coordination structure  
    struct VLLIntentEntry {
        std::atomic<uint32_t> shared_readers{0};       // Concurrent reader count
        std::atomic<uint32_t> intent_exclusive{0};     // Exclusive intent flag (0/1)
        std::atomic<uint32_t> exclusive_owner{UINT32_MAX}; // Owner core ID
        
        // DragonflyDB-style: Ultra-fast shared lock acquisition
        bool try_acquire_shared() {
            ...
            if (intent_exclusive.load(std::memory_order_acquire) == 0) {
                ...
                // Double-check exclusive intent didn't appear during increment
                if (intent_exclusive.load(std::memory_order_acquire) == 0) {
                    return true;  // Success
                }
        ...
        // DragonflyDB-style: Ultra-fast exclusive intent acquisition
        bool try_acquire_exclusive(uint32_t core_id) {
            // Step 1: Try to set exclusive intent atomically
...
public:
    ...
    // Ensures cross-core pipeline correctness through lightweight intent locks
    bool acquire_pipeline_intent(const std::vector<std::string>& keys, uint32_t core_id, 
                                bool write_intent = false) {
        if (keys.empty()) return true;
        
        // Step 1: Sort keys by hash to prevent deadlock (critical for correctness)
        ...
        // Step 2: Try to acquire all locks atomically (all-or-nothing)
        ...
        for (const auto& [key, lock_idx] : sorted_key_indices) {
            ...
            if (success) {
                ...
                // Rollback all acquired locks on any failure
                ...
                return false;  // Pipeline intent acquisition failed
            }
        }
        
        return true;  // All pipeline intents acquired successfully
    }
...
// **DRAGONFLY VLL: Global intent lock manager (shared across all cores)**
static VLLIntentLockManager global_vll_manager;

// **DRAGONFLY PIPELINE COORDINATION**: Cross-core operation tracking
struct VLLPipelineOperation {
    ...
    VLLPipelineOperation(const std::string& cmd, const std::string& k, 
                        const std::string& val, uint32_t target)
        : command(cmd), key(k), value(val), target_core_id(target) {}
};

// **DRAGONFLY PIPELINE BATCH**: Coordinated pipeline execution with VLL
struct VLLPipelineBatch {
    std::vector<VLLPipelineOperation> operations;
    std::vector<std::string> responses;
    std::vector<std::string> locked_keys;  // For cleanup
    uint32_t client_fd;
    uint32_t coordinator_core_id;
    bool has_write_operations = false;
    
    VLLPipelineBatch(uint32_t fd, uint32_t coordinator)
        : client_fd(fd), coordinator_core_id(coordinator) {}
};
...
public:
    ...
    bool process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands, uint32_t core_id);
...
// Process entire pipeline as atomic batch - DragonflyDB VLL style
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands, uint32_t core_id) {
    ...
    // **DRAGONFLY VLL: Extract all keys for pipeline intent coordination**
    std::vector<std::string> pipeline_keys;
    bool has_write_operations = false;
    
    // Build pipeline commands and extract keys
    for (const auto& cmd_parts : commands) {
        if (cmd_parts.size() >= 2) {
            ...
            // Track keys for VLL intent coordination
            pipeline_keys.push_back(key);
            
            // Detect write operations for exclusive intent
            std::string cmd_upper = command;
            std::transform(cmd_upper.begin(), cmd_upper.end(), cmd_upper.begin(), ::toupper);
            if (cmd_upper == "SET" || cmd_upper == "DEL") {
    ...
    // **DRAGONFLY VLL: Acquire pipeline intent locks for cross-core correctness**
    if (!dragonfly_vll::global_vll_manager.acquire_pipeline_intent(pipeline_keys, core_id, has_write_operations)) {
        // VLL intent acquisition failed - pipeline conflict detected
        // Send error response for pipeline correctness
        std::string error_response = "-ERR pipeline intent conflict\r\n";
        send(client_fd, error_response.c_str(), error_response.length(), MSG_NOSIGNAL);
        return false;
    }
    
    // Process entire pipeline atomically with VLL protection
    bool result = execute_pipeline_batch(conn_state);
    
    // **DRAGONFLY VLL: Release pipeline intent locks**
    dragonfly_vll::global_vll_manager.release_pipeline_intent(pipeline_keys, core_id, has_write_operations);
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **DRAGONFLY VLL**: Process ALL pipelines with VLL intent coordination!
            // VLL ensures cross-core correctness while preserving shared-nothing performance
            processor_->process_pipeline_batch(client_fd, parsed_commands, core_id_);
        } else {
