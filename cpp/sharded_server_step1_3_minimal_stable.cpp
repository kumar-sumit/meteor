// **TEMPORAL COHERENCE STEP 1.3: MINIMAL STABLE CROSS-CORE CORRECTNESS**
// Minimal fix for cross-core pipeline correctness while preserving Step 1.2 stability
...
// - CROSS-CORE PIPELINES: Simple direct cross-core execution for correctness
// - STABILITY: Avoid complex temporal infrastructure that causes crashes
// Target: Achieve >=90% of Step 1.2 performance (>=3.4M QPS for 4 cores) + 100% correctness
...
public:
    ...
    // **STEP 1.3: Synchronous GET for cross-core correctness**
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            if (all_local) {
                ...
                // **CROSS-CORE PIPELINE**: Simple direct execution for correctness (STEP 1.3)
                temporal_metrics_.temporal_pipelines_detected.fetch_add(1);
                
                // **MINIMAL APPROACH**: Direct cross-core execution without complex temporal logic
                process_simple_cross_core_pipeline(parsed_commands, client_fd);
            }
        } else {
    ...
    // **STEP 1.3: SIMPLE CROSS-CORE PIPELINE PROCESSING**
    // Process cross-core pipelines with direct execution for 100% correctness
    void process_simple_cross_core_pipeline(const std::vector<std::vector<std::string>>& parsed_commands, int client_fd) {
        // **SIMPLE APPROACH**: Build response by executing commands directly on correct cores
        ...
        for (const auto& parsed_cmd : parsed_commands) {
            if (!parsed_cmd.empty()) {
                ...
                // **ROUTE TO CORRECT CORE**: Determine target core
                ...
                if (target_core == core_id_) {
                    // **LOCAL EXECUTION**: Execute locally
                    response = execute_command_locally(operation, key, value);
                } else {
                    // **CROSS-CORE EXECUTION**: Execute on target core
                    response = execute_command_on_target_core(operation, key, value, target_core);
        ...
        // **SEND RESPONSE**: Complete pipeline response to client
        send(client_fd, pipeline_response.c_str(), pipeline_response.length(), MSG_NOSIGNAL);
    }
    
    // **STEP 1.3: LOCAL COMMAND EXECUTION**
    std::string execute_command_locally(const std::string& operation, const std::string& key, const std::string& value) {
        ...
        if (cmd_upper == "SET") {
            ...
            // **SYNCHRONOUS GET**: Retrieve actual data
            auto value_opt = processor_->get_value_sync(key);
            if (value_opt) {
                return "$" + std::to_string(value_opt->length()) + "\r\n" + *value_opt + "\r\n";
            } else {
                return "$-1\r\n";
            }
        } else if (cmd_upper == "DEL") {
            processor_->submit_operation(operation, key, "", -1);
            processor_->flush_pending_operations(); // Ensure immediate commit
            return ":1\r\n";
    ...
    // **STEP 1.3: CROSS-CORE COMMAND EXECUTION**
    std::string execute_command_on_target_core(const std::string& operation, const std::string& key, const std::string& value, uint32_t target_core) {
        ...
        return target_thread->execute_command_locally(operation, key, value);
    }
