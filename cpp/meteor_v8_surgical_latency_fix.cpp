...
public:
    ...
    // **SURGICAL LATENCY FIX**: Process single commands immediately, preserve batching for multiple commands
    void submit_operation(const std::string& command, const std::string& key, 
        ...
        // **LATENCY PARADOX FIX**: Process single commands immediately (no 100μs delay)
        if (pending_operations_.size() == 1) {
            // Single command: process immediately using existing pipeline infrastructure
            process_pending_batch();
            return;
        }
        
        // **PRESERVE BATCHING**: Multiple commands use existing delay logic for efficiency
        auto now = std::chrono::steady_clock::now();
