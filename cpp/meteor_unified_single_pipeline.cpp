...
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands, size_t core_id, size_t total_shards) {
    ...
    for (size_t cmd_idx = 0; cmd_idx < commands.size(); ++cmd_idx) {
        ...
        if (cmd_parts.size() >= 2) {
            ...
            // **UNIFIED SIMPLIFIED**: Use core-based routing like working baseline
            // Map shard to core for consistent routing
            size_t target_core = target_shard % (total_shards / 4); // Assuming 4 cores, adjust if needed
            
            if (target_core == core_id) {
                // **LOCAL FAST PATH**: Execute immediately on correct core
                ...
                // **SIMPLIFIED CROSS-CORE**: Execute locally for now (will optimize later)
                // This ensures correctness first, then we can add cross-core optimizations
                BatchOperation op(command, key, value, client_fd);
                std::string response = execute_single_operation(op);
                response_trackers.emplace_back(cmd_idx, true, response);
            }
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **UNIFIED APPROACH**: Always treat commands as pipeline (single command = pipeline of 1)
        // This eliminates dual code paths and uses proven pipeline processing for everything
        ...
        if (!parsed_commands.empty()) {
            // **UNIFIED PROCESSING**: Use pipeline processing for ALL commands (1 or many)
            // This ensures single commands and pipeline commands use identical logic
