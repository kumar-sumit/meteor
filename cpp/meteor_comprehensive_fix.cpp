...
public:
    ...
    // **SEGFAULT FIX**: Thread-safe operation submission
    void submit_operation(const std::string& command, const std::string& key, 
                         const std::string& value, int client_fd) {
        try {
            // **SOCKET VALIDATION**: Check FD validity immediately
            if (client_fd < 0) {
                return; // Skip invalid FDs
            }
            
            // Add to batch for efficiency with bounds checking
            if (pending_operations_.size() < MAX_BATCH_SIZE * 2) { // **SAFETY**: Prevent unbounded growth
                pending_operations_.emplace_back(command, key, value, client_fd);
            } else {
                // **OVERFLOW PROTECTION**: Process immediately if queue is too full
                process_pending_batch();
                pending_operations_.emplace_back(command, key, value, client_fd);
            }
            ...
            // **ERROR RECOVERY**: Skip failed submissions
            return;
        }
    }
    ...
    std::optional<std::function<void()>> extract_work_task() {
        // **SEGFAULT FIX**: Add thread safety for concurrent access
        ...
        try {
            ...
            // **SAFETY**: Ensure we don't exceed available operations
            extract_count = std::min(extract_count, pending_operations_.size());
            if (extract_count == 0) {
                return std::nullopt;
            }
            ...
            for (size_t i = 0; i < extract_count; ++i) {
                if (pending_operations_.empty()) break; // **SAFETY**: Check before access
            ...
            // **SEGFAULT FIX**: Use completely safe version for work-stealing too
            // Work-stealing was also using dangerous thread-local buffers!
            return std::make_optional<std::function<void()>>([this, batch = std::move(work_batch)]() mutable {
            for (auto& op : batch) {
                try {
                    // **CRITICAL**: Use the same safe pipeline1 function to avoid thread-local corruption
                    std::string response = this->execute_single_operation_pipeline1_safe(op);
                    
                    // **SOCKET VALIDATION**: Check FD before sending
                    if (op.client_fd >= 0 && !response.empty()) {
                        ssize_t sent = send(op.client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
                        // Ignore send errors in work-stealing (client may have disconnected)
                    }
                    this->operations_processed_++;
                } catch (const std::exception& e) {
                    // **ERROR RECOVERY**: Skip failed operations in work-stealing
                    continue;
                }
            }
            });
        } catch (const std::exception& e) {
            // **ERROR RECOVERY**: Return null if extraction fails
            return std::nullopt;
        }
    }
...
public:
    ...
    void process_pending_batch() {
        ...
        // **PHASE 1 FIX**: RAII Response Management - prevent memory leaks and crashes
        // Use static thread-local map to avoid repeated allocations
        static thread_local std::unordered_map<int, std::vector<std::string>> client_responses;
        client_responses.clear(); // Clear previous batch data
        
        // **EXCEPTION SAFETY**: Process operations with proper error handling
        try {
            // Process operations and collect responses for batching
            for (auto& op : pending_operations_) {
                // **SOCKET VALIDATION**: Check if client FD is still valid
                if (op.client_fd < 0) {
                ...
                try {
                    ...
                    std::string response = execute_single_operation_pipeline1_safe(op);
                    
                    // **SAFE**: Store actual string objects with validation
                    if (!response.empty()) {
                    ...
                    // **ERROR RECOVERY**: Skip failed operations, continue processing
                    // Log error but don't crash the server
                    continue;
                }
            }
        } catch (const std::exception& e) {
            // **CRITICAL ERROR RECOVERY**: Clear state and return to prevent corruption
        ...
        for (auto& [client_fd, responses] : client_responses) {
            if (responses.size() == 1) {
                ...
                if (sent < 0) {
                    // **CONNECTION ERROR**: Client likely disconnected, skip
                    continue;
                }
                ...
                ssize_t sent = writev(client_fd, iov_array, responses.size());
                if (sent < 0) {
                    // **CONNECTION ERROR**: Client likely disconnected, skip
                    continue;
                }
                ...
                while (offset < responses.size()) {
                    ...
                    ssize_t sent = writev(client_fd, iov_array, chunk_size);
                    if (sent < 0) {
                        // **CONNECTION ERROR**: Stop sending to this client
                        break;
                    }
                    offset += chunk_size;
                }
    ...
    // **PHASE 1 CRASH FIX**: Memory-optimized safe version for pipeline=1 only  
    std::string execute_single_operation_pipeline1_safe(const BatchOperation& op) {
        // **FOR PIPELINE=1 ONLY**: Use optimized string allocation to reduce memory pressure
        if (zero_copy::is_get_command(op.command)) {
            auto value = cache_->get(std::string(op.key));
            if (value) {
                // **PHASE 1 FIX**: Pre-calculate exact size to avoid reallocations
                size_t length_str_size = std::to_string(value->length()).length();
                size_t total_size = 1 + length_str_size + 2 + value->length() + 2; // $<len>\r\n<data>\r\n
                
                std::string response;
                response.reserve(total_size); // **CRITICAL**: Single allocation
                response += "$";
                ...
                return "$-1\r\n"; // Static response, no allocation issues
            }
        }
        ...
        // **SEGFAULT FIX**: NEVER use thread-local buffers in pipeline=1 path
        // Implement all commands with safe string-based responses
