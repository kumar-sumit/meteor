// **PHASE 7 STEP 1: INCREMENTAL io_uring Integration (Complete Phase 6 Step 3 + io_uring)**
// Building on Phase 6 Step 3 (proven working baseline) with:
// - INCREMENTAL replacement of epoll with io_uring (DragonflyDB style)
// - PRESERVES ALL Phase 6 Step 3 optimizations:
...
//   * Multi-accept + CPU affinity with SO_REUSEPORT
//   * SIMD & AVX-512 optimizations
//   * SSD cache & hybrid storage
//   * Advanced bottleneck detection
// - Target: Beat DragonflyDB performance (1.5M+ RPS)
// - Strategy: Replace only epoll event handling with io_uring, keep everything else
...
// **PHASE 7 STEP 1: Add io_uring alongside existing epoll (incremental approach)**
#ifdef HAS_LINUX_EPOLL
#include <sys/epoll.h>
#include <sched.h>
#include <pthread.h>
#include <emmintrin.h>  // For _mm_pause()
// **NEW: Add io_uring support on Linux**
#include <liburing.h>
...
private:
    ...
    // **PHASE 7 STEP 1: io_uring response callback**
    std::function<void(int, const std::string&)> write_callback_;
    bool use_io_uring_write_{false};
...
public:
    ...
    // **PHASE 7 STEP 1: io_uring integration methods**
    void set_write_callback(std::function<void(int, const std::string&)> callback) {
        write_callback_ = callback;
        use_io_uring_write_ = true;
    }
    
    // **PHASE 7 STEP 1: Execute command for routing (DragonflyDB-style)**
    std::string execute_command_for_routing(const std::string& command, const std::string& key, 
                                           const std::string& value) {
        // Execute command locally and return response (no client_fd needed for routing)
        BatchOperation op(command, key, value, -1); // Use -1 as placeholder for routing
        return execute_single_operation(op);
    }
    
    void disable_io_uring_write() {
        use_io_uring_write_ = false;
...
private:
    ...
    void process_pending_batch() {
        ...
        for (auto& op : pending_operations_) {
            ...
            // **PHASE 7 STEP 1: Use io_uring write if available, otherwise fall back to send()**
            if (use_io_uring_write_ && write_callback_) {
                write_callback_(op.client_fd, response);
            } else {
    ...
    std::string execute_single_operation(const BatchOperation& op) {
        ...
        // **PHASE 7 STEP 1: Add CONFIG command support to eliminate redis-benchmark warnings**
        if (cmd_upper == "CONFIG") {
            // For BatchOperation, we only have command, key, value
            // CONFIG GET commands come as CONFIG with key as the subcommand
            std::string subcommand = op.key;
            std::transform(subcommand.begin(), subcommand.end(), subcommand.begin(), ::toupper);
            
            if (subcommand == "GET" || subcommand.empty()) {
                // Return minimal config for redis-benchmark compatibility
                return "*2\r\n$7\r\ntimeout\r\n$1\r\n0\r\n";
            } else if (subcommand == "SET") {
                // Accept config set but don't actually change anything
                return "+OK\r\n";
            }
            return "+OK\r\n"; // Default OK response for CONFIG commands
        }
...
bool DirectOperationProcessor::execute_pipeline_batch(std::shared_ptr<ConnectionState> conn_state) {
    ...
    // **PHASE 7 STEP 1: Single response call (io_uring or send) for entire pipeline response**
    bool success = false;
    if (!conn_state->response_buffer.empty()) {
        if (use_io_uring_write_ && write_callback_) {
            write_callback_(conn_state->client_fd, conn_state->response_buffer);
            success = true; // Assume success for async writes
        } else {
...
// **PHASE 7 STEP 1: DragonflyDB-style Command Routing Message**
struct CommandRoutingMessage {
    int client_fd;
    int source_core;
    std::string command;
    std::string key;
    ...
    CommandRoutingMessage(int fd, int src_core, const std::string& cmd, 
                         const std::string& k, const std::string& v)
        : client_fd(fd), source_core(src_core), command(cmd), key(k), value(v),
...
private:
    ...
    // **PHASE 7 STEP 1: DragonflyDB-style command routing (shared-nothing architecture)**
    lockfree::ContentionAwareQueue<CommandRoutingMessage> incoming_commands_;
    ...
    // **PHASE 7 STEP 1: Event loop with io_uring option**
#ifdef HAS_LINUX_EPOLL
    int epoll_fd_;
    std::array<struct epoll_event, 1024> events_;
    
    // **NEW: io_uring support alongside epoll**
#ifdef HAS_IO_URING
    bool use_io_uring_{false};  // Runtime flag for incremental testing
    struct io_uring ring_;
    // **PHASE 7 STEP 1: ADVANCED io_uring configuration (DragonflyDB style)**
    static constexpr int IO_URING_QUEUE_DEPTH = 1024; // Increased for advanced batching
    static constexpr int IO_BUFFER_SIZE = 65536;       // Larger buffers for better throughput
    static constexpr int READS_PER_CONNECTION = 1;     // **FIXED: Only 1 read per connection to prevent busy loops**
    static constexpr int FIXED_BUFFER_COUNT = 2048;    // Fixed buffers for zero-copy
    
    // **DragonflyDB-style multishot operations**
    bool multishot_enabled_{false};
    std::atomic<int> multishot_accept_count_{0};
    ...
    std::unordered_map<uint64_t, std::pair<int, std::string>> pending_writes_; // Store client_fd and data copy
    std::mutex io_ops_mutex_;
    
    // **PHASE 7 STEP 1: Batch submission optimization**
    std::atomic<uint32_t> pending_submissions_{0};
    static constexpr uint32_t BATCH_SUBMIT_THRESHOLD = 8; // Submit when we have 8+ pending ops
    
    // **PHASE 7 STEP 1: DragonflyDB-style buffer management for io_uring**
    struct IoBuffer {
        // **CRITICAL FIX: Properly aligned memory for io_uring**
        alignas(4096) char data[IO_BUFFER_SIZE];  // Page-aligned for optimal performance
        std::atomic<bool> in_use{false};         // Atomic to prevent race conditions
        int buffer_id{-1};                       // For fixed buffer registration
        std::atomic<int> ref_count{0};           // Reference counting for safe cleanup
        
        // **Make struct movable and copyable for vector operations**
        IoBuffer() = default;
        IoBuffer(const IoBuffer& other) : buffer_id(other.buffer_id) {
            in_use.store(other.in_use.load());
            ref_count.store(other.ref_count.load());
            std::memcpy(data, other.data, IO_BUFFER_SIZE);
        }
        IoBuffer(IoBuffer&& other) noexcept : buffer_id(other.buffer_id) {
            in_use.store(other.in_use.load());
            ref_count.store(other.ref_count.load());
            std::memcpy(data, other.data, IO_BUFFER_SIZE);
        }
    ...
    // **Connection-specific buffer tracking for safe cleanup**
    std::unordered_map<int, std::vector<char*>> connection_buffers_;
    std::mutex connection_buffers_mutex_;
    
    // **Fixed buffers for zero-copy operations**
    struct iovec* fixed_buffers_{nullptr};
    bool fixed_buffers_registered_{false};
    
    // **Advanced io_uring features**
    bool sqpoll_enabled_{false};
    std::atomic<uint32_t> active_reads_per_connection_{0};
#endif
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        // **PHASE 7 STEP 1: Initialize event system with optional io_uring**
#ifdef HAS_LINUX_EPOLL
        ...
        // **NEW: Try to initialize io_uring alongside epoll**
#ifdef HAS_IO_URING
        std::cout << "🔧 Core " << core_id_ << " HAS_IO_URING is defined - checking environment..." << std::endl;
        // Check if io_uring should be enabled (environment variable for testing)
        const char* enable_io_uring = std::getenv("METEOR_USE_IO_URING");
        if (enable_io_uring && std::string(enable_io_uring) == "1") {
            // **PHASE 7 STEP 1: Advanced io_uring initialization with SQPOLL**
            struct io_uring_params params = {};
            
            // **DRAGONFLY-STYLE: Enable SQPOLL for kernel-side polling (reduces syscalls)**
            const char* sqpoll_env = std::getenv("METEOR_USE_SQPOLL");
            if (sqpoll_env && std::string(sqpoll_env) == "1") {
                params.flags |= IORING_SETUP_SQPOLL;
                params.sq_thread_idle = 1000; // 1 second idle timeout
                sqpoll_enabled_ = true;
                std::cout << "🔥 Core " << core_id_ << " enabling SQPOLL mode for maximum performance" << std::endl;
            }
            
            int ret = io_uring_queue_init_params(IO_URING_QUEUE_DEPTH, &ring_, &params);
            if (ret >= 0) {
                use_io_uring_ = true;
                
                // Initialize advanced buffer pool
                io_buffers_.resize(FIXED_BUFFER_COUNT);
                for (size_t i = 0; i < io_buffers_.size(); ++i) {
                    io_buffers_[i].in_use = false;
                    io_buffers_[i].buffer_id = static_cast<int>(i);
                }
                
                // **Register fixed buffers for zero-copy operations**
                setup_fixed_buffers();
                
                std::cout << "✅ Core " << core_id_ << " initialized ADVANCED io_uring:" << std::endl;
                std::cout << "   • Queue depth: " << IO_URING_QUEUE_DEPTH << std::endl;
                std::cout << "   • Buffer count: " << FIXED_BUFFER_COUNT << std::endl;
                std::cout << "   • SQPOLL: " << (sqpoll_enabled_ ? "enabled" : "disabled") << std::endl;
                std::cout << "   • Fixed buffers: " << (fixed_buffers_registered_ ? "registered" : "disabled") << std::endl;
                std::cout << "   • SO_REUSEPORT: " << (num_cores_ > 1 ? "enabled" : "disabled") << " for multi-core scaling" << std::endl;
                
                // **Set up io_uring write callback for DirectOperationProcessor**
                processor_->set_write_callback([this](int client_fd, const std::string& data) {
                    this->submit_io_uring_write(client_fd, data);
                });
                
            } else {
                std::cout << "⚠️  Core " << core_id_ << " failed to initialize io_uring: " 
                          << strerror(-ret) << ", falling back to epoll" << std::endl;
                use_io_uring_ = false;
            }
        } else {
            std::cout << "🔧 Core " << core_id_ << " using epoll (set METEOR_USE_IO_URING=1 to enable io_uring)" << std::endl;
        }
#else
        std::cout << "⚠️  Core " << core_id_ << " HAS_IO_URING not defined - io_uring support not compiled" << std::endl;
#endif
    ...
    ~CoreThread() {
        ...
        // **PHASE 7 STEP 1: Clean up io_uring if initialized**
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            // **Cleanup fixed buffers**
            cleanup_fixed_buffers();
    ...
    void start() {
        ...
        // **PHASE 7 STEP 1: Skip dedicated accept thread if io_uring is handling accepts**
#ifdef HAS_IO_URING
        if (dedicated_accept_fd_ != -1 && !use_io_uring_) {
#else
...
#ifdef HAS_IO_URING
        if (use_io_uring_ && dedicated_accept_fd_ != -1) {
            std::cout << " + io_uring accept";
        } else if (dedicated_accept_fd_ != -1) {
            std::cout << " + dedicated accept";
        }
#else
        if (dedicated_accept_fd_ != -1) {
            std::cout << " + dedicated accept";
    ...
    // **PHASE 7 STEP 1: DragonflyDB-style command routing (shared-nothing)**
    void route_command_to_core(size_t target_core, int client_fd, 
                              const std::string& command, const std::string& key, 
                              const std::string& value) {
        if (target_core < all_cores_.size() && all_cores_[target_core]) {
            CommandRoutingMessage cmd_msg(client_fd, core_id_, command, key, value);
            all_cores_[target_core]->receive_routed_command(cmd_msg);
            
            if (metrics_) {
                metrics_->requests_migrated_out.fetch_add(1); // Reuse migration metrics for routing
            }
        }
    }
    
    void receive_routed_command(const CommandRoutingMessage& cmd_msg) {
        incoming_commands_.enqueue(cmd_msg);
        if (metrics_) {
            metrics_->requests_migrated_in.fetch_add(1); // Reuse migration metrics for routing
        }
    }
    
    // **PHASE 7 STEP 1: Send response back to client on source core**
    void send_response_to_client(int client_fd, const std::string& response) {
        // Send response directly using the connection's original core
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            submit_io_uring_write(client_fd, response);
        } else {
#endif
            // Fallback to blocking send for epoll mode
    ...
    void add_client_connection(int client_fd) {
        ...
        // **PHASE 7 STEP 1: Skip event loop registration if io_uring is enabled**
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            // io_uring handles events asynchronously, no need for epoll/kqueue registration
        } else {
#endif
#ifdef HAS_LINUX_EPOLL
...
private:
    void process_connection_migrations() {
        ...
        // **PHASE 7 STEP 1: Process routed commands (DragonflyDB-style shared-nothing)**
        CommandRoutingMessage cmd_msg(0, 0, "", "", ""); // Default constructor
        while (incoming_commands_.dequeue(cmd_msg)) {
            // Process the command locally and send response back to source core
            std::string response = processor_->execute_command_for_routing(
                cmd_msg.command, cmd_msg.key, cmd_msg.value);
            
            // Send response back to the source core for client communication
            if (cmd_msg.source_core < all_cores_.size() && all_cores_[cmd_msg.source_core]) {
                all_cores_[cmd_msg.source_core]->send_response_to_client(cmd_msg.client_fd, response);
            }
        }
    }
    ...
    void add_migrated_client_connection(int client_fd) {
        ...
        // **PHASE 7 STEP 1: Handle both epoll and io_uring modes for migration**
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            // **io_uring mode: Submit read operation for migrated connection**
            client_connections_.push_back(client_fd);
            submit_io_uring_read(client_fd);
            std::cout << "✅ Core " << core_id_ << " added migrated client (fd=" << client_fd << ") [io_uring]" << std::endl;
        } else {
#endif
#ifdef HAS_LINUX_EPOLL
            struct epoll_event ev;
...
#endif
            client_connections_.push_back(client_fd);
            std::cout << "✅ Core " << core_id_ << " added migrated client (fd=" << client_fd << ") [epoll]" << std::endl;
#ifdef HAS_IO_URING
        }
#endif
    }
    ...
    void run() {
        std::cout << "🚀 Core " << core_id_ << " event loop started";
        
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            std::cout << " (io_uring mode)" << std::endl;
            
            // **CRITICAL FIX: Submit multiple initial accept operations for io_uring**
            if (dedicated_accept_fd_ >= 0) {
                std::cout << "🔧 Core " << core_id_ << " submitting initial io_uring accepts..." << std::endl;
                // **DRAGONFLY-STYLE: Submit accepts based on configured cores, not hardware concurrency**
                int accept_count = std::max(2, static_cast<int>(num_cores_));
                std::cout << "🎯 Core " << core_id_ << " configured for " << num_cores_ << " total cores" << std::endl;
                for (int i = 0; i < accept_count; ++i) {
                    submit_io_uring_accept();
                }
                force_submit();
                std::cout << "✅ Core " << core_id_ << " initial " << accept_count << " accepts submitted" << std::endl;
            }
        } else {
            std::cout << " (epoll mode)" << std::endl;
        }
#else
        std::cout << std::endl;
#endif
        ...
        while (running_.load()) {
            // Process connection migrations first (keep Phase 6 Step 3 feature)
...
#ifdef HAS_IO_URING
    // **PHASE 7 STEP 1: FIXED io_uring event processing (DragonflyDB style)**
    void process_io_uring_events() {
        struct __kernel_timespec ts;
        ts.tv_sec = 0;
        ts.tv_nsec = 100000; // 0.1ms timeout - short to prevent blocking
        
        struct io_uring_cqe *cqe;
        uint32_t completions_processed = 0;
        
        // **CRITICAL FIX: Use non-blocking peek first, then wait only if needed**
        ...
        // **CRITICAL FIX: Only wait if no completions were processed**
        if (completions_processed == 0) {
            ...
            if (ret == 0) {
                ...
                // Process any additional completions that arrived
                while (io_uring_peek_cqe(&ring_, &cqe) == 0) {
                    handle_io_uring_completion(cqe);
                    io_uring_cqe_seen(&ring_, cqe);
                    completions_processed++;
        ...
        // **CRITICAL FIX: Always submit pending operations**
        if (pending_submissions_.load() > 0 || completions_processed > 0) {
            force_submit();
        }
        
        // **ENSURE ACCEPT AVAILABILITY: Maintain minimum pending accepts**
        if (pending_accepts_.size() < 2) {
            submit_io_uring_accept();
        }
        
        // **Track metrics**
        if (metrics_ && completions_processed > 0) {
            metrics_->requests_processed.fetch_add(completions_processed);
        }
    }
    ...
    void handle_accept_completion(uint64_t op_id, int result) {
        ...
        if (result > 0) {
            ...
            // **Set non-blocking (keep Phase 6 Step 3 behavior)**
            int flags = fcntl(client_fd, F_GETFL, 0);
            fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
            
            // **Add client connection using existing Phase 6 Step 3 logic**
            add_client_connection_io_uring(client_fd);
            ...
            std::cout << "⚠️  Core " << core_id_ << " accept error: " << strerror(-result) << std::endl;
        }
        
        // **CRITICAL FIX: Always resubmit accept unless multishot is active**
        if (!multishot_enabled_) {
            submit_io_uring_accept();
        }
    }
    ...
    void handle_read_completion(uint64_t op_id, int result) {
        ...
        if (result > 0) {
            // **CRITICAL FIX: Process data and keep connection alive**
            std::string data;
            data.reserve(result);
            ...
            std::cout << "📥 Core " << core_id_ << " read " << result << " bytes from fd=" << client_fd << std::endl;
            
            // **Use existing Phase 6 Step 3 command processing**
            parse_and_submit_commands(data, client_fd);
            
            // **CRITICAL FIX: Only resubmit read AFTER processing data (DragonflyDB style)**
            active_reads_per_connection_.fetch_sub(1);
            submit_io_uring_read(client_fd);
            
            // **ENSURE ACCEPT AVAILABILITY: Submit additional accept if needed**
            if (pending_accepts_.size() < 2) {
                submit_io_uring_accept();
            }
            ...
            // **CRITICAL FIX: result == 0 means EOF - client disconnected gracefully**
            // **Don't resubmit reads - this is the source of the infinite loop!**
            return_buffer(buffer);
            close_client_connection_safe(client_fd);
            std::cout << "🔌 Core " << core_id_ << " client fd=" << client_fd << " disconnected (EOF)" << std::endl;
            
        } else if (result == -EAGAIN || result == -EWOULDBLOCK) {
            // **Would block - don't resubmit immediately to avoid busy loop**
            return_buffer(buffer);
            std::cout << "⏳ Core " << core_id_ << " would block for fd=" << client_fd << ", not resubmitting" << std::endl;
            
        } else {
            // **Handle specific error codes**
            if (result == -ECONNRESET || result == -EPIPE || result == -ENOTCONN) {
                // **Real connection errors - close connection**
                return_buffer(buffer);
                std::cout << "🔌 Core " << core_id_ << " connection error for fd=" << client_fd 
                          << ": " << strerror(-result) << " - closing connection" << std::endl;
                close_client_connection_safe(client_fd);
            } else {
                // **Other errors - keep trying**
                return_buffer(buffer);
                submit_io_uring_read(client_fd);
                std::cout << "⚠️  Core " << core_id_ << " read error for fd=" << client_fd 
                          << ": " << strerror(-result) << " - retrying" << std::endl;
            }
        }
    }
    ...
    void handle_write_completion(uint64_t op_id, int result) {
        ...
        if (it != pending_writes_.end()) {
            ...
            if (result < 0) {
                ...
                std::cout << "⚠️  Core " << core_id_ << " write failed for client " << client_fd 
                          << ": " << strerror(-result) << std::endl;
                close_client_connection(client_fd);
            } else if (result > 0) {
                // **Successful write - update metrics**
                if (metrics_) {
                    // Use available metrics fields
                    metrics_->requests_processed.fetch_add(1);
                }
                // **DEBUG: Log successful writes occasionally**
                static std::atomic<uint64_t> write_count{0};
                if (write_count.fetch_add(1) % 1000 == 0) {
                    std::cout << "✅ Core " << core_id_ << " completed " << write_count.load() 
                              << " io_uring writes (last: " << result << " bytes)" << std::endl;
                }
            }
    ...
    // **PHASE 7 STEP 1: Fixed accept with proper resubmission**
    void submit_io_uring_accept() {
        struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
        if (!sqe) {
            // **CRITICAL FIX: If no SQE available, force submit and retry**
            force_submit();
            sqe = io_uring_get_sqe(&ring_);
            if (!sqe) {
                std::cerr << "❌ Core " << core_id_ << " failed to get SQE for accept" << std::endl;
                return;
            }
        }
        ...
        // **CRITICAL FIX: Use regular accept for stability, disable multishot for now**
        io_uring_prep_accept(sqe, dedicated_accept_fd_, nullptr, nullptr, 0);
        
        io_uring_sqe_set_data(sqe, reinterpret_cast<void*>(op_id));
        pending_accepts_[op_id] = dedicated_accept_fd_;
        
        // **CRITICAL FIX: Use batch submission for accepts**
        pending_submissions_.fetch_add(1);
        try_batch_submit();
    }
    
    // **PHASE 7 STEP 1: Advanced read submission with fixed buffer support**
    void submit_io_uring_read(int client_fd) {
        // Submit multiple reads per connection for better pipelining
        for (int i = 0; i < READS_PER_CONNECTION; ++i) {
            ...
            // **CRITICAL FIX: Track buffer for safe cleanup**
            track_buffer_for_connection(client_fd, buffer);
            
            uint64_t op_id = io_op_counter_.fetch_add(1);
            
                    // **PHASE 7 STEP 1: Use fixed buffers when available (DragonflyDB style)**
        bool use_fixed = fixed_buffers_registered_ && getenv("METEOR_USE_FIXED_BUFFERS");
        if (use_fixed) {
            int buffer_id = get_buffer_id(buffer);
            if (buffer_id >= 0) {
                // Use fixed buffer read for zero-copy performance
                io_uring_prep_read_fixed(sqe, client_fd, buffer, IO_BUFFER_SIZE, 0, buffer_id);
                // Only log in debug mode to avoid spam
                if (getenv("METEOR_DEBUG_FIXED_BUFFERS")) {
                    std::cout << "🔧 Core " << core_id_ << " using fixed buffer " << buffer_id << " for fd=" << client_fd << std::endl;
                }
            } else {
                // Fallback to regular recv if buffer ID not found
                io_uring_prep_recv(sqe, client_fd, buffer, IO_BUFFER_SIZE, 0);
                if (getenv("METEOR_DEBUG_FIXED_BUFFERS")) {
                    std::cout << "⚠️  Core " << core_id_ << " fallback to recv (buffer_id not found) for fd=" << client_fd << std::endl;
                }
            }
        } else {
            // Use regular recv when fixed buffers are not registered or disabled
            io_uring_prep_recv(sqe, client_fd, buffer, IO_BUFFER_SIZE, 0);
        }
            ...
            // **Track active reads per connection**
            active_reads_per_connection_.fetch_add(1);
        }
        
        // **OPTIMIZATION: Use batch submission**
        pending_submissions_.fetch_add(READS_PER_CONNECTION);
        try_batch_submit();
    }
    ...
    void submit_io_uring_write(int client_fd, const std::string& data) {
        ...
        // **Store a copy of the data to ensure it remains valid during async operation**
        pending_writes_[op_id] = {client_fd, data};
        
        // **Use the stored copy for the io_uring operation**
        const std::string& stored_data = pending_writes_[op_id].second;
        io_uring_prep_send(sqe, client_fd, stored_data.c_str(), stored_data.length(), MSG_NOSIGNAL);
        io_uring_sqe_set_data(sqe, reinterpret_cast<void*>(op_id));
    ...
    // **PHASE 7 STEP 1: Thread-safe buffer management with reference counting**
    ...
    void return_buffer(char* buffer_ptr) {
        ...
        for (auto& buf : io_buffers_) {
            if (buf.data == buffer_ptr) {
                int refs = buf.ref_count.fetch_sub(1);
                if (refs <= 1) {
    ...
    // **PHASE 7 STEP 1: Connection-specific buffer tracking for safe cleanup**
    void track_buffer_for_connection(int client_fd, char* buffer_ptr) {
        if (!buffer_ptr) return;
        
        std::lock_guard<std::mutex> lock(connection_buffers_mutex_);
        connection_buffers_[client_fd].push_back(buffer_ptr);
    }
    
    void cleanup_connection_buffers(int client_fd) {
        std::lock_guard<std::mutex> lock(connection_buffers_mutex_);
        auto it = connection_buffers_.find(client_fd);
        if (it != connection_buffers_.end()) {
            for (char* buffer_ptr : it->second) {
                return_buffer(buffer_ptr);
            }
            connection_buffers_.erase(it);
        }
    }
    
    // **PHASE 7 STEP 1: Safe connection close with proper cleanup**
    void close_client_connection_safe(int client_fd) {
        std::lock_guard<std::mutex> lock(connections_mutex_);
        
        // **CRITICAL FIX: Clean up connection buffers before closing**
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            cleanup_connection_buffers(client_fd);
            
            // **Clean up any pending io_uring operations for this client**
            std::lock_guard<std::mutex> io_lock(io_ops_mutex_);
            for (auto it = pending_reads_.begin(); it != pending_reads_.end();) {
                if (it->second.first == client_fd) {
                    return_buffer(it->second.second);
        ...
        std::cout << "🔌 Core " << core_id_ << " safely closed connection (fd=" << client_fd << ")" << std::endl;
    }
    
    // **PHASE 7 STEP 1: Optimized batch submission to reduce syscalls**
    void try_batch_submit() {
        uint32_t pending = pending_submissions_.load();
        if (pending >= BATCH_SUBMIT_THRESHOLD) {
            int submitted = io_uring_submit(&ring_);
            if (submitted > 0) {
                pending_submissions_.fetch_sub(submitted);
                if (metrics_) {
                    metrics_->requests_processed.fetch_add(submitted);
                }
            }
        }
    }
    ...
    void force_submit() {
        int submitted = io_uring_submit(&ring_);
        if (submitted > 0) {
            pending_submissions_.store(0);
        }
    }
    
    // **PHASE 7 STEP 1: Fixed buffer management for zero-copy operations**
    void setup_fixed_buffers() {
        if (!use_io_uring_) return;
        
        // **Initialize buffer IDs in the io_buffers_ array**
        for (size_t i = 0; i < io_buffers_.size(); ++i) {
            io_buffers_[i].buffer_id = static_cast<int>(i);
        }
        
        // Allocate iovec array for fixed buffer registration
        fixed_buffers_ = static_cast<struct iovec*>(malloc(FIXED_BUFFER_COUNT * sizeof(struct iovec)));
        if (!fixed_buffers_) {
            std::cout << "⚠️  Core " << core_id_ << " failed to allocate fixed buffer iovecs" << std::endl;
            return;
        }
        
        // Register each buffer with io_uring
        for (int i = 0; i < FIXED_BUFFER_COUNT; ++i) {
            fixed_buffers_[i].iov_base = io_buffers_[i].data;
            fixed_buffers_[i].iov_len = IO_BUFFER_SIZE;
        }
        
        int ret = io_uring_register_buffers(&ring_, fixed_buffers_, FIXED_BUFFER_COUNT);
        if (ret == 0) {
            fixed_buffers_registered_ = true;
            std::cout << "✅ Core " << core_id_ << " registered " << FIXED_BUFFER_COUNT << " fixed buffers for zero-copy" << std::endl;
        } else {
            std::cout << "⚠️  Core " << core_id_ << " failed to register fixed buffers: " << strerror(-ret) << std::endl;
            free(fixed_buffers_);
            fixed_buffers_ = nullptr;
        }
    }
    
    void cleanup_fixed_buffers() {
    ...
    // **Helper method to get buffer ID for fixed buffer operations**
    int get_buffer_id(char* buffer_ptr) {
        for (size_t i = 0; i < io_buffers_.size(); ++i) {
            if (io_buffers_[i].data == buffer_ptr) {
                // **CRITICAL FIX: Return the array index as buffer_id**
                // Fixed buffer registration uses array indices as buffer IDs
    ...
    void add_client_connection_io_uring(int client_fd) {
        std::lock_guard<std::mutex> lock(connections_mutex_);
        
        // **Skip epoll registration for io_uring mode, but keep Phase 6 Step 3 connection management**
        client_connections_.push_back(client_fd);
        
        // **Keep Phase 6 Step 3 metrics**
        if (metrics_) {
            metrics_->active_connections.fetch_add(1);
            metrics_->total_connections_accepted.fetch_add(1);
        }
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            if (all_local) {
                ...
                // **DRAGONFLY-STYLE PIPELINE**: Process all commands locally to avoid migration complexity
                // This ensures pipeline atomicity and connection stability
                processor_->process_pipeline_batch(client_fd, parsed_commands);
            }
        } else {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        // **DRAGONFLY-STYLE: Shared-nothing with message passing (no connection migration)**
                        size_t shard_id = std::hash<std::string>{}(key) % total_shards_;
                        size_t target_core = shard_id % num_cores_;
                        
                        if (target_core == core_id_) {
                            // This core owns the shard, process locally
                            processor_->submit_operation(command, key, value, client_fd);
                        } else {
                            // **MESSAGE PASSING**: Route command to owning core, keep connection stable
                            route_command_to_core(target_core, client_fd, command, key, value);
                        }
                    } else {
                        // Non-key commands (PING, etc.) can be processed on any core
                        processor_->submit_operation(command, key, value, client_fd);
                    }
                }
            }
