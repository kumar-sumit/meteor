// **PHASE 8 STEP 4: TRUE DRAGONFLY-STYLE SIMPLIFICATION**
...
// Target: 140K+ RPS, P99 < 10ms (restore Phase 6 Step 3 performance)
...
// **AVX-512 and Advanced Performance Includes**
...
namespace storage {
    // **ULTRA-SIMPLE WORKING HASH TABLE**
    // Forget complex Robin Hood - just use what works:
    // - std::unordered_map with simple locking
    // - Focus on architecture, not exotic data structures
    // - DragonflyDB's speed comes from shared-nothing, not hash table tricks
    class SimpleDragonflyTable {
    private:
        // SIMPLE APPROACH: Just use std::unordered_map with minimal locking
        mutable std::shared_mutex mutex_;
        std::unordered_map<std::string, std::string> data_;
        std::atomic<size_t> size_{0};
        
    public:
        SimpleDragonflyTable() = default;
        
        // SIMPLE: Just use std::unordered_map - it works!
        bool set(const std::string& key, const std::string& value) {
            std::unique_lock<std::shared_mutex> lock(mutex_);
            bool is_new = data_.find(key) == data_.end();
            data_[key] = value;
            if (is_new) {
        ...
        // Find slot containing the key (DragonflyDB-style linear probe)
        Slot* find_slot(uint64_t hash, const std::string& key) {
            size_t bucket_idx = get_bucket_index(hash);
            
            // CRITICAL FIX: Limited probe distance like real DragonflyDB
            constexpr size_t MAX_PROBE_DISTANCE = 64;  // DragonflyDB limit
            
            // Search regular buckets with limited probe distance
            for (size_t i = 0; i < MAX_PROBE_DISTANCE && i < REGULAR_BUCKETS; ++i) {
                size_t current_bucket = (bucket_idx + i) % REGULAR_BUCKETS;
                Bucket& bucket = buckets_[current_bucket];
                
                for (auto& slot : bucket.slots) {
                    uint64_t slot_hash = slot.hash.load(std::memory_order_acquire);
                    if (slot_hash == hash && slot.key == key) {
                        return &slot;
                    }
                    if (slot_hash == EMPTY_SLOT) {
                        goto check_stash;  // End of probe sequence for this bucket
                    }
                }
            }
            ...
            for (size_t i = REGULAR_BUCKETS; i < TOTAL_BUCKETS; ++i) {
                ...
                for (auto& slot : bucket.slots) {
                    if (slot.hash.load(std::memory_order_acquire) == hash && slot.key == key) {
                        return &slot;
                    }
                }
            }
        ...
        struct Stats {
            ...
            size_t regular_bucket_usage;
            size_t stash_bucket_usage;
        };
        ...
        Stats get_stats() const {
            ...
            for (size_t i = 0; i < REGULAR_BUCKETS; ++i) {
                for (const auto& slot : buckets_[i].slots) {
                    if (!slot.is_empty()) stats.regular_bucket_usage++;
                }
            }
            
            for (size_t i = REGULAR_BUCKETS; i < TOTAL_BUCKETS; ++i) {
                for (const auto& slot : buckets_[i].slots) {
                    if (!slot.is_empty()) stats.stash_bucket_usage++;
                }
            }
    ...
    // Shard-per-core wrapper (DragonflyDB approach)
    class VLLHashTable {
    private:
        struct Shard {
            SimpleDragonflyTable data;
            mutable std::shared_mutex mutex;  // Minimal locking
    ...
    public:
        VLLHashTable(size_t shard_count = 16) : shard_count_(shard_count) {
        ...
        bool set(const std::string& key, const std::string& value) {
            ...
            bool result = shard.data.set(key, value);
        ...
        bool del(const std::string& key) {
            ...
            std::unique_lock<std::shared_mutex> lock(shard.mutex);
            bool result = shard.data.del(key);
            if (result) {
                shard.operation_count.fetch_add(1);
            }
            return result;
        ...
        Stats get_stats() const {
            ...
            for (const auto& shard_ptr : shards_) {
                ...
                // No lock_contention in SimpleDragonflyTable
            }
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **TRUE DRAGONFLY BEHAVIOR**: Process ALL commands locally, no migration
            // DragonflyDB never migrates connections - each core processes all keys
            processor_->process_pipeline_batch(client_fd, parsed_commands);
            
            if (false) { // Disabled migration logic
                if (metrics_) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        ...
                        // **TRUE DRAGONFLY BEHAVIOR**: Process ALL commands locally
                        // DragonflyDB never migrates - each core has access to entire keyspace
