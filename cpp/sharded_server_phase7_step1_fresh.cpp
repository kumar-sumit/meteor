// **PHASE 7 STEP 1: io_uring Integration (Fresh Implementation)**
// Building on Phase 6 Step 3 (working baseline) with:
// - Native io_uring support like DragonflyDB
// - Per-connection buffer management
// - Proper async command processing
// - Both pipelined and non-pipelined support
// - Linear scaling with cores
// Preserving all Phase 6 optimizations
...
private:
    ...
    // **PHASE 7 STEP 1: io_uring buffer management**
    ...
    std::vector<IoUringBuffer> io_buffers_;
    std::mutex io_buffer_mutex_;
    
    // io_uring operation tracking
    std::atomic<uint64_t> io_op_counter_{1};
    std::unordered_map<uint64_t, int> pending_accepts_;
    std::unordered_map<uint64_t, std::pair<int, IoUringBuffer*>> pending_reads_;
...
public:
...
#ifdef HAS_IO_URING
        // Try to initialize io_uring first
        if (io_uring_queue_init(256, &ring_, 0) == 0) {
            use_io_uring_ = true;
            
            // Initialize buffer pool
            io_buffers_.reserve(BUFFER_COUNT);
            for (size_t i = 0; i < BUFFER_COUNT; ++i) {
                IoUringBuffer buffer;
            ...
            use_io_uring_ = false;
            std::cout << "⚠️  Core " << core_id_ << " failed to initialize io_uring, falling back to epoll" << std::endl;
        }
#endif
    ...
    ~CoreThread() {
        ...
        if (use_io_uring_) {
            ...
            // Clean up buffers
            for (auto& buffer : io_buffers_) {
                delete[] buffer.data;
            }
        }
#endif
#ifdef HAS_LINUX_EPOLL
        if (!use_io_uring_ && epoll_fd_ >= 0) {
            close(epoll_fd_);
        }
#elif defined(HAS_MACOS_KQUEUE)
        if (kqueue_fd_ >= 0) {
...
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            // With io_uring, we don't need to add to epoll
...
private:
    ...
    void process_events_io_uring() {
        // Submit any pending accept operations
        if (dedicated_accept_fd_ >= 0 && pending_accepts_.empty()) {
            submit_accept_io_uring();
        }
        ...
        // Wait for at least one completion if nothing was processed
        if (count == 0) {
    ...
    void submit_read_io_uring(int client_fd) {
        // Get a free buffer
        IoUringBuffer* buffer = nullptr;
        {
            std::lock_guard<std::mutex> lock(io_buffer_mutex_);
        ...
        if (!buffer) {
            // No buffers available, fall back to regular read later
            return;
        }
        ...
        uint64_t op_id = io_op_counter_.fetch_add(1);
        io_uring_prep_recv(sqe, client_fd, buffer->data, buffer->size, 0);
        io_uring_sqe_set_data(sqe, reinterpret_cast<void*>(op_id));
        
        pending_reads_[op_id] = {client_fd, buffer};
    }
    
    void submit_write_io_uring(int client_fd, const std::string& data) {
        // For simplicity, we'll use regular send for writes
        // In production, you'd want to use io_uring_prep_send with proper buffer management
    ...
    void process_io_uring_completion(struct io_uring_cqe *cqe) {
        ...
        if (pending_accepts_.count(op_id)) {
            ...
            if (result > 0) {
                ...
                add_client_connection(client_fd);
                submit_read_io_uring(client_fd);
            }
        ...
        if (pending_reads_.count(op_id)) {
            auto [client_fd, buffer] = pending_reads_[op_id];
            pending_reads_.erase(op_id);
            
            if (result > 0) {
                // Process the received data
                std::string data(buffer->data, result);
                handle_client_data_io_uring(client_fd, data);
    ...
    void handle_client_data_io_uring(int client_fd, const std::string& data) {
        // Get or create connection state
        auto conn_state = processor_->get_or_create_connection_state(client_fd);
        if (!conn_state) return;
        
        // Accumulate data in partial command buffer
        ...
        while (pos < conn_state->partial_command_buffer.length()) {
            size_t end = conn_state->partial_command_buffer.find("\r\n", pos);
            if (end == std::string::npos) break;
            
            std::string line = conn_state->partial_command_buffer.substr(pos, end - pos);
            if (!line.empty()) {
                // Simple parsing - split by space for basic commands
                ...
                if (!cmd_parts.empty()) {
                    parsed_commands.push_back(cmd_parts);
                }
            }
            pos = end + 2;
        }
        ...
        if (pos > 0) {
            conn_state->partial_command_buffer.erase(0, pos);
        }
