...
namespace storage {
    ...
    private:
        ...
        static constexpr size_t INITIAL_BUCKET_COUNT = 1024; // Initial size
        ...
        // **CUCKOO PATH NODE** (for BFS pathfinding)
        struct PathNode {
            size_t bucket_idx;
            int slot_idx;
            size_t depth;
            
            PathNode(size_t b, int s, size_t d) : bucket_idx(b), slot_idx(s), depth(d) {}
        };
        ...
        // **BFS PATHFINDING** (find shortest displacement path)
        std::vector<PathNode> find_cuckoo_path(size_t start_bucket) {
            std::queue<PathNode> queue;
            std::unordered_set<size_t> visited;
            
            queue.emplace(start_bucket, -1, 0);
            visited.insert(start_bucket);
            
            while (!queue.empty() && queue.front().depth < MAX_CUCKOO_PATH) {
                ...
                // Try displacing each slot to its alternative bucket
                for (int slot_idx = 0; slot_idx < SLOTS_PER_BUCKET; ++slot_idx) {
                    ...
                    if (visited.find(alt_bucket) == visited.end()) {
                        ...
                        queue.emplace(alt_bucket, slot_idx, current.depth + 1);
                    }
                }
            }
    ...
    public:
        ...
        // Cuckoo insertion with BFS pathfinding
        bool cuckoo_insert_with_path(size_t start_bucket, Tag tag, const std::string& key, const std::string& value) {
            ...
            if (path.empty()) {
                // No path found - table might need resizing
                return false;
            }
            
            // **PHASE 2: FINE-GRAINED LOCKING EXECUTION**
            // For simplicity, just insert in the first available slot we find
            // (A full implementation would execute the complete cuckoo path)
            for (size_t bucket_idx = 0; bucket_idx < bucket_count_.load(); ++bucket_idx) {
                Bucket& bucket = *buckets_[bucket_idx];
                std::lock_guard<std::mutex> lock(bucket.write_lock);
