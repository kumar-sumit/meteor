...
namespace storage {
    ...
    public:
        ...
        bool set(const std::string& key, const std::string& value) {
            // **LAZY EXPIRATION**: Remove if exists and expired
            lazy_expire_if_needed(key);
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            auto it = data_.find(key);
            if (it != data_.end()) {
                // **LRU UPDATE**: Update access time and LRU clock
...
public:
    ...
    void submit_operation_with_ttl(const std::string& command, const std::string& key, 
        ...
        // Add to batch with TTL for efficiency
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    // **COMMERCIAL CACHE: Handle TTL commands**
                    bool handled = false;
                    ...
                    if (!handled && cmd_upper == "EXPIRE" && parsed_cmd.size() >= 3) {
                        // For EXPIRE command, value field contains the seconds
