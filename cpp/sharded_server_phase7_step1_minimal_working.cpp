// **PHASE 7 STEP 1: MINIMAL Working io_uring Integration (Based on Phase 6 Step 3)**
// **DragonflyDB-Style Event-Driven io_uring Implementation**:
// - Event-driven I/O (no continuous polling)
// - Request-response cycle maintains connection state  
// - Proper accept resubmission for redis-benchmark compatibility
// - All previous Phase 6 Step 3 optimizations preserved
// - Works with redis-benchmark, memtier-benchmark, and real workloads
...
private:
    ...
    // **PHASE 7 STEP 1: Response sender callback for io_uring compatibility**
...
private:
    ...
    void process_pending_batch() {
        ...
        for (auto& op : pending_operations_) {
            ...
            // Send response immediately (no queuing) - use io_uring if available
            send_response(op.client_fd, response);
    ...
    // **PHASE 7 STEP 1: Response sender interface for io_uring compatibility**
    bool send_response(int client_fd, const std::string& response) {
        if (response_sender_) {
            return response_sender_(client_fd, response);
        }
        // Fallback to traditional send with error handling
        ssize_t bytes_sent = send(client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
        if (bytes_sent <= 0) {
            if (errno != EWOULDBLOCK && errno != EAGAIN && errno != EPIPE && errno != ECONNRESET) {
...
// **PHASE 7 STEP 1: Set response sender callback for io_uring**
void DirectOperationProcessor::set_response_sender(std::function<bool(int, const std::string&)> sender) {
    response_sender_ = sender;
}
...
bool DirectOperationProcessor::execute_pipeline_batch(std::shared_ptr<ConnectionState> conn_state) {
    ...
    // Single send() call for entire pipeline response - use io_uring if available
    bool success = false;
    if (!conn_state->response_buffer.empty()) {
        success = send_response(conn_state->client_fd, conn_state->response_buffer);
    }
...
#ifdef HAS_IO_URING
    // **PHASE 7 STEP 1: MINIMAL io_uring support (event-driven, DragonflyDB style)**
    bool use_io_uring_{false};
    struct io_uring ring_;
    static constexpr int IO_URING_QUEUE_DEPTH = 256;
    static constexpr int IO_BUFFER_SIZE = 8192;
...
public:
...
#ifdef HAS_IO_URING
        // **PHASE 7 STEP 1: Initialize io_uring if environment variable is set**
        if (std::getenv("METEOR_USE_IO_URING")) {
            ...
            if (ret == 0) {
                ...
                buffer_pool_.reserve(64);
                for (int i = 0; i < 64; ++i) {
                    buffer_pool_.push_back(new char[IO_BUFFER_SIZE]);
                }
                
                std::cout << "✅ Core " << core_id_ << " initialized MINIMAL io_uring:" << std::endl;
                std::cout << "   • Queue depth: " << IO_URING_QUEUE_DEPTH << std::endl;
                std::cout << "   • Buffer size: " << IO_BUFFER_SIZE << std::endl;
                std::cout << "   • Event-driven (DragonflyDB style)" << std::endl;
                
                // **PHASE 7 STEP 1: Set io_uring response sender callback**
                processor_->set_response_sender([this](int client_fd, const std::string& response) {
                    submit_io_uring_write(client_fd, response);
                    return true;
                });
            } else {
                std::cout << "⚠️  Core " << core_id_ << " io_uring init failed: " << strerror(-ret) << ", using epoll" << std::endl;
                use_io_uring_ = false;
            }
        }
#endif
    ...
    void start() {
        ...
        // **PHASE 7 STEP 1: Skip dedicated accept thread when using io_uring**
#ifdef HAS_IO_URING
        bool should_start_accept_thread = (dedicated_accept_fd_ != -1) && !use_io_uring_;
#else
        bool should_start_accept_thread = (dedicated_accept_fd_ != -1);
#endif
        if (should_start_accept_thread) {
...
#ifdef HAS_IO_URING
        if (use_io_uring_) {
            accept_method = " + io_uring accept";
        } else if (dedicated_accept_fd_ != -1) {
            accept_method = " + dedicated accept";
        }
#else
...
#endif
        std::cout << "🚀 Core " << core_id_ << " started with mandatory CPU affinity" << accept_method << std::endl;
    }
...
private:
    ...
    void run() {
        ...
        if (use_io_uring_) {
            ...
            // **DRAGONFLY-STYLE: Submit initial accepts for event-driven processing**
            if (dedicated_accept_fd_ >= 0) {
                std::cout << "🔧 Core " << core_id_ << " submitting initial io_uring accepts..." << std::endl;
                submit_io_uring_accept();
                submit_io_uring_accept(); // Submit 2 initial accepts
                // **CRITICAL: Submit initial accepts immediately**
                io_uring_submit(&ring_);
                std::cout << "✅ Core " << core_id_ << " initial accepts submitted" << std::endl;
            }
        } else {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        ...
                        if (target_core == core_id_) {
                            // **LOCAL PROCESSING**: Process on current core for optimal performance
                            processor_->submit_operation(command, key, value, client_fd);
                        } else {
                            // **PIPELINE MIGRATION**: Restore optimal pipeline performance
...
#ifdef HAS_IO_URING
    // **PHASE 7 STEP 1: DragonflyDB-Style Event-Driven io_uring Methods**
    
    void process_io_uring_events() {
        struct __kernel_timespec ts;
        ts.tv_sec = 0;
        ts.tv_nsec = 10000000; // 10ms timeout - DragonflyDB style
        
        struct io_uring_cqe *cqe;
        
        // **PERFORMANCE OPTIMIZATION: Submit any pending operations first**
        // io_uring_submit(&ring_); // Not needed - operations submit individually
        
        // **DRAGONFLY-STYLE: Non-blocking event processing**
        while (io_uring_peek_cqe(&ring_, &cqe) == 0) {
            handle_io_uring_completion(cqe);
            io_uring_cqe_seen(&ring_, cqe);
        }
        
        // **DRAGONFLY-STYLE: Only wait if no events were processed**
    ...
    void handle_accept_completion(uint64_t op_id, int result) {
        ...
        // **DRAGONFLY-STYLE: Always resubmit accept for continuous availability**
        submit_io_uring_accept();
        // **CRITICAL: Submit accept immediately to maintain availability**
        io_uring_submit(&ring_);
    }
    ...
    void handle_read_completion(uint64_t op_id, int result) {
        ...
        if (result > 0) {
            // **DRAGONFLY-STYLE: Process data and maintain connection**
            std::string data(buffer, result);
            return_buffer(buffer);
            
            // std::cout << "📥 Core " << core_id_ << " read " << result << " bytes from fd=" << client_fd << std::endl;
            
            // Process command using existing Phase 6 Step 3 logic
            process_client_data(data, client_fd);
            
            // **DRAGONFLY-STYLE: Resubmit read for continuous processing**
            submit_io_uring_read(client_fd);
            // **CRITICAL: Submit read immediately to maintain responsiveness**
            io_uring_submit(&ring_);
            
        } else if (result == 0) {
            // **DRAGONFLY-STYLE: Client disconnected gracefully**
            return_buffer(buffer);
            close_client_connection(client_fd);
            std::cout << "🔌 Core " << core_id_ << " client fd=" << client_fd << " disconnected" << std::endl;
            
        } else {
            // **DRAGONFLY-STYLE: Handle errors appropriately**
            return_buffer(buffer);
            if (result == -EAGAIN || result == -EWOULDBLOCK) {
                // Would block - resubmit read
                submit_io_uring_read(client_fd);
                io_uring_submit(&ring_);
            } else {
    ...
    void handle_write_completion(uint64_t op_id, int result) {
        ...
        if (result > 0) {
            // std::cout << "✅ Core " << core_id_ << " completed write (" << result << " bytes)" << std::endl;
        } else if (result < 0) {
            std::cout << "⚠️  Core " << core_id_ << " write error: " << strerror(-result) << std::endl;
        }
    }
    ...
    void submit_io_uring_accept() {
        ...
        // **CRITICAL: Submit accept immediately for continuous connection handling**
        io_uring_submit(&ring_);
    }
    ...
    void submit_io_uring_read(int client_fd) {
        ...
        // **CRITICAL: Submit read immediately for continuous operation**
        io_uring_submit(&ring_);
    }
    ...
    void submit_io_uring_write(int client_fd, const std::string& data) {
        ...
        io_uring_prep_send(sqe, client_fd, data.c_str(), data.size(), 0);
        io_uring_sqe_set_data(sqe, reinterpret_cast<void*>(op_id));
        
        pending_writes_[op_id] = client_fd;
        // **CRITICAL: Submit write immediately for response delivery**
        io_uring_submit(&ring_);
    }
    ...
    char* get_buffer() {
        ...
        if (buffer_pool_.empty()) {
            return new char[IO_BUFFER_SIZE];
        }
    ...
    void process_client_data(const std::string& data, int client_fd) {
        // **DRAGONFLY-STYLE: Use existing Phase 6 Step 3 command processing**
        std::vector<std::string> commands = parse_resp_commands(data);
        
        for (const std::string& cmd_data : commands) {
            std::vector<std::string> parts = parse_single_resp_command(cmd_data);
            
            if (!parts.empty()) {
                std::string command = parts[0];
                std::string key = parts.size() > 1 ? parts[1] : "";
                std::string value = parts.size() > 2 ? parts[2] : "";
                
                // Route command using existing Phase 6 Step 3 logic
                std::string cmd_upper = command;
                std::transform(cmd_upper.begin(), cmd_upper.end(), cmd_upper.begin(), ::toupper);
                ...
                if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                    ...
                    if (target_core == core_id_) {
                        ...
                        // For io_uring, send response directly instead of migration
                        std::string response = "-ERR MOVED " + std::to_string(target_core) + "\r\n";
                        submit_io_uring_write(client_fd, response);
                    }
                } else {
                    processor_->submit_operation(command, key, value, client_fd);
