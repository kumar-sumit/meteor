...
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands, size_t core_id, size_t total_shards) {
    ...
    for (size_t cmd_idx = 0; cmd_idx < commands.size(); ++cmd_idx) {
        ...
        if (cmd_parts.size() >= 2) {
            ...
            // **BREAKTHROUGH FIX**: Use same submit_operation approach as working baseline
            // This ensures identical cache access patterns and eliminates ResponsePool issues
            if (target_shard == current_shard) {
                // **WORKING APPROACH**: Use proven submit_operation instead of broken direct execution
                submit_operation(command, key, value, client_fd);
                // Note: Response will be sent by batch processing - we'll handle pipeline response collection differently
                response_trackers.emplace_back(cmd_idx, true, "+OK\r\n"); // Placeholder for now
                
            } else {
                // **CROSS-SHARD PATH**: For now, process locally to ensure correctness
                submit_operation(command, key, value, client_fd);
                response_trackers.emplace_back(cmd_idx, true, "+OK\r\n"); // Placeholder for now
            }
        }
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **FINAL BREAKTHROUGH**: Use working submit_operation for all single commands
            // This eliminates the dual code paths and hanging issues completely
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    std::string command = parsed_cmd[0];
                    ...
                    // **UNIFIED PROCESSING**: Always use the proven submit_operation approach
                    // This ensures cache consistency and eliminates hanging
                    processor_->submit_operation(command, key, value, client_fd);
                }
            }
        }
