...
public:
    ...
    void process_pending_batch() {
        ...
        // **DEBUG LOG 1: ENTRY**
        std::cout << "[DEBUG] process_pending_batch() ENTRY - batch_size=" << pending_operations_.size() 
        ...
        // **DEBUG LOG 2: RESPONSE COLLECTION START**
        std::cout << "[DEBUG] Starting response collection phase - operations=" << batch_size << std::endl;
        ...
        for (auto& op : pending_operations_) {
            // **DEBUG LOG 3: PER-OPERATION**
            if (op_index % 100 == 0 || op_index < 10) {
                std::cout << "[DEBUG] Processing op[" << op_index << "] client_fd=" << op.client_fd 
                          << ", command=" << std::string(op.command) << ", key=" << std::string(op.key) << std::endl;
            }
            try {
                // **DEBUG LOG 4: FD VALIDATION**
                if (op.client_fd < 0) {
                    std::cout << "[DEBUG] WARNING: Invalid client_fd=" << op.client_fd << " for op[" << op_index << "]" << std::endl;
                    op_index++;
                    continue;
                }
                ...
                // **DEBUG LOG 5: RESPONSE SIZE CHECK**
                if (response.empty()) {
                    std::cout << "[DEBUG] WARNING: Empty response for op[" << op_index << "] client_fd=" << op.client_fd << std::endl;
                } else if (response.size() > 10000) {
                    std::cout << "[DEBUG] WARNING: Large response size=" << response.size() << " for op[" << op_index << "]" << std::endl;
                }
                ...
                std::cout << "[DEBUG] ERROR: Exception in op[" << op_index << "]: " << e.what() << std::endl;
            }
            op_index++;
        }
        
        // **DEBUG LOG 6: VECTORED I/O START**
        std::cout << "[DEBUG] Starting vectored I/O phase - clients=" << client_responses.size() << std::endl;
        
        // **OPTIMIZATION 1.2B.1**: Send all responses using vectored I/O
        size_t client_index = 0;
        for (auto& [client_fd, responses] : client_responses) {
            // **DEBUG LOG 7: PER-CLIENT PROCESSING**
            std::cout << "[DEBUG] Client[" << client_index << "] fd=" << client_fd 
                      << ", responses=" << responses.size() << std::endl;
            try {
                // **DEBUG LOG 8: FD VALIDATION BEFORE SEND**
                if (client_fd < 0) {
                    std::cout << "[DEBUG] ERROR: Invalid client_fd=" << client_fd << " in send phase" << std::endl;
                    client_index++;
                    continue;
                }
                ...
                if (responses.size() == 1) {
                    ...
                    std::cout << "[DEBUG] Single response send - fd=" << client_fd << ", size=" << responses[0].size() << std::endl;
                    const std::string& resp = responses[0];
                    ssize_t sent = send(client_fd, resp.c_str(), resp.length(), MSG_NOSIGNAL);
                    if (sent == -1) {
                        std::cout << "[DEBUG] ERROR: send() failed - fd=" << client_fd << ", errno=" << errno << " (" << strerror(errno) << ")" << std::endl;
                    }
                } else if (responses.size() <= syscall_reduction::MAX_VECTOR_RESPONSES) {
                    // **DEBUG LOG 9: WRITEV PREPARATION**
                    std::cout << "[DEBUG] Preparing writev - fd=" << client_fd << ", responses=" << responses.size() << std::endl;
                    ...
                    for (size_t i = 0; i < responses.size(); ++i) {
                        ...
                        if (resp.empty()) {
                            std::cout << "[DEBUG] WARNING: Empty response in writev array[" << i << "]" << std::endl;
                        }
                        iov_array[i].iov_base = const_cast<char*>(resp.c_str());
                        iov_array[i].iov_len = resp.length();
                        total_bytes += resp.length();
                    }
                    
                    // **DEBUG LOG 10: WRITEV EXECUTION**
                    std::cout << "[DEBUG] Executing writev - fd=" << client_fd << ", iov_count=" << responses.size() 
                              << ", total_bytes=" << total_bytes << std::endl;
                    
                    ssize_t sent = writev(client_fd, iov_array, responses.size());
                    if (sent == -1) {
                        std::cout << "[DEBUG] ERROR: writev() failed - fd=" << client_fd << ", errno=" << errno 
                                  << " (" << strerror(errno) << ")" << std::endl;
                    } else if (sent != (ssize_t)total_bytes) {
                        std::cout << "[DEBUG] WARNING: Partial writev - sent=" << sent << ", expected=" << total_bytes << std::endl;
                    } else {
                        std::cout << "[DEBUG] writev() SUCCESS - fd=" << client_fd << ", bytes=" << sent << std::endl;
                    }
                } else {
                    // **DEBUG LOG 11: CHUNKED WRITEV**
                    std::cout << "[DEBUG] Chunked writev - fd=" << client_fd << ", total_responses=" << responses.size() << std::endl;
                    ...
                    while (offset < responses.size()) {
                        ...
                        std::cout << "[DEBUG] Chunk[" << chunk_count << "] - offset=" << offset << ", size=" << chunk_size << std::endl;
                        ...
                        if (sent == -1) {
                            std::cout << "[DEBUG] ERROR: Chunked writev() failed - chunk=" << chunk_count 
                ...
                std::cout << "[DEBUG] ERROR: Exception in send phase for fd=" << client_fd << ": " << e.what() << std::endl;
            }
            
            client_index++;
        }
        
        // **DEBUG LOG 12: EXIT SUCCESS**
        std::cout << "[DEBUG] process_pending_batch() SUCCESS - processed_ops=" << operations_processed_ 
                  << ", clients=" << client_responses.size() << std::endl;
