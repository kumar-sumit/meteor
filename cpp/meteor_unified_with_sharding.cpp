...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **UNIFIED WITH SHARDING**: Single code path with proper shard-based routing
        // This combines the best of both: unified processing + correct sharding
        ...
        if (!parsed_commands.empty()) {
            // **UNIFIED PROCESSING**: Use same shard routing logic for ALL commands
            // This ensures identical behavior for single commands and pipelines
