...
private:
    ...
    // **ASYNC FUTURE PROCESSING**: Store pending single command futures (like pipeline pattern)
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        if (dragonfly_cross_shard::global_cross_shard_coordinator) {
            std::cout << "✅ Core " << core_id_ << " coordinator initialized successfully!" << std::endl;
        } else {
            std::cout << "❌ Core " << core_id_ << " coordinator initialization failed!" << std::endl;
        }
...
private:
    ...
    void process_pending_single_futures() {
        // **NON-BLOCKING CONCURRENCY FIX**: Use try_lock to avoid blocking event loop
        std::unique_lock<std::mutex> lock(futures_mutex_, std::try_to_lock);
        if (!lock.owns_lock()) {
            return; // Skip this iteration if mutex is busy - will retry next cycle
        }
        ...
        while (it != pending_single_futures_.end()) {
            ...
            if (future.wait_for(std::chrono::seconds(0)) == boost::fibers::future_status::ready) {
                try {
                    ...
                    // **CLIENT VALIDATION**: Check if client is still connected
                    if (is_client_connected(client_fd)) {
                        // Send response to client
                        ssize_t bytes_sent = send(client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
                        if (bytes_sent <= 0) {
                            // Client disconnected during send - remove from tracking
                            close_client_connection(client_fd);
                        }
                    }
                    
                } catch (const std::exception& e) {
                    // **CLIENT VALIDATION**: Only send error if client still connected
                    if (is_client_connected(client_fd)) {
                ...
                // **TIMEOUT HANDLING**: Remove futures for disconnected clients
                if (!is_client_connected(client_fd)) {
                    it = pending_single_futures_.erase(it);
                } else {
    ...
    // **CLIENT CONNECTION VALIDATION**: Check if client FD is still valid
    bool is_client_connected(int client_fd) {
        std::lock_guard<std::mutex> lock(connections_mutex_);
        return std::find(client_connections_.begin(), client_connections_.end(), client_fd) != client_connections_.end();
    }
    ...
    void run() {
        ...
        while (running_.load()) {
            ...
            // **ASYNC SINGLE COMMAND FUTURES**: Process pending cross-shard responses
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **SINGLE COMMAND**: Apply working cross-shard pattern from minimal server
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    std::string command = parsed_cmd[0];
                    ...
                    // **PROVEN WORKING PATTERN**: Apply cross-shard coordination from minimal server
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        size_t target_shard = std::hash<std::string>{}(key) % total_shards_;
                        size_t current_shard = core_id_ % total_shards_;
                        
                        std::cout << "🔍 Core " << core_id_ << " processing " << cmd_upper << " " << key 
                                  << " (current_shard=" << current_shard << ", target_shard=" << target_shard << ")" << std::endl;
                        
                        if (target_shard != current_shard) {
                            std::cout << "🔄 Cross-shard routing needed from " << current_shard << " to " << target_shard << std::endl;
                            // **ASYNC CROSS-SHARD**: Store future like pipeline pattern
                            if (dragonfly_cross_shard::global_cross_shard_coordinator) {
                                std::cout << "✅ Coordinator available, sending cross-shard command..." << std::endl;
                                try {
                                    auto future = dragonfly_cross_shard::global_cross_shard_coordinator->send_cross_shard_command(
                                        target_shard, command, key, value, client_fd
                                    );
                                    ...
                                    // **EXACT PIPELINE PATTERN**: Store future, don't wait immediately
                                    {
                                        ...
                                        if (lock.owns_lock()) {
                                            ...
                                            // **FALLBACK**: If mutex busy, process locally to avoid blocking
                                            try {
                                                std::string response = future.get(); // Will block briefly
                                                ssize_t bytes_sent = send(client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
                                            } catch (...) {
                                    ...
                                    std::cout << "❌ Cross-shard send error: " << e.what() << ", falling back to local" << std::endl;
                                    // Fallback to local processing on error
                                }
                            } else {
                                std::cout << "⚠️  Coordinator not available, processing locally" << std::endl;
                            }
                        } else {
                            std::cout << "✅ Local processing (same shard)" << std::endl;
                        }
                    } else {
                        std::cout << "✅ Non-routable command (" << cmd_upper << "), processing locally" << std::endl;
                    }
                    
                    // **LOCAL FAST PATH**: Process locally (correct shard or fallback)
                    std::cout << "⚡ Calling processor->submit_operation for " << cmd_upper << " " << key << std::endl;
    ...
    void close_client_connection(int client_fd) {
        ...
        // **ASYNC FUTURE CLEANUP**: Remove any pending futures for this client
        {
            std::unique_lock<std::mutex> futures_lock(futures_mutex_, std::try_to_lock);
            if (futures_lock.owns_lock()) {
