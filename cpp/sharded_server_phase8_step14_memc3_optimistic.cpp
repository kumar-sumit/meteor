...
namespace storage {
    ...
    public:
        ...
        // **SIMPLIFIED CUCKOO INSERTION** to avoid hangs
        bool cuckoo_insert_with_path(size_t start_bucket, Tag tag, const std::string& key, const std::string& value) {
            // **SIMPLIFIED APPROACH: Try direct insertion first, then simple displacement**
            
            auto [b1, b2] = get_bucket_indices(key);
            
            // Try inserting in both possible buckets first
            for (size_t bucket_idx : {b1, b2}) {
                Bucket& bucket = *buckets_[bucket_idx];
                std::lock_guard<std::mutex> lock(bucket.write_lock);
            ...
            // **SIMPLE SINGLE-STEP DISPLACEMENT** (avoid complex chains)
            // Try to displace one item from each bucket to make room
            for (size_t bucket_idx : {b1, b2}) {
                Bucket& bucket = *buckets_[bucket_idx];
                std::lock_guard<std::mutex> lock(bucket.write_lock);
                ...
                // Try to displace the first item to its alternative bucket
                for (int slot_idx = 0; slot_idx < SLOTS_PER_BUCKET; ++slot_idx) {
                    auto& slot = bucket.slots[slot_idx];
                    if (slot.is_empty()) continue;
                    
                    auto* displaced_key = slot.key.load(std::memory_order_acquire);
                    if (!displaced_key) continue;
                    
                    auto [db1, db2] = get_bucket_indices(*displaced_key);
                    size_t alt_bucket_idx = (db1 == bucket_idx) ? db2 : db1;
                    
                    // Try to move displaced item to its alternative bucket
                    Bucket& alt_bucket = *buckets_[alt_bucket_idx];
                    if (alt_bucket_idx != bucket_idx) { // Avoid self-lock
                        std::lock_guard<std::mutex> alt_lock(alt_bucket.write_lock);
                        
                        alt_bucket.version.fetch_add(1, std::memory_order_acq_rel);
                        int alt_empty_slot = alt_bucket.find_empty_slot();
                        if (alt_empty_slot >= 0) {
                            // Move displaced item to alternative bucket
                            auto* displaced_value = slot.value.load(std::memory_order_acquire);
                            Tag displaced_tag = slot.tag.load(std::memory_order_acquire);
                            
                            auto& alt_slot = alt_bucket.slots[alt_empty_slot];
                            alt_slot.key.store(displaced_key, std::memory_order_release);
                            alt_slot.value.store(displaced_value, std::memory_order_release);
                            alt_slot.tag.store(displaced_tag, std::memory_order_release);
                            ...
                            // Insert new item in the freed slot
                            slot.key.store(new std::string(key), std::memory_order_release);
                            slot.value.store(new std::string(value), std::memory_order_release);
                            slot.tag.store(tag, std::memory_order_release);
