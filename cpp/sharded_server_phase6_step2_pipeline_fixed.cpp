// **PHASE 6 STEP 2 PIPELINE OPTIMIZED: Multi-Accept + CPU Affinity + AVX-512 + DragonflyDB-Style Pipeline Processing**
...
public:
    ...
    ~DirectOperationProcessor() {
        ...
        // **PIPELINE FIX: INTELLIGENT BATCHING FOR PIPELINE EFFICIENCY**
    void submit_operation(const std::string& command, const std::string& key,
        ...
        // **PIPELINE FIX: Intelligent batching strategy**
        ...
        // **PIPELINE FIX: Process smaller batches faster for pipelined requests**
        else if (batch_size >= 10 && (now - last_batch_time_) >= std::chrono::microseconds(10)) {
            should_process = true;
        }
        // **PIPELINE FIX: Process any batch after max delay**
        else if ((now - last_batch_time_) >= MAX_BATCH_DELAY) {
            should_process = true;
        }
        // **PIPELINE FIX: Process immediately if we have multiple clients in batch**
        else if (batch_size >= 5) {
            std::unordered_set<int> unique_clients;
            for (const auto& op : pending_operations_) {
                unique_clients.insert(op.client_fd);
                if (unique_clients.size() >= 2) {
                    should_process = true;
                    break;
                }
            }
