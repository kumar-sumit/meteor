...
namespace zero_copy {
    ...
    // **SEPARATE TTL COMMAND DETECTION** - independent from baseline commands
...
namespace storage {
    ...
    private:
        ...
        // **SEPARATE TTL SYSTEM** - completely independent from baseline data
    ...
    public:
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            if (it != data_.end()) {
                // **SEPARATE TTL CHECK** - only if key exists in TTL system
                auto ttl_it = ttl_expiry_.find(key);
                if (ttl_it != ttl_expiry_.end() && std::chrono::steady_clock::now() >= ttl_it->second) {
                    // Key expired - return null (passive expiry, don't remove)
                    return std::nullopt;
                }
                // **BASELINE UNCHANGED** - return value as before
        ...
        bool del(const std::string& key) {
            // **TRUE SHARED-NOTHING**: Direct access to core's own data (no locking needed!)
            // **SEPARATE TTL CLEANUP** - clean TTL entry if exists
            ttl_expiry_.erase(key);
            return data_.erase(key) > 0;
        }
        
        // **SEPARATE TTL METHODS** - completely independent system
        bool set_with_ttl(const std::string& key, const std::string& value, std::chrono::seconds ttl_seconds) {
            // **BASELINE SET** - unchanged
            data_[key] = Entry(key, value);
            // **SEPARATE TTL TRACKING** - add expiry to separate map
            ttl_expiry_[key] = std::chrono::steady_clock::now() + ttl_seconds;
            return true;
        }
        
        int ttl(const std::string& key) {
            // Check baseline data first
            if (data_.find(key) == data_.end()) {
                return -2; // Key doesn't exist
            }
            
            // Check separate TTL system
            auto ttl_it = ttl_expiry_.find(key);
            ...
            auto remaining = ttl_it->second - now;
...
namespace cache {
    ...
    public:
        ...
        // **SEPARATE TTL SYSTEM METHODS** - pass-through to VLLHashTable
        bool set_with_ttl(const std::string& key, const std::string& value, std::chrono::seconds ttl_seconds) {
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **SEPARATE TTL COMMAND PROCESSING** - independent from baseline
