// **PHASE 8 STEP 2: VLL + SCA IMPLEMENTATION FOR SUB-1MS P99 LATENCY**
//
// Building on Step 1's success (121K RPS, 0.35ms P50, 0.69ms P95), now eliminating P99 contention:
// **MISSION**: Replace std::shared_mutex with VLL + SCA to achieve sub-1ms P99 latency
//
// PHASE 2 - VLL + SCA Implementation:
// 1. **VLL (Very Lightweight Locking)**: Atomic read/write counters co-located with each slot
// 2. **SCA (Selective Contention Analysis)**: Detect and handle high-contention scenarios  
// 3. **Intent Locks**: Prevent deadlocks and reduce lock acquisition overhead
// 4. **Contention-Aware Backoff**: Exponential backoff for high-contention slots
// 5. **Lock-Free Fast Path**: Most operations bypass locking entirely
// 6. **NUMA-Aware Memory Layout**: Optimize for multi-core cache coherence
//
// Target Performance: 120K+ RPS with P99 < 1ms (eliminate 139ms spikes)
// Architecture: Replace VLLHashTable::shards_[i].mutex with per-slot VLL counters
// Research: Based on VLL paper + DragonflyDB's actual implementation
//
// Previous Results: 121,505 RPS, P50: 0.35ms, P95: 0.69ms, P99: 139ms ❌
// Target Results:   120K+ RPS,     P50: <0.5ms,  P95: <1ms,   P99: <1ms ✅
...
namespace storage {
    ...
    private:
        ...
        struct alignas(64) Slot {  // Cache-line aligned for performance
            ...
            // **SCA: Check if slot is experiencing high contention**
            bool is_high_contention() const {
                return contention_count.load(std::memory_order_relaxed) > 10;  // Threshold for high contention
            }
        ...
        struct alignas(64) Bucket {  // Cache-line aligned
            ...
            bool insert(const K& key, const V& value, uint32_t primary_hash, uint32_t secondary_hash, bool is_stash = false) {
                // Check for existing key first (update case) - use VLL read locks
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].occupied.load(std::memory_order_acquire) && slots[i].key == key) {
                        // VLL: Acquire write lock for update
                        int attempts = 0;
                        while (!slots[i].try_write_lock() && attempts < 100) {
                            attempts++;
                            if (slots[i].is_high_contention()) {
                                // SCA: Exponential backoff for high contention
                                std::this_thread::sleep_for(std::chrono::microseconds(1 << std::min(attempts / 10, 6)));
                            }
                        }
                        
                        if (attempts >= 100) return false;  // Failed to acquire lock
                ...
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (!slots[i].occupied.load(std::memory_order_acquire)) {
                        // VLL: Acquire write lock for insertion
                        int attempts = 0;
                        while (!slots[i].try_write_lock() && attempts < 100) {
                            attempts++;
                            if (slots[i].is_high_contention()) {
                        ...
                        if (attempts >= 100) continue;  // Try next slot
            ...
            // **VLL-Enabled 2Q Cache Policy: Find with automatic promotion**
            bool find(const K& key, V& result) {
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].occupied.load(std::memory_order_acquire) && slots[i].key == key) {
                        // VLL: Acquire read lock for safe access
                        int attempts = 0;
                        while (!slots[i].try_read_lock() && attempts < 50) {
                            attempts++;
                            if (slots[i].is_high_contention()) {
                                // SCA: Brief backoff for high contention
                                std::this_thread::sleep_for(std::chrono::microseconds(1));
                            }
                        }
                        
                        if (attempts >= 50) continue;  // Try next slot
        ...
        struct alignas(64) Segment {
            ...
            // **VLL-Enabled DragonflyDB Insert with Stash Overflow**
            bool insert(const K& key, const V& value) {
