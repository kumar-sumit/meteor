...
namespace ultra_optimized {
    ...
    struct ResponsePool {
        ...
        static constexpr const char* MINUS_ONE_RESPONSE = ":-1\r\n";
        static constexpr const char* MINUS_TWO_RESPONSE = ":-2\r\n";
        ...
        // **TTL RESPONSE SIZES**
        static constexpr size_t MINUS_ONE_SIZE = 5;    // ":-1\r\n"
        static constexpr size_t MINUS_TWO_SIZE = 5;    // ":-2\r\n"
...
namespace zero_copy {
    ...
    // **TTL COMMAND DETECTION** - Redis-compatible commands
...
namespace storage {
    ...
    public:
        struct Entry {
            ...
            // **REDIS-STYLE TTL + LRU FIELDS** - Only used when TTL is explicitly set
            std::chrono::steady_clock::time_point expiry_time = std::chrono::steady_clock::time_point::max();
            std::chrono::steady_clock::time_point last_access = std::chrono::steady_clock::time_point::max();
            bool has_ttl = false;
            
            // **BASELINE CONSTRUCTOR** - unchanged for existing flows
            Entry() = default;
            Entry(const std::string& k, const std::string& v) : key(k), value(v) {}
            
            // **TTL CONSTRUCTOR** - for SETEX and TTL-enabled operations
            ...
            // **REDIS-STYLE EXPIRY CHECK** - lazy expiration
    ...
    private:
        ...
        // **LRU + TTL MANAGEMENT** - for memory management
        size_t max_entries_ = 1000000; // Default 1M entries per core
        mutable std::chrono::steady_clock::time_point last_cleanup_ = std::chrono::steady_clock::now();
    ...
    public:
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            if (it != data_.end()) {
                // **CONDITIONAL TTL CHECK** - only if TTL is set (zero impact on non-TTL keys)  
                if (it->second.has_ttl && it->second.is_expired()) {
                    // **LAZY EXPIRATION** - return null but don't remove here (Redis-style)
                    return std::nullopt;
                }
                // **LRU UPDATE** - track access time for TTL keys only
                if (it->second.has_ttl) {
                    it->second.last_access = std::chrono::steady_clock::now();
                }
                // **BASELINE BEHAVIOR** - unchanged for non-TTL keys
        ...
        // **TTL + LRU METHODS** - Redis-compatible functionality
        bool set_with_ttl(const std::string& key, const std::string& value, std::chrono::seconds ttl_seconds) {
            // **LRU EVICTION** - manage memory by evicting TTL keys if needed
            lru_evict_ttl_keys_if_needed();
            
            // **TTL SET** - using TTL constructor
        ...
        int ttl(const std::string& key) {
            auto it = data_.find(key);
            if (it == data_.end()) {
                return -2; // Key doesn't exist (Redis standard)
            }
            
            if (!it->second.has_ttl) {
                return -1; // Key exists but no TTL set (Redis standard)
            }
            
            if (it->second.is_expired()) {
                return -2; // Expired key treated as non-existent
            }
        ...
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
            ...
            // **SET TTL ON EXISTING KEY** - Redis EXPIRE command
    ...
    private:
        // **LRU EVICTION FOR TTL KEYS** - Redis-inspired space management
        void lru_evict_ttl_keys_if_needed() {
            if (data_.size() <= max_entries_) {
                return; // No eviction needed
            }
            
            // **COLLECT TTL KEYS** - only evict keys with TTL set
            std::vector<std::pair<std::string, Entry*>> ttl_keys;
            for (auto& pair : data_) {
                if (pair.second.has_ttl) {
                    ttl_keys.emplace_back(pair.first, &pair.second);
                }
            }
            
            if (ttl_keys.empty()) {
                return; // No TTL keys to evict
            }
            
            // **SORT BY EVICTION PRIORITY** - prefer keys expiring latest first (as requested)
            // Then by least recently accessed
            std::sort(ttl_keys.begin(), ttl_keys.end(), 
                [](const auto& a, const auto& b) {
                    // First priority: Expire latest first (reverse expiry order)
                    if (a.second->expiry_time != b.second->expiry_time) {
                        return a.second->expiry_time > b.second->expiry_time;
                    }
                    // Second priority: Least recently accessed first
                    return a.second->last_access < b.second->last_access;
                });
            
            // **EVICT EXCESS TTL KEYS** - free up space
            size_t keys_to_evict = data_.size() - max_entries_ + 1;
            keys_to_evict = std::min(keys_to_evict, ttl_keys.size());
            
            for (size_t i = 0; i < keys_to_evict; ++i) {
                data_.erase(ttl_keys[i].first);
            }
        }
    };
}
...
namespace cache {
    ...
    public:
        ...
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
            return memory_cache_->expire(key, ttl_seconds);
        }
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **TTL COMMANDS** - Redis-compatible TTL functionality  
        if (zero_copy::is_ttl_command(op.command)) {
            int remaining = cache_->ttl(std::string(op.key));
            if (remaining == -2) {
                return ResponseInfo(ultra_optimized::ResponsePool::MINUS_TWO_RESPONSE,
                                  ultra_optimized::ResponsePool::MINUS_TWO_SIZE);
            } else if (remaining == -1) {
                return ResponseInfo(ultra_optimized::ResponsePool::MINUS_ONE_RESPONSE,
                                  ultra_optimized::ResponsePool::MINUS_ONE_SIZE);
            } else if (remaining == 0) {
                return ResponseInfo(ultra_optimized::ResponsePool::ZERO_RESPONSE,
                                  ultra_optimized::ResponsePool::ZERO_SIZE);
            } else {
                // For positive TTL values, use static thread_local buffer
                static thread_local std::string ttl_response_buffer;
                ttl_response_buffer = ":" + std::to_string(remaining) + "\r\n";
                return ResponseInfo(ttl_response_buffer.c_str(), ttl_response_buffer.length());
            }
        }
        ...
        if (zero_copy::is_expire_command(op.command)) {
            ...
            try {
                int seconds = std::stoi(std::string(op.value));
                bool success = cache_->expire(std::string(op.key), std::chrono::seconds(seconds));
                return success ? 
                    ResponseInfo(ultra_optimized::ResponsePool::ONE_RESPONSE,
                               ultra_optimized::ResponsePool::ONE_SIZE) :
                    ResponseInfo(ultra_optimized::ResponsePool::ZERO_RESPONSE,
        ...
        if (zero_copy::is_setex_command(op.command)) {
            ...
            try {
                int seconds = std::stoi(std::string(op.key)); // In SETEX, key field contains seconds
                bool success = cache_->set_with_ttl(std::string(op.value), std::string(op.command), 
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL" || 
                         cmd_upper == "TTL" || cmd_upper == "EXPIRE" || cmd_upper == "SETEX") && !key.empty()) {
