...
namespace cpu_affinity {
    ...
    inline int create_reuseport_socket(const std::string& host, int port, bool& reuseport_success) {
        ...
        // **CONNECTION BURST FIX**: Optimize TCP settings for high connection loads
        ...
        // **CONNECTION BURST FIX**: Increase socket buffers for connection burst handling
        int buffer_size = 2 * 1024 * 1024; // 2MB buffers for high-throughput connections
        if (setsockopt(server_fd, SOL_SOCKET, SO_RCVBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            std::cerr << "⚠️  SO_RCVBUF optimization failed: " << strerror(errno) << " (continuing anyway)" << std::endl;
        }
        if (setsockopt(server_fd, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            std::cerr << "⚠️  SO_SNDBUF optimization failed: " << strerror(errno) << " (continuing anyway)" << std::endl;
        }
        
        // **CONNECTION BURST FIX**: Enable keep-alive for connection stability
        if (setsockopt(server_fd, SOL_SOCKET, SO_KEEPALIVE, &opt, sizeof(opt)) < 0) {
            std::cerr << "⚠️  SO_KEEPALIVE failed: " << strerror(errno) << " (continuing anyway)" << std::endl;
        }
        ...
        // **CONNECTION BURST FIX**: Increase listen backlog for connection burst handling
        int backlog = std::max(SOMAXCONN, 8192); // Much higher backlog for connection bursts
...
implee    // **CONNECTION BURST FIX**: Optimized accept loop for high connection bursts
    void run_dedicated_accept() {
        std::cout << "🔌 Core " << core_id_ << " dedicated accept thread started (burst-optimized)" << std::endl;
        
        // Connection burst optimization counters
        int consecutive_accepts = 0;
        int total_accepted = 0;
        auto last_batch_log = std::chrono::steady_clock::now();
        
        while (running_.load()) {
            struct sockaddr_in client_addr;
            socklen_t client_len = sizeof(client_addr);
            
            // **BURST OPTIMIZATION**: Try to accept multiple connections in one iteration
            consecutive_accepts = 0;
            while (consecutive_accepts < 128 && running_.load()) { // Max 128 accepts per burst
                ...
                // **BURST OPTIMIZATION**: Efficient per-connection setup
                // Set non-blocking (essential for io_uring)
                int flags = fcntl(client_fd, F_GETFL, 0);
                fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
                
                // **BURST OPTIMIZATION**: Optimize client TCP settings
                int opt = 1;
            ...
            // **BURST OPTIMIZATION**: Adaptive logging (reduce spam under high load)
            auto now = std::chrono::steady_clock::now();
            if (consecutive_accepts > 0 && 
                std::chrono::duration_cast<std::chrono::seconds>(now - last_batch_log).count() >= 1) {
                std::cout << "🔌 Core " << core_id_ << " batch accepted " << consecutive_accepts 
                         << " connections (total: " << total_accepted << ", rate: " 
                         << total_accepted << "/sec)" << std::endl;
                last_batch_log = now;
                total_accepted = 0; // Reset counter for next second
            }
            
            // **BURST OPTIMIZATION**: Adaptive sleep based on accept activity
            if (consecutive_accepts == 0) {
                // No connections ready - brief sleep to prevent busy waiting
                std::this_thread::sleep_for(std::chrono::microseconds(50)); // 50μs when no connections
            } else if (consecutive_accepts < 10) {
                // Light activity - minimal yield
                std::this_thread::yield();
            }
            // Heavy activity (consecutive_accepts >= 10) - no sleep, keep accepting
        }
