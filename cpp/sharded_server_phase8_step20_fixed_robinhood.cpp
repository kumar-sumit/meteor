// **PHASE 8 STEP 20: FIXED ROBIN HOOD + EPOLL**
...
// - FIXED: Robin Hood insertion logic with proper displacement and PSL handling
// - FIXED: Key existence check during insertion to handle updates properly
            ...
            bool insert(const Key& key, const Value& value) {
                ...
                for (size_t i = 0; i < cap; ++i) {
                    ...
                    if (existing_psl == Entry::EMPTY_PSL || existing_psl == Entry::DELETED_PSL) {
                        ...
                        new (&current.key) Key(std::move(insert_key));
                        new (&current.value) Value(std::move(insert_value));
