// **PHASE 8 STEP 16: DYNAMIC HASH RANGE PARTITIONING**
...
// **DYNAMIC HASH RANGE PARTITIONING** (Simple + Unlimited)
namespace storage {
    ...
    class DynamicHashRangeStorage {
    public:
        static constexpr size_t MAX_MAP_SIZE = 50000;        // When to split a map
        static constexpr size_t INITIAL_MAPS = 1;            // Start with 1 map per shard
    ...
    private:
        std::vector<std::unique_ptr<HashRangeMap>> maps_;
        mutable std::mutex maps_mutex_;  // Protects maps_ vector modifications
        std::hash<Key> hasher_;
        
    public:
        DynamicHashRangeStorage() {
            // Start with one map covering the entire hash space
            maps_.emplace_back(std::make_unique<HashRangeMap>(0, SIZE_MAX));
        }
        
        // **FIND** - Lock-free reads within each map
        bool find(const Key& key, Value& out_val) {
            size_t hash = hasher_(key);
            
            // Find the correct map (read-only access to maps_)
            std::unique_lock<std::mutex> maps_lock(maps_mutex_);
        ...
        // **INSERT** - May trigger map splitting
        bool insert(const Key& key, const Value& value) {
            ...
            while (true) {
                ...
                if (target_map->load_factor() > SPLIT_THRESHOLD) {
                    split_map(target_map);
                    maps_lock.unlock();
                ...
                target_map->data[key] = value;  // Insert or update
                return true;
            }
        }
        ...
        bool del(const Key& key) {
            ...
            std::unique_lock<std::shared_mutex> lock(target_map->mutex);
            return target_map->data.erase(key) > 0;
        }
        ...
        double load_factor() const {
            ...
            for (const auto& map : maps_) {
                std::shared_lock<std::shared_mutex> lock(map->mutex);
                total_size += map->data.size();
                total_capacity += MAX_MAP_SIZE;
            }
            return total_capacity > 0 ? static_cast<double>(total_size) / total_capacity : 0.0;
        }
        
    private:
        // **SPLIT MAP** when it gets too full
        void split_map(HashRangeMap* full_map) {
            ...
            while (it != full_map->data.end()) {
                size_t key_hash = hasher_(it->first);
