// **TEMPORAL COHERENCE STEP 1.2: PERFORMANCE FIXED**
// Fixed cross-core pipeline performance bottleneck by using batch processing
...
// - CROSS-CORE PIPELINES: Now use BATCH processing too (instead of individual)
// - CORRECTNESS: Temporal coherence architecture preserved with performance fix
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            if (all_local) {
                ...
                // **CROSS-CORE PIPELINE**: Apply temporal coherence protocol with BATCH processing
                temporal_metrics_.temporal_pipelines_detected.fetch_add(1);
                
                // **PERFORMANCE FIX**: Use batch processing for cross-core pipelines too
                // This maintains the fast path while adding temporal coherence tracking
