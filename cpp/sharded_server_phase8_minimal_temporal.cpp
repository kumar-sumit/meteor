// **MINIMAL TEMPORAL COHERENCE: SURGICAL CORRECTNESS FIX**
// CRITICAL: Fix cross-core pipeline correctness bug while preserving 4.92M QPS baseline
// - BASELINE: Preserve 100% of single command processing (ZERO changes)
// - CORRECTNESS: Fix only the cross-core pipeline processing bug
// - PERFORMANCE: Minimal overhead - no temporal processing unless actually needed
// - SURGICAL: Only 3 lines changed from baseline for maximum performance preservation
// Target: 4.92M+ QPS maintained + 100% cross-core pipeline correctness
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **MINIMAL TEMPORAL COHERENCE**: Fix cross-core pipeline correctness
            if (all_local) {
                // **LOCAL PIPELINE**: Use existing high-performance batch processing
                processor_->process_pipeline_batch(client_fd, parsed_commands);
            } else {
                // **CROSS-CORE PIPELINE CORRECTNESS FIX**: Route commands to correct cores
                // Minimal overhead solution - execute each command on correct core individually
