...
namespace zero_copy {
    ...
    // **SURGICAL TTL STEP 1**: TTL Response Formatter (same pattern as GET)
    inline size_t format_ttl_response_direct(char* buffer, int ttl_value) {
        char* ptr = buffer;
        
        // Write ":"
        *ptr++ = ':';
        
        // Write TTL value using fast conversion
        if (ttl_value == -2) {
            *ptr++ = '-';
            *ptr++ = '2';
        } else if (ttl_value == -1) {
            ...
            *ptr++ = '1';
            ...
            // For positive values (future use)
            ptr += fast_itoa(ttl_value, ptr);
        }
    ...
    // **SURGICAL TTL STEP 1**: Minimal TTL command detection
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **SURGICAL TTL STEP 1**: TTL command using same pattern as GET
        if (zero_copy::is_ttl_command(op.command)) {
            // For Step 1: Always return -2 (key not found) using proper buffer pattern
            char* buffer = ultra_optimized::ResponsePool::get_response_buffer;
            size_t len = zero_copy::format_ttl_response_direct(buffer, -2);
            
            ultra_optimized::ResponsePool::get_response_size = len;
            return ResponseInfo(buffer, len, false); // Same pattern as GET
        }
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL" || cmd_upper == "TTL") && !key.empty()) {
