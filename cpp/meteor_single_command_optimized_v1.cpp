// **METEOR v7.0: SYSCALL REDUCTION + PIPELINE + SINGLE COMMAND CORRECTNESS**
//
...
namespace monitoring {
    ...
    struct CoreMetrics {
        ...
        // **PHASE 1 OPTIMIZATION METRICS**: Zero-copy ring buffer performance
        std::atomic<uint64_t> fast_commands_sent{0};
        std::atomic<uint64_t> fast_commands_processed{0};
        std::atomic<uint64_t> ring_buffer_full{0};
        std::atomic<uint64_t> responses_sent_back{0};
...
// **PHASE 1 OPTIMIZATION: Zero-Copy Shared Memory Command Structure**
struct alignas(64) FastCommand {
    ...
    uint64_t command_id;               // Unique command ID for response routing
    ...
    FastCommand() : state(0), client_fd(0), command_len(0), key_len(0), 
                   value_len(0), source_core(0), command_id(0), timestamp_tsc(0) {
    ...
    // **ULTRA-FAST SETUP**: Direct memory copy, no string objects
    void setup(int fd, int src_core, uint64_t cmd_id, std::string_view cmd, std::string_view k, std::string_view v) {
        client_fd = fd;
        source_core = src_core;
        command_id = cmd_id;
        command_len = std::min(cmd.size(), (size_t)MAX_COMMAND_SIZE - 1);
        key_len = std::min(k.size(), (size_t)MAX_KEY_SIZE - 1);
        value_len = std::min(v.size(), (size_t)MAX_VALUE_SIZE - 1);
        
        memcpy(command, cmd.data(), command_len);
        ...
        // **HARDWARE TIMESTAMP**: Ultra-low overhead timing
        timestamp_tsc = __builtin_ia32_rdtsc();
        
        // **ATOMIC RELEASE**: Make command visible to target core
        state.store(1, std::memory_order_release);
    }
    
    std::string_view get_command() const { return std::string_view(command, command_len); }
    std::string_view get_key() const { return std::string_view(key, key_len); }
    std::string_view get_value() const { return std::string_view(value, value_len); }
};

// **SHARED MEMORY RING BUFFER**: Zero-copy cross-core communication
struct alignas(64) CrossCoreCommandRing {
    ...
    bool try_enqueue(int client_fd, int source_core, uint64_t command_id, std::string_view cmd, 
                     std::string_view key, std::string_view value) {
        uint64_t current_tail = tail.load(std::memory_order_relaxed);
        uint64_t next_tail = current_tail + 1;
        
        // **FAST CHECK**: Ring buffer full?
        if (next_tail - head.load(std::memory_order_acquire) >= RING_SIZE) {
            return false; // Full
        }
        
        FastCommand& slot = commands[current_tail & (RING_SIZE - 1)];
        
        // **ZERO-COPY SETUP**: Direct inline data placement
        slot.setup(client_fd, source_core, command_id, cmd, key, value);
        
        // **COMMIT**: Advance tail pointer
        tail.store(next_tail, std::memory_order_release);
        return true;
    }
    
    // **ULTRA-FAST DEQUEUE**: Process ready commands (race-condition free)
    bool try_dequeue(FastCommand*& cmd) {
        ...
        FastCommand& slot = commands[current_head & (RING_SIZE - 1)];
        
        // **ATOMIC CHECK**: Command ready?
        uint32_t expected_state = 1;
        if (!slot.state.compare_exchange_weak(expected_state, 2, std::memory_order_acq_rel)) {
            return false; // Not ready yet or already being processed
        }
        
        // **SAFE TO PROCESS**: State changed from 1 (ready) to 2 (processing)
        cmd = &slot;
...
// **RESPONSE ROUTING**: Route responses back to source core that owns client socket
struct alignas(64) ResponseRoutingMessage {
    int client_fd;
    int source_core;
    uint64_t command_id;
    uint16_t response_len;
    char response[FastCommand::MAX_VALUE_SIZE];
    std::chrono::steady_clock::time_point timestamp;
    
    ResponseRoutingMessage() : client_fd(0), source_core(0), command_id(0), response_len(0), timestamp(std::chrono::steady_clock::now()) {
        memset(response, 0, sizeof(response));
    }
    
    ResponseRoutingMessage(int fd, int src_core, uint64_t cmd_id, const std::string& resp) 
        : client_fd(fd), source_core(src_core), command_id(cmd_id), timestamp(std::chrono::steady_clock::now()) {
        response_len = std::min(resp.size(), sizeof(response) - 1);
        memcpy(response, resp.c_str(), response_len);
        response[response_len] = '\0';
    }
    
    std::string_view get_response() const { return std::string_view(response, response_len); }
};

// **LEGACY COMPATIBILITY**: Keep original structure for existing pipeline code
...
private:
    ...
    // **PHASE 1 OPTIMIZATION**: Zero-copy ring buffer for ultra-fast cross-core communication
    std::unique_ptr<CrossCoreCommandRing> fast_command_ring_;
    
    // **RESPONSE ROUTING**: Queue for responses from other cores back to this core
    lockfree::ContentionAwareQueue<ResponseRoutingMessage> response_queue_;
    
    // **LEGACY COMPATIBILITY**: Keep original queue for pipeline migration  
    ...
    // **COMMAND ID GENERATION**: Unique IDs for response routing
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        // **PHASE 1 OPTIMIZATION**: Initialize zero-copy ring buffer for cross-core commands
        fast_command_ring_ = std::make_unique<CrossCoreCommandRing>();
    ...
    void route_single_command_to_target_core(size_t target_core, int client_fd, 
        ...
        if (target_core < all_cores_.size() && all_cores_[target_core]) {
            ...
            bool success = all_cores_[target_core]->fast_command_ring_->try_enqueue(
                client_fd, core_id_, 0, command, key, value);
...
private:
    void process_connection_migrations() {
        ...
        while (fast_command_ring_->try_dequeue(fast_cmd)) {
            ...
            // **DIRECT RESPONSE**: Send response directly to client (same as baseline approach)
            // This works because socket operations are thread-safe in this implementation
            ssize_t bytes_sent = send(fast_cmd->client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
            if (bytes_sent < 0 && errno != EPIPE && errno != ECONNRESET) {
                // Log unexpected send errors if needed
            }
        ...
        // **LEGACY COMPATIBILITY**: Process traditional migration queue
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        // **CRITICAL FIX**: Use IDENTICAL routing logic as pipeline commands
                        size_t shard_id = std::hash<std::string>{}(key) % total_shards_;
