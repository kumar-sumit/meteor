// **PHASE 7 STEP 1: IO_URING HIGH-PERFORMANCE I/O INTEGRATION**
// Building on Phase 6 Step 3 (DragonflyDB-style pipeline batch processing) with:
// - io_uring async I/O replacing epoll for 3x performance improvement
// - Batched accept/recv/send operations for reduced syscall overhead
// - Zero-copy buffer management for memory efficiency
// - Advanced completion queue processing
// Target: 2.4M+ RPS (3x improvement over Phase 6 Step 3)
// Preserving all previous optimizations: multi-accept, CPU affinity, batch processing
...
// **PHASE 7 STEP 1: Conditional io_uring includes**
#ifndef DISABLE_IO_URING
  #ifdef __has_include
    ...
    #endif
  #else
    // Fallback: try to include and let compiler decide
    #include <liburing.h>
    #include <sys/mman.h>
    #include <sys/eventfd.h>
    #define HAS_IO_URING 1
...
// **PHASE 7 STEP 1: io_uring operation types and structures**
#if HAS_IO_URING
...
struct IoUringOperation {
    ...
    IoUringOperation(IoUringOpType op_type, int fd = -1) 
        : type(op_type), client_fd(fd), buffer(nullptr), buffer_size(0), 
          bytes_processed(0), start_time(std::chrono::steady_clock::now()) {}
};
...
#if HAS_IO_URING
    // **PHASE 7 STEP 1: io_uring specific fields**
    char* recv_buffer;  // Pre-allocated receive buffer from pool
    char* send_buffer;  // Pre-allocated send buffer from pool
    size_t pending_recv_bytes;
...
#if HAS_IO_URING
          , recv_buffer(nullptr), send_buffer(nullptr), 
          pending_recv_bytes(0), pending_send_bytes(0),
          has_pending_recv(false), has_pending_send(false)
#endif
    {}
};
...
private:
    ...
    std::unordered_map<int, std::unique_ptr<IoUringOperation>> pending_operations_;
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        std::cout << "🚀 Core " << core_id_ << " using io_uring with queue depth " << QUEUE_DEPTH << std::endl;
        ...
        std::cout << "🍎 Core " << core_id_ << " using kqueue (macOS)" << std::endl;
#endif
    ...
    // **PHASE 7 STEP 1: Dedicated accept loop - io_uring vs traditional**
    void run_dedicated_accept() {
#if HAS_IO_URING
        std::cout << "🚀 Core " << core_id_ << " io_uring accept integration started" << std::endl;
        
        // With io_uring, accepts are handled in the main event loop
        // Submit initial accept operation
        submit_accept_operation();
        
        // The dedicated accept thread is not needed for io_uring
        // All I/O operations are handled asynchronously in the main event loop
...
private:
    ...
    void process_events_iouring() {
        ...
        if (ret < 0) {
            if (ret != -ETIME && ret != -EINTR) {
                std::cerr << "io_uring_wait_cqe failed on core " << core_id_ << ": " << strerror(-ret) << std::endl;
            }
            return;
        }
    ...
    void submit_pending_operations() {
        // This will be implemented when we add batching
        io_uring_submit(&ring_);
    }
    
    void process_completion(struct io_uring_cqe *cqe) {
        auto op = reinterpret_cast<IoUringOperation*>(io_uring_cqe_get_data(cqe));
        if (!op) return;
        ...
        switch (op->type) {
            ...
            case IoUringOpType::RECV: {
                if (result > 0) {
                    ...
                    // Return buffer to pool and submit new recv
                    if (op->buffer) {
            ...
            case IoUringOpType::SEND: {
                if (result > 0) {
                    op->bytes_processed += result;
                    
                    // If not all data sent, submit another send
                    if (op->bytes_processed < op->buffer_size) {
                        char* remaining_data = static_cast<char*>(op->buffer) + op->bytes_processed;
                        size_t remaining_size = op->buffer_size - op->bytes_processed;
                        submit_send_operation(op->client_fd, remaining_data, remaining_size);
                    }
                }
                break;
            }
        ...
        // Cleanup operation
        {
            ...
            pending_operations_.erase(op->client_fd);
        }
        delete op;
    }
    ...
    void submit_accept_operation() {
        ...
        auto op = new IoUringOperation(IoUringOpType::ACCEPT);
    ...
    void submit_recv_operation(int client_fd) {
        ...
        auto op = new IoUringOperation(IoUringOpType::RECV, client_fd);
        op->buffer = buffer;
        op->buffer_size = IoUringBufferPool::buffer_size();
        
        {
            std::lock_guard<std::mutex> lock(uring_mutex_);
            pending_operations_[client_fd] = std::unique_ptr<IoUringOperation>(op);
        }
    ...
    void submit_send_operation(int client_fd, const void* data, size_t size) {
        ...
        auto op = new IoUringOperation(IoUringOpType::SEND, client_fd);
        op->buffer = const_cast<void*>(data);
        op->buffer_size = size;
        
        io_uring_prep_send(sqe, client_fd, data, size, 0);
        io_uring_sqe_set_data(sqe, op);
    }
