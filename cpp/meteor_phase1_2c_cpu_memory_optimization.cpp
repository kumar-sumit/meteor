// **METEOR PHASE 1.2C: ADVANCED CPU & MEMORY OPTIMIZATION**
// 
// TARGET PERFORMANCE (Phase 1.2C Advanced CPU & Memory Optimizations):
// - PHASE 1.2B BASELINE: 5.27M ops/sec (Syscall reduction + work-stealing foundation)
// - PHASE 1.2C TARGET: 6.0M+ ops/sec (CPU & memory optimization, +730K QPS gain)
// - 12C:4S: 6.0M+ ops/sec (optimal configuration with SIMD + NUMA + cache optimization)
// - 16C:8S: 7.0M+ ops/sec (linear scaling with full hardware utilization)
// 
...
//
// **PHASE 1.2C ADVANCED CPU & MEMORY OPTIMIZATIONS**:
// ✅ SYSCALL REDUCTION FOUNDATION: All Phase 1.2B optimizations (Phase 1.2B preserved)
// ✅ SIMD BATCH COMMAND PROCESSING: 8x parallel command classification (NEW)
// ✅ NUMA-AWARE MEMORY ALLOCATION: Optimized memory access patterns (NEW)
// ✅ ADVANCED IO_URING SQPOLL: Zero-syscall async I/O optimization (NEW)
// ✅ CPU CACHE OPTIMIZATION: Minimize cache misses in hot paths (NEW)
// ✅ PREFETCH OPTIMIZATION: Strategic data prefetching for reduced latency (NEW)
// ✅ MEMORY POOL ALIGNMENT: Cache-line aligned memory pools (NEW)
// Target: 6.0M+ QPS (from 5.27M Phase 1.2B, +730K QPS improvement)
...
// **SIMD CPU FEATURE DETECTION** (moved here for early use)
namespace simd {
...
// **PHASE 1.2C: ADVANCED CPU & MEMORY OPTIMIZATIONS**
namespace cpu_memory_optimization {
    
    // **OPTIMIZATION 1.2C.1: SIMD Batch Command Processing**
    static constexpr size_t SIMD_BATCH_SIZE = 8;  // Process 8 commands simultaneously with AVX-256
    static constexpr size_t SIMD_ALIGNMENT = 32;  // AVX-256 requires 32-byte alignment
    
    struct alignas(SIMD_ALIGNMENT) SIMDCommandBatch {
        struct CommandInfo {
            const char* command_ptr;
            size_t command_len;
            uint32_t command_hash;
            uint8_t command_type;  // 0=unknown, 1=GET, 2=SET, 3=DEL, 4=PING
            
            CommandInfo() : command_ptr(nullptr), command_len(0), command_hash(0), command_type(0) {}
        };
        
        CommandInfo commands[SIMD_BATCH_SIZE];
        size_t count;
        
        SIMDCommandBatch() : count(0) {}
        ...
        bool add_command(const char* cmd_ptr, size_t cmd_len) {
            if (count >= SIMD_BATCH_SIZE) return false;
            
            commands[count].command_ptr = cmd_ptr;
            commands[count].command_len = cmd_len;
            count++;
            return true;
        }
    };
    
    // Thread-local SIMD batch buffer
    thread_local SIMDCommandBatch simd_batch;
    
    // **OPTIMIZATION 1.2C.1**: SIMD Command Classification
    inline void classify_commands_simd(SIMDCommandBatch& batch) {
        if (batch.count == 0) return;
        
        // Prepare SIMD constants for command comparison
        const __m256i get_pattern = _mm256_set1_epi32(0x746567); // "get" in little-endian
        const __m256i set_pattern = _mm256_set1_epi32(0x746573); // "set" in little-endian
        const __m256i del_pattern = _mm256_set1_epi32(0x6c6564); // "del" in little-endian
        const __m256i ping_pattern = _mm256_set1_epi32(0x676e6970); // "ping" in little-endian (truncated)
        
        // Process commands in batches of 8
        for (size_t i = 0; i < batch.count; i += 8) {
            size_t process_count = std::min(static_cast<size_t>(8), batch.count - i);
            
            // Load first 4 bytes of each command (handles most command comparisons)
            __m256i cmd_data = _mm256_setzero_si256();
            uint32_t cmd_words[8] = {0};
            
            for (size_t j = 0; j < process_count; ++j) {
                if (batch.commands[i + j].command_len >= 3) {
                    // Load first 4 bytes (or less) of command
                    memcpy(&cmd_words[j], batch.commands[i + j].command_ptr, 
                           std::min(static_cast<size_t>(4), batch.commands[i + j].command_len));
                    // Convert to lowercase using bitwise OR
                    cmd_words[j] |= 0x20202020; // Convert to lowercase for all 4 bytes
                }
            }
            
            cmd_data = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(cmd_words));
            
            // Parallel comparison with all command types
            __m256i get_mask = _mm256_cmpeq_epi32(cmd_data, get_pattern);
            __m256i set_mask = _mm256_cmpeq_epi32(cmd_data, set_pattern);
            __m256i del_mask = _mm256_cmpeq_epi32(cmd_data, del_pattern);
            
            // Store classification results
            uint32_t get_results = _mm256_movemask_ps(_mm256_castsi256_ps(get_mask));
            uint32_t set_results = _mm256_movemask_ps(_mm256_castsi256_ps(set_mask));
            uint32_t del_results = _mm256_movemask_ps(_mm256_castsi256_ps(del_mask));
            
            // Apply classification results
            for (size_t j = 0; j < process_count; ++j) {
                uint8_t cmd_type = 0; // unknown
                
                if (get_results & (1 << j)) {
                    cmd_type = 1; // GET
                } else if (set_results & (1 << j)) {
                    cmd_type = 2; // SET
                } else if (del_results & (1 << j)) {
                    cmd_type = 3; // DEL
                } else if (batch.commands[i + j].command_len == 4) {
                    // Check for PING manually (4 bytes)
                    const char* cmd = batch.commands[i + j].command_ptr;
                    if ((cmd[0] | 0x20) == 'p' && (cmd[1] | 0x20) == 'i' &&
                        (cmd[2] | 0x20) == 'n' && (cmd[3] | 0x20) == 'g') {
                        cmd_type = 4; // PING
                    }
                }
                
                batch.commands[i + j].command_type = cmd_type;
            }
        }
    }
    
    // **OPTIMIZATION 1.2C.2: NUMA-Aware Memory Allocation**
    ...
    public:
        NUMAMemoryAllocator() : current_numa_node_(0), numa_available_(false) {
            if (numa_available() != -1) {
        ...
        void* allocate_numa_aligned(size_t size, size_t alignment = CACHE_LINE_SIZE) {
            if (!numa_available_) {
                return aligned_alloc(alignment, size);
            }
            
            void* ptr = numa_alloc_onnode(size, current_numa_node_);
            if (!ptr) {
        ...
        int get_current_numa_node() const {
            return numa_available_ ? numa_node_of_cpu(sched_getcpu()) : 0;
        }
    };
    
    // Thread-local NUMA allocator
    thread_local NUMAMemoryAllocator numa_allocator;
    
    // **OPTIMIZATION 1.2C.3: Cache-Line Aligned Memory Pools**
    ...
    private:
        ...
        struct alignas(CACHE_LINE_SIZE) PoolNode {
            alignas(CACHE_LINE_SIZE) char data[ALIGNED_SIZE];
            std::atomic<bool> in_use{false};
        };
        
        PoolNode* pool_nodes_;
        std::atomic<size_t> allocation_hint_{0};
        size_t pool_size_;
        
    public:
        CacheAlignedPool() : pool_size_(PoolSize) {
            // Allocate pool using NUMA-aware allocation
            size_t total_size = sizeof(PoolNode) * pool_size_;
            pool_nodes_ = reinterpret_cast<PoolNode*>(
                numa_allocator.allocate_numa_aligned(total_size, CACHE_LINE_SIZE));
            
            // Initialize pool nodes
            for (size_t i = 0; i < pool_size_; ++i) {
                pool_nodes_[i].in_use.store(false);
            }
        }
        
        ~CacheAlignedPool() {
            numa_allocator.deallocate_numa(pool_nodes_, sizeof(PoolNode) * pool_size_);
        }
        
        T* allocate() {
            size_t start_hint = allocation_hint_.load();
            
            // First try: linear search from hint
            for (size_t i = 0; i < pool_size_; ++i) {
                size_t idx = (start_hint + i) % pool_size_;
                bool expected = false;
                if (pool_nodes_[idx].in_use.compare_exchange_weak(expected, true)) {
                    allocation_hint_.store((idx + 1) % pool_size_);
                    return reinterpret_cast<T*>(pool_nodes_[idx].data);
                }
            }
            
            // Fallback: regular malloc with alignment
            return reinterpret_cast<T*>(numa_allocator.allocate_numa_aligned(sizeof(T), CACHE_LINE_SIZE));
        }
        
        void deallocate(T* ptr) {
            // Check if pointer is from our pool
            if (ptr >= reinterpret_cast<T*>(pool_nodes_) && 
                ptr < reinterpret_cast<T*>(pool_nodes_ + pool_size_)) {
                
                size_t idx = (reinterpret_cast<char*>(ptr) - reinterpret_cast<char*>(pool_nodes_)) / sizeof(PoolNode);
                if (idx < pool_size_) {
                    pool_nodes_[idx].in_use.store(false);
                    return;
                }
            }
            
            // Not from pool, use regular deallocation
            numa_allocator.deallocate_numa(ptr, sizeof(T));
        }
    };
    
    // **OPTIMIZATION 1.2C.4: CPU Cache Optimization with Prefetching**
    class CachePrefetcher {
    private:
        static constexpr size_t PREFETCH_DISTANCE = 2; // Prefetch 2 cache lines ahead
    ...
    public:
        ...
        static inline void prefetch_range(const void* start, size_t size) {
            const char* ptr = reinterpret_cast<const char*>(start);
            const char* end = ptr + size;
            
            for (; ptr < end; ptr += 64) { // 64 = cache line size
                _mm_prefetch(ptr, _MM_HINT_T0);
            }
        }
        
        // Strategic prefetch for command processing
        static inline void prefetch_commands(const std::vector<std::string_view>& commands) {
            for (size_t i = 0; i < std::min(commands.size(), PREFETCH_DISTANCE); ++i) {
                if (!commands[i].empty()) {
                    prefetch_read(commands[i].data());
                }
            }
        }
    };
    
    // **OPTIMIZATION 1.2C.5: Advanced io_uring SQPOLL Configuration**
    struct AdvancedUringConfig {
        static constexpr uint32_t QUEUE_DEPTH = 512;
        static constexpr uint32_t BATCH_SUBMIT = 64;
        static constexpr uint32_t SQPOLL_TIMEOUT_MS = 1;
        
        // Enhanced SQPOLL setup with CPU affinity
        static void setup_advanced_sqpoll(struct io_uring* ring, int cpu_id) {
            struct io_uring_params params = {};
            params.flags = IORING_SETUP_SQPOLL | IORING_SETUP_SQ_AFF;
            params.sq_thread_cpu = cpu_id;
            params.sq_thread_idle = SQPOLL_TIMEOUT_MS;
            
            int ret = io_uring_queue_init_params(QUEUE_DEPTH, ring, &params);
            if (ret < 0) {
                // Fallback to regular io_uring without SQPOLL
                io_uring_queue_init(QUEUE_DEPTH, ring, 0);
            }
        }
        
        // Batch submission optimization
        static int submit_batch_optimized(struct io_uring* ring, 
                                        const std::vector<struct iovec>& responses,
                                        const std::vector<int>& client_fds) {
            size_t submitted = 0;
            
            for (size_t i = 0; i < responses.size() && i < client_fds.size(); ++i) {
                struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
                if (!sqe) break;
                
                io_uring_prep_send(sqe, client_fds[i], 
                                  responses[i].iov_base, responses[i].iov_len, 
                                  MSG_NOSIGNAL);
                sqe->user_data = client_fds[i]; // Store client_fd for completion handling
                submitted++;
                
                // Submit in batches for optimal performance
                if (submitted % BATCH_SUBMIT == 0) {
    ...
    // Global CPU and memory optimization state
    static std::atomic<bool> cpu_optimizations_enabled{false};
    static std::atomic<bool> numa_optimizations_enabled{false};
    static std::atomic<size_t> total_simd_operations{0};
    static std::atomic<size_t> cache_prefetch_operations{0};
    
    // Initialize CPU and memory optimizations
    void initialize_cpu_memory_optimizations() {
        // Check CPU features
        if (simd::has_avx2()) {
            cpu_optimizations_enabled.store(true);
        }
        
        // Check NUMA availability
        if (numa_available() != -1) {
            numa_optimizations_enabled.store(true);
        }
    }
    ...
    struct OptimizationStats {
        ...
        size_t cache_prefetch_ops;
        bool cpu_optimizations_active;
        bool numa_optimizations_active;
        int current_numa_node;
    };
    
    OptimizationStats get_optimization_stats() {
        return {
            total_simd_operations.load(),
            cache_prefetch_operations.load(),
            cpu_optimizations_enabled.load(),
            numa_optimizations_enabled.load(),
            numa_allocator.get_current_numa_node()
        };
    }
...
public:
    // **PHASE 1.2C: ULTRA-ADVANCED BATCH PROCESSING WITH CPU & MEMORY OPTIMIZATION**
    void process_pending_batch() {
        ...
        // **OPTIMIZATION 1.2C.1**: SIMD Batch Command Processing
        if (batch_size >= cpu_memory_optimization::SIMD_BATCH_SIZE && 
            cpu_memory_optimization::cpu_optimizations_enabled.load()) {
            
            // Prepare SIMD command batch
            cpu_memory_optimization::simd_batch.clear();
            
            // Prefetch command data for better cache performance
            std::vector<std::string_view> command_views;
            command_views.reserve(batch_size);
            
            for (const auto& op : pending_operations_) {
                command_views.emplace_back(op.command);
                cpu_memory_optimization::simd_batch.add_command(op.command.data(), op.command.length());
            }
            
            // **OPTIMIZATION 1.2C.4**: Strategic prefetching
            cpu_memory_optimization::CachePrefetcher::prefetch_commands(command_views);
            cpu_memory_optimization::cache_prefetch_operations.fetch_add(1);
            
            // **OPTIMIZATION 1.2C.1**: SIMD command classification
            cpu_memory_optimization::classify_commands_simd(cpu_memory_optimization::simd_batch);
            cpu_memory_optimization::total_simd_operations.fetch_add(1);
        ...
        // **STEP 4A: Legacy SIMD-optimized batch key hashing (preserved for compatibility)**
        if (batch_size >= 4 && simd::has_avx2()) {
            prepare_simd_batch();
        }
        
        // **PHASE 1.2B.1: Response Vectoring** - Group responses by client_fd for batched sends  
        // **OPTIMIZATION 1.2C.2**: Use NUMA-aware memory allocation for response storage
...
public:
    ThreadPerCoreServer(const std::string& host, int port, size_t num_cores, size_t num_shards,
        ...
        // **PHASE 1.2C: Initialize advanced CPU & memory optimizations**
        cpu_memory_optimization::initialize_cpu_memory_optimizations();
        auto opt_stats = cpu_memory_optimization::get_optimization_stats();
        
        std::cout << "🧠 CPU & Memory Optimizations:" << std::endl;
        std::cout << "   ✅ SIMD Processing: " << (opt_stats.cpu_optimizations_active ? "ENABLED (AVX-256)" : "DISABLED") << std::endl;
        std::cout << "   ✅ NUMA Memory: " << (opt_stats.numa_optimizations_active ? "ENABLED" : "DISABLED") 
                  << " (Node: " << opt_stats.current_numa_node << ")" << std::endl;
        std::cout << "   ✅ Cache Prefetching: ENABLED" << std::endl;
        std::cout << "   ✅ Memory Pool Alignment: ENABLED (64-byte cache lines)" << std::endl;
