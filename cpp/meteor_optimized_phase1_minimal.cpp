...
// **PHASE 1 MINIMAL OPTIMIZATION**: Stack-based buffers to eliminate string copying
struct ConnectionMigrationMessage {
    ...
    // **ZERO-ALLOCATION BUFFERS**: Stack-based, no heap allocations
    char pending_command[MAX_COMMAND_SIZE];
    char pending_key[MAX_KEY_SIZE]; 
    char pending_value[MAX_VALUE_SIZE];
    ...
    ConnectionMigrationMessage(int fd, int src_core, const std::string& cmd = "", 
        ...
        : client_fd(fd), source_core(src_core), timestamp(std::chrono::steady_clock::now()) {
        
        // **OPTIMIZED COPYING**: Direct memory copy, no string objects
        ...
        memcpy(pending_command, cmd.c_str(), command_len);
        memcpy(pending_key, k.c_str(), key_len);
        memcpy(pending_value, v.c_str(), value_len);
        
        pending_command[command_len] = '\0';
        pending_key[key_len] = '\0';
        pending_value[value_len] = '\0';
    }
    
    // **FAST ACCESSORS**: Return string_view to avoid string construction
    std::string_view get_command() const { return std::string_view(pending_command, command_len); }
    std::string_view get_key() const { return std::string_view(pending_key, key_len); }  
    std::string_view get_value() const { return std::string_view(pending_value, value_len); }
};
