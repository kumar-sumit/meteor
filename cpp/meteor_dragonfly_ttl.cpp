// **METEOR v8.0: DRAGONFLY-INSPIRED TTL OVERLAY - SENIOR ARCHITECT IMPLEMENTATION**
//
...
namespace zero_copy {
    ...
    // **DRAGONFLY TTL COMMAND DETECTION**: Zero-allocation, fast detection
...
namespace storage {
    ...
    public:
        struct Entry {
            ...
            // **DRAGONFLY-INSPIRED TTL**: Optional field, zero overhead when not used
            ...
            Entry(const std::string& k, const std::string& v, std::chrono::seconds ttl) 
                : key(k), value(v), expire_at(std::chrono::steady_clock::now() + ttl) {}
                
            // **DRAGONFLY-STYLE**: Efficient TTL check
    ...
    private:
        ...
        // **DRAGONFLY-INSPIRED TTL OVERLAY**: Separate, conditional TTL storage
        std::unordered_map<std::string, std::chrono::steady_clock::time_point> ttl_expiry_;
    ...
    public:
        ...
        std::optional<std::string> get(const std::string& key) {
            // **BASELINE PRESERVED**: Exact original logic for baseline keys
            auto it = data_.find(key);
            if (it == data_.end()) return std::nullopt;
            
            // **CONDITIONAL TTL CHECK**: Only if TTL map has entries (zero overhead for baseline)
            if (!ttl_expiry_.empty()) {
                auto ttl_it = ttl_expiry_.find(key);
                if (ttl_it != ttl_expiry_.end() && std::chrono::steady_clock::now() >= ttl_it->second) {
                    // **SAFE EXPIRATION**: Return nullopt without corrupting data structure
                    return std::nullopt;
                }
            }
        ...
        bool del(const std::string& key) {
            // **TRUE SHARED-NOTHING**: Direct access to core's own data (no locking needed!)
            ttl_expiry_.erase(key); // Clean TTL overlay if present
            return data_.erase(key) > 0;
        }
        
        // **DRAGONFLY-INSPIRED TTL COMMANDS**: Minimal overlay implementation
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
            // Only works on existing keys (Dragonfly principle)
            if (data_.find(key) == data_.end()) return false;
            ttl_expiry_[key] = std::chrono::steady_clock::now() + ttl_seconds;
            return true;
        }
        
        std::optional<std::chrono::seconds> ttl(const std::string& key) {
            // Check baseline first (Dragonfly approach)
            if (data_.find(key) == data_.end()) {
                return std::nullopt; // -2 in Redis (key doesn't exist)
            }
            
            auto ttl_it = ttl_expiry_.find(key);
            if (ttl_it == ttl_expiry_.end()) {
                return std::chrono::seconds(-1); // Key exists but no TTL
            }
            
            auto now = std::chrono::steady_clock::now();
            if (now >= ttl_it->second) {
                // Key expired - return as non-existent (Redis behavior)
                return std::nullopt; // -2 in Redis
            }
            
            auto remaining = std::chrono::duration_cast<std::chrono::seconds>(ttl_it->second - now);
            return remaining.count() > 0 ? remaining : std::chrono::seconds(0);
        }
        
        bool persist(const std::string& key) {
            // Only works on existing keys with TTL
            if (data_.find(key) == data_.end()) return false;
            return ttl_expiry_.erase(key) > 0;
        }
...
namespace cache {
    ...
    public:
        ...
        // **DRAGONFLY TTL OVERLAY**: Pass-through to VLLHashTable TTL methods
        bool expire(const std::string& key, std::chrono::seconds ttl_seconds) {
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        if (zero_copy::is_get_command(op.command)) {
            ...
            if (value) {
                ...
                static thread_local std::string get_response_copy; // Thread-local string for copying
                char temp_buffer[8192];
                size_t len = zero_copy::format_get_response_direct(temp_buffer, *value);
                
                // **FIX: Copy to persistent thread-local string to avoid buffer corruption**
                get_response_copy.assign(temp_buffer, len);
                return ResponseInfo(get_response_copy.c_str(), get_response_copy.length(), false);
            } else {
        ...
        // **DRAGONFLY TTL COMMANDS**: Minimal overlay, zero baseline impact
        if (zero_copy::is_expire_command(op.command)) {
            try {
                ...
                return success ?
                    ResponseInfo(":1\r\n", 4) :  // Redis: 1 if timeout set
                    ResponseInfo(":0\r\n", 4);   // Redis: 0 if key doesn't exist
            } catch (...) {
        ...
        if (zero_copy::is_ttl_command(op.command)) {
            ...
            if (remaining_ttl) {
                ...
                if (ttl_count == -1) {
                    ...
                    // Key has TTL
                    static thread_local std::string ttl_response_copy; // Thread-local copy
                    int len = snprintf(ttl_buffer, sizeof(ttl_buffer), ":%ld\r\n", ttl_count);
                    ttl_response_copy.assign(ttl_buffer, len);
                    return ResponseInfo(ttl_response_copy.c_str(), ttl_response_copy.length(), false);
                }
            } else {
        ...
        if (zero_copy::is_persist_command(op.command)) {
            ...
            return success ?
                ResponseInfo(":1\r\n", 4) :  // Redis: 1 if TTL removed
                ResponseInfo(":0\r\n", 4);   // Redis: 0 if key has no TTL
        }
