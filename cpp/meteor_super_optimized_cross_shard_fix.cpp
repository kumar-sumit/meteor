...
private:
    ...
    // **ROUTING FIX: Core and shard information for proper cross-shard coordination**
    int core_id_;
    size_t total_shards_;
    
    // **ASYNC CROSS-SHARD**: Store pending cross-shard operations (non-blocking)
    ...
    std::vector<PendingCrossShardOp> pending_cross_shard_operations_;
...
public:
    ...
    // **ROUTING FIX: NON-BLOCKING CROSS-SHARD ROUTING FOR SINGLE COMMANDS**
    void submit_operation(const std::string& command, const std::string& key, 
        ...
        // **CRITICAL FIX**: Apply same cross-shard routing as pipelines, but non-blocking
        if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
            ...
            if (target_shard != current_shard) {
                // **NON-BLOCKING CROSS-SHARD**: Store future for later processing (like pipelines)
                if (dragonfly_cross_shard::global_cross_shard_coordinator) {
                    try {
                        ...
                        // **ASYNC PATTERN**: Store future for event loop to check later (NO BLOCKING!)
                        pending_cross_shard_operations_.emplace_back(std::move(future), client_fd);
    ...
    void flush_pending_operations() {
        ...
        // **ASYNC CROSS-SHARD**: Check and process ready cross-shard futures
        process_pending_cross_shard_operations();
    }
    
private:
    // **ASYNC CROSS-SHARD**: Process ready cross-shard operations (non-blocking)
    void process_pending_cross_shard_operations() {
        if (pending_cross_shard_operations_.empty()) {
            return;
        }
        
        auto now = std::chrono::steady_clock::now();
        static constexpr auto TIMEOUT = std::chrono::milliseconds(1000); // 1 second timeout
        
        // Process ready futures (iterate backwards for safe removal)
        for (auto it = pending_cross_shard_operations_.rbegin(); it != pending_cross_shard_operations_.rend(); ) {
            bool should_remove = false;
            
            // Check if future is ready (non-blocking) - FIX: Use boost::fibers::future_status
            ...
            if (should_remove) {
                ...
                auto forward_it = pending_cross_shard_operations_.erase((it + 1).base());
...
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands) {
    ...
    // **ROUTING FIX**: Use proper shard calculation for this core
    size_t current_shard = core_id_ % total_shards_;  // FIXED: Was hardcoded to 0!
