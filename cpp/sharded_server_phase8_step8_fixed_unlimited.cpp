// **PHASE 8 STEP 8: RACE-CONDITION-FREE UNLIMITED STORAGE**
// Fixed all race conditions from Step 7 and implemented truly unlimited storage:
// - NO connection migration (DragonflyDB never migrates)
// - Fixed B+ tree race conditions (atomic operations)
// - Unlimited node expansion (no capacity limits)
// - 256 segments for minimal contention
// - Proven pipeline batch processing
// Target: 1.9M+ RPS + zero SET failures + sub-ms latency + 16-core scaling
...
// **PHASE 8 STEP 7: FREECACHE + DRAGONFLY HYBRID STORAGE**
namespace storage {
    ...
    class RaceFreeBTreeStorage {
    private:
        // **OPTIMIZED CONSTANTS** (race-condition-free + unlimited)
        static constexpr size_t SEGMENTS_PER_CORE = 256;  // FreeCache uses 256 segments
        static constexpr size_t BTREE_NODE_SIZE = 1024;   // Larger nodes = fewer allocations
        static constexpr size_t MAX_KEYS_PER_NODE = 64;   // 4x larger nodes for better performance
        
        // **B+ TREE NODE** (DragonflyDB-style)
        struct BTreeNode {
            std::array<std::string, MAX_KEYS_PER_NODE> keys;
            std::array<std::string, MAX_KEYS_PER_NODE> values;
            std::atomic<size_t> count{0};
            std::atomic<bool> is_sorted{true};
            
            // **BINARY SEARCH** for O(log N) lookups
            std::optional<std::string> find(const std::string& key) const {
                if (count.load() == 0) return std::nullopt;
                
                // Linear search for small nodes, binary search for larger ones
                size_t node_count = count.load();
                if (node_count <= 4) {
                    ...
                    // Binary search for larger nodes (DragonflyDB approach)
                    size_t left = 0, right = node_count;
                    while (left < right) {
            ...
            // **RACE-CONDITION-FREE INSERT** (atomic operations)
            bool insert(const std::string& key, const std::string& value) {
                // **FIX 1**: Atomic reservation of slot BEFORE writing
                size_t slot = count.fetch_add(1);
                if (slot >= MAX_KEYS_PER_NODE) {
                    count.fetch_sub(1);  // Rollback
                    return false;  // Node full
                }
                
                // **FIX 2**: Write to reserved slot (no race condition)
                keys[slot] = key;
                values[slot] = value;
                is_sorted.store(false);
                
                // Sort if we have enough elements to benefit from binary search
                if (slot + 1 > 4) {
                    sort_node();
                }
            ...
            // **SORT NODE** for binary search efficiency
            void sort_node() {
                if (is_sorted.load()) return;
                
                size_t node_count = count.load();
                // Simple insertion sort (efficient for small arrays)
                for (size_t i = 1; i < node_count; ++i) {
                    std::string key_temp = keys[i];
                    std::string value_temp = values[i];
                    size_t j = i;
                    
                    while (j > 0 && keys[j-1] > key_temp) {
                        keys[j] = keys[j-1];
                        values[j] = values[j-1];
                        j--;
                    }
                ...
                is_sorted.store(true);
            }
            
            // **DELETE** with compaction
            bool remove(const std::string& key) {
                size_t node_count = count.load();
                for (size_t i = 0; i < node_count; ++i) {
                    if (keys[i] == key) {
                        // Shift elements down
                        for (size_t j = i; j < node_count - 1; ++j) {
                            keys[j] = keys[j + 1];
                            values[j] = values[j + 1];
                        }
                        count.fetch_sub(1);
        ...
        // **FREECACHE SEGMENT** (ring buffer + B+ tree hybrid)
        struct Segment {
            std::vector<std::unique_ptr<BTreeNode>> nodes;
            mutable std::shared_mutex mutex;  // Per-segment lock (minimal contention)
            std::atomic<size_t> total_keys{0};
            
            Segment() {
                nodes.reserve(64);  // Start with reasonable capacity
            }
            
            std::optional<std::string> get(const std::string& key) {
                std::shared_lock<std::shared_mutex> lock(mutex);
                
                // Search through B+ tree nodes
            ...
            bool set(const std::string& key, const std::string& value) {
                ...
                // **FIX 3**: First, try to update existing key
                for (const auto& node : nodes) {
                    size_t node_count = node->count.load();
                    for (size_t i = 0; i < node_count; ++i) {
                        if (node->keys[i] == key) {
                ...
                // **FIX 4**: Try inserting into existing nodes
                for (const auto& node : nodes) {
                ...
                // **FIX 5**: UNLIMITED EXPANSION - always create new node if needed
                auto new_node = std::make_unique<BTreeNode>();
                if (new_node->insert(key, value)) {
                    nodes.push_back(std::move(new_node));
                    total_keys.fetch_add(1);
                    return true;
                }
                
                // **FIX 6**: Fallback - should NEVER fail with unlimited storage
                std::cerr << "❌ CRITICAL: Unlimited storage failed for key: " << key << std::endl;
                return false;
            }
        ...
        // **PER-CORE STORAGE** (FreeCache approach)
        std::array<Segment, SEGMENTS_PER_CORE> segments_;
        std::unique_ptr<memory::AdvancedMemoryPool> memory_pool_;
        
        // **HASH TO SEGMENT** (FreeCache-style)
        size_t hash_to_segment(const std::string& key) const {
            // Use high-quality hash function (similar to FreeCache)
            std::hash<std::string> hasher;
            return hasher(key) % SEGMENTS_PER_CORE;
        }
                
    public:
        RaceFreeBTreeStorage()
            : memory_pool_(std::make_unique<memory::AdvancedMemoryPool>()) {
            // No initialization needed - segments are initialized by default
        }
        
        bool set(const std::string& key, const std::string& value) {
            size_t segment_id = hash_to_segment(key);
            return segments_[segment_id].set(key, value);
        }
        
        std::optional<std::string> get(const std::string& key) {
            size_t segment_id = hash_to_segment(key);
            return segments_[segment_id].get(key);
        }
        
        bool del(const std::string& key) {
            size_t segment_id = hash_to_segment(key);
            return segments_[segment_id].del(key);
        }
        
        size_t size() const {
            size_t total = 0;
            for (const auto& segment : segments_) {
                total += segment.total_keys.load();
            }
            return total;
        }
        ...
        struct Stats {
            ...
            size_t total_nodes;
            double average_keys_per_segment;
            double average_keys_per_node;
        };
        ...
        Stats get_stats() const {
            ...
            for (const auto& segment : segments_) {
                stats.total_entries += segment.total_keys.load();
                stats.total_segments++;
                
                std::shared_lock<std::shared_mutex> lock(segment.mutex);
                stats.total_nodes += segment.nodes.size();
            }
            
            if (stats.total_segments > 0) {
                stats.average_keys_per_segment = static_cast<double>(stats.total_entries) / stats.total_segments;
            }
            if (stats.total_nodes > 0) {
                stats.average_keys_per_node = static_cast<double>(stats.total_entries) / stats.total_nodes;
            }
...
namespace cache {
    class HybridCache {
    private:
        std::unique_ptr<storage::RaceFreeBTreeStorage> memory_cache_;
    ...
    public:
        HybridCache(size_t memory_shards, const std::string& ssd_path, size_t max_memory_mb = 256)
            ...
            memory_cache_ = std::make_unique<storage::RaceFreeBTreeStorage>();
        ...
        struct Stats {
            storage::RaceFreeBTreeStorage::Stats memory_stats;
    ...
    void start_core_threads() {
        ...
        for (size_t i = 0; i < num_cores_; ++i) {
            ...
            if (dedicated_fd == -1) {
                ...
                std::cerr << "💡 This might be due to port " << port_ << " already in use or SO_REUSEPORT not supported" << std::endl;
                std::cerr << "🔧 Try: sudo pkill -f meteor_phase8_step7 or use a different port" << std::endl;
