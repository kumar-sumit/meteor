...
public:
    ...
    // **REUSE PROVEN PIPELINE PATTERN**: Apply same cross-shard logic to single commands
    bool process_single_command_with_routing(const std::string& command, const std::string& key, 
                                           const std::string& value, int client_fd);
...
// **REUSE PROVEN PIPELINE PATTERN**: Apply exact same cross-shard logic to single commands
bool DirectOperationProcessor::process_single_command_with_routing(const std::string& command, const std::string& key, 
                                                                 const std::string& value, int client_fd) {
    // **EXACT SAME PATTERN AS PIPELINE**: Per-command routing (not all-or-nothing)
    std::vector<boost::fibers::future<std::string>> cross_shard_futures;
    std::vector<std::string> local_responses;
    size_t total_shards = 6;  // Optimal for 12-core systems (reduced from 12)
    // TODO: Get current shard from thread-local storage or pass as parameter
    ...
    // **SINGLE COMMAND ROUTING**: Same logic as pipeline per-command
    if (!key.empty()) {
        // Determine target shard for this specific command
        size_t target_shard = std::hash<std::string>{}(key) % total_shards;
        ...
        // **NO KEY**: Process locally (PING, INFO, etc.)
        BatchOperation op(command, key, value, client_fd);
        std::string response = execute_single_operation(op);
        local_responses.push_back(response);
    }
    
    // **EXACT SAME RESPONSE COLLECTION**: Local + cross-shard (like pipeline)
    std::vector<std::string> all_responses;
    ...
    for (auto& future : cross_shard_futures) {
        try {
            // Cooperative wait - yields to other fibers
            std::string response = future.get();
            all_responses.push_back(response);
        } catch (const boost::fibers::channel_closed& e) {
            // Target shard is down - return error response
            all_responses.push_back("-ERR target shard unavailable\r\n");
        }
    }
    
    // **SINGLE RESPONSE**: Send combined response (for single command, just one response)
    bool success = false;
    if (!all_responses.empty()) {
        std::string response = all_responses[0]; // Single command = single response
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    // **REUSE PROVEN PIPELINE PATTERN**: Apply same cross-shard logic as working pipelines
                    // This uses the exact same Boost.Fibers coordination that gives pipelines 3.85M QPS
                    processor_->process_single_command_with_routing(command, key, value, client_fd);
                }
            }
        }
    }
