...
public:
    ...
    // **UNIFIED PROCESSING FIX**: All commands use execute_single_operation() directly
    // This eliminates the 100μs MAX_BATCH_DELAY that caused pipeline=1 p99.9 ~10ms latency
    void submit_operation(const std::string& command, const std::string& key, 
                         const std::string& value, int client_fd) {
        // **LATENCY FIX**: Process immediately using execute_single_operation()
        // This matches the immediate processing behavior used by pipeline commands
        BatchOperation op(command, key, value, client_fd);
        std::string response = execute_single_operation(op);
        
        // Send response immediately (same pattern as process_pending_batch)
