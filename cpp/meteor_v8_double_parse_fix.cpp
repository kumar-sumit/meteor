...
private:
    ...
    void process_pipeline_with_mget_support(int client_fd, std::vector<std::vector<std::string>>& parsed_commands) {
        ...
        if (parsed_commands.size() == 1) {
            ...
            // **APPROACH 1 TEST**: Use same single send() as pipeline=2+ (should work if issue was in response collection)
        ...
        if (parsed_commands.size() == 2 && 
            ...
            if (first_response_end != std::string::npos) {
                ...
                // **SUCCESS**: Send trimmed response (should achieve pipeline=2+ performance)
                send(client_fd, trimmed_response.c_str(), trimmed_response.length(), MSG_NOSIGNAL);
                return;
            }
        }
        
        // **NORMAL PATH**: Send complete pipeline response
        send(client_fd, pipeline_response.c_str(), pipeline_response.length(), MSG_NOSIGNAL);
    }
