...
public:
    ...
    // **DRAGONFLY-STYLE IMMEDIATE PROCESSING - NO BATCHING DELAYS FOR SINGLE COMMANDS**
    void submit_operation(const std::string& command, const std::string& key, 
                         const std::string& value, int client_fd) {
        
        // **CRITICAL FIX**: IMMEDIATE processing for single commands (DragonflyDB approach)
        // Root cause: batching delay of 100μs + event loop delay = ~10ms total latency
        // DragonflyDB uses immediate processing without batching for low latency
...
private:
    ...
    void process_pipeline_with_mget_support(int client_fd, std::vector<std::vector<std::string>>& parsed_commands) {
        ...
        for (const auto& cmd : parsed_commands) {
            if (!cmd.empty()) {
                ...
                // **UNIFIED COMMAND CONVERSION**: Convert single commands to multi-command equivalents
                if (cmd_upper == "GET" && cmd.size() == 2) {
                ...
                } else if (cmd_upper == "SET" && cmd.size() == 3) {
                    ...
                    // **REGULAR COMMAND WITH CROSS-SHARD ROUTING**: Essential for correctness
                    ...
                    if (cmd_upper == "SETEX" && cmd.size() >= 4) {
                        std::string ttl_seconds = cmd[2];
                        std::string actual_value = cmd[3];
                        value = ttl_seconds + " " + actual_value;
                    }
                    
                    // **CROSS-SHARD ROUTING FOR KEY COMMANDS**: Critical for correctness 
                    // Note: GET and SET are handled by MGET/MSET conversion above
                    if (!key.empty() && (cmd_upper == "DEL" || cmd_upper == "SETEX" || 
                                       cmd_upper == "TTL" || cmd_upper == "EXPIRE")) {
                        size_t shard_id = std::hash<std::string>{}(key) % total_shards_;
                        size_t key_target_core = shard_id % num_cores_;
                        
                        if (key_target_core == core_id_) {
                            // **LOCAL EXECUTION**: Process on correct core (this core)
                            BatchOperation op(command, key, value, client_fd);
                            std::string response = processor_->execute_single_operation(op);
                            pipeline_response += response;
                        } else {
                            // **CROSS-CORE EXECUTION**: Route to correct core like MGET does
                            if (key_target_core < all_cores_.size() && all_cores_[key_target_core] && 
                                all_cores_[key_target_core]->processor_) {
                                
                                auto* target_processor = all_cores_[key_target_core]->processor_.get();
                                if (target_processor) {
                                    BatchOperation op(command, key, value, 0);
                                    std::string response = target_processor->execute_single_operation(op);
                                    pipeline_response += response;
                                } else {
                                    std::string error_response = "-ERR target processor unavailable\r\n";
                                    pipeline_response += error_response;
                                }
                            } else {
                                std::string error_response = "-ERR target core unavailable\r\n";
                                pipeline_response += error_response;
                            }
                        }
                    } else {
                        // **NON-KEY COMMANDS**: Process locally (PING, etc.)
                        BatchOperation op(command, key, value, client_fd);
                        std::string response = processor_->execute_single_operation(op);
                        pipeline_response += response;
                    }
    ...
    bool process_events_zero_syscall() {
        ...
        // **DRAGONFLY OPTIMIZATION**: Process events efficiently
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1 || has_mget_in_commands) {  // **CORRECT CONDITION**: Only multi-command pipelines or MGET
            ...
            // **DEPRECATED PATH**: This path is never reached since ALL commands now use pipeline infrastructure
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    std::string command = parsed_cmd[0];
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL" || cmd_upper == "SETEX" || cmd_upper == "TTL" || cmd_upper == "EXPIRE") && !key.empty()) {
                        ...
                        if (target_core == core_id_) {
                            // **LOCAL FAST PATH**: Use optimized single operation processing (same as pipeline commands)
                            BatchOperation single_op(command, key, value, client_fd);
                            std::string response = processor_->execute_single_operation(single_op);
                    ...
                    // **FALLBACK**: For non-key commands (PING, INFO, etc.) - use optimized processing
                    BatchOperation fallback_op(command, key, value, client_fd);
                    std::string response = processor_->execute_single_operation(fallback_op);
                    send(client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
                }
