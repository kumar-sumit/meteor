...
namespace cpu_affinity {
    ...
    inline int create_reuseport_socket(const std::string& host, int port, bool& reuseport_success) {
        ...
        // **CONNECTION BURST FIX**: Optimize socket for high-concurrency connection bursts
        // Increase socket buffer sizes for better connection burst handling
        ...
        // Enable TCP_NODELAY for immediate response sending (disable Nagle)
        if (setsockopt(server_fd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) < 0) {
            // Don't fail if TCP_NODELAY fails (not critical for connection burst handling)
        }
...
public:
    ...
    // **CONNECTION BURST FIX**: Optimized accept loop for high-concurrency bursts
    void run_dedicated_accept() {
        std::cout << "🔌 Core " << core_id_ << " dedicated accept thread started (burst-optimized)" << std::endl;
        
        // Connection burst counters for adaptive behavior
        ...
        while (running_.load()) {
            ...
            while (consecutive_accepts < 64 && running_.load()) { // Max 64 accepts per iteration
                ...
                // **BURST OPTIMIZATION**: Minimal per-connection setup
                // Set non-blocking (essential)
                int flags = fcntl(client_fd, F_GETFL, 0);
                fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
                
                // **BURST OPTIMIZATION**: Optimize TCP settings for low latency
                int opt = 1;
            ...
            if (consecutive_accepts > 0 && total_accepted % 100 == 0) {
                std::cout << "🔌 Core " << core_id_ << " batch accepted " << consecutive_accepts 
                         << " connections (total: " << total_accepted << ")" << std::endl;
            }
            
            // **BURST OPTIMIZATION**: Brief yield if no accepts (prevent busy waiting)
            if (consecutive_accepts == 0) {
                std::this_thread::sleep_for(std::chrono::microseconds(100)); // 100μs when no connections
            }
        }
    ...
    void add_client_connection(int client_fd) {
        ...
        // **BURST OPTIMIZATION**: Remove verbose per-connection logging (causes bottleneck under high load)
        // Connection acceptance is tracked via metrics and batch logging in run_dedicated_accept()
    }
