...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        // **PURE PIPELINE UNIFIED**: Route ALL commands through proven pipeline processing
        // Single commands treated as pipeline with 1 command, multiple commands as normal pipeline
        ...
        if (!parsed_commands.empty()) {
            // **UNIFIED PROCESSING**: Use proven pipeline processing for ALL commands
            // No modifications to pipeline logic - just route everything through it
