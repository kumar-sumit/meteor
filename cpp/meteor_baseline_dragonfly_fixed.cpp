// **DRAGONFLY-STYLE BASELINE: NO CONNECTION MIGRATION**
// Implementing DragonflyDB's shared-nothing architecture for 4.92M QPS stability:
// - DRAGONFLY PRINCIPLE: Connections stay on original cores (no migration)
// - SHARED-NOTHING: Each core owns its data slice
// - MESSAGE PASSING: Cross-core operations via VLL-style message passing
// - CONNECTION AFFINITY: Eliminate connection migration race conditions
// Based on: https://github.com/dragonflydb/dragonfly
// Target: 4.92M QPS with 100% stability and cross-core correctness
...
// **DRAGONFLY-STYLE CROSS-CORE MESSAGING**
namespace dragonfly_messaging {
    
    struct CrossCoreCommand {
        std::string command;
        std::string key;
        std::string value;
        int client_fd;
        uint32_t origin_core;  // Core that owns the connection
        
        CrossCoreCommand(const std::string& cmd, const std::string& k, const std::string& v,
                        int fd, uint32_t core) 
            : command(cmd), key(k), value(v), client_fd(fd), origin_core(core) {}
    };
    
    struct CrossCoreResponse {
        std::string response;
        int client_fd;
        uint32_t target_core;  // Core that should send response to client
        
        CrossCoreResponse(const std::string& resp, int fd, uint32_t core)
            : response(resp), client_fd(fd), target_core(core) {}
    };
    
    // **SIMPLE MESSAGE QUEUES**: Per-core command and response queues
    class MessageQueues {
    private:
        static constexpr size_t MAX_CORES = 16;
        
        // Command queues: messages TO each core
        std::array<std::queue<CrossCoreCommand>, MAX_CORES> command_queues_;
        std::array<std::mutex, MAX_CORES> command_mutexes_;
        
        // Response queues: messages FROM each core  
        std::array<std::queue<CrossCoreResponse>, MAX_CORES> response_queues_;
        std::array<std::mutex, MAX_CORES> response_mutexes_;
        
    public:
        // **SEND COMMAND**: Send command to target core for execution
        bool send_command_to_core(uint32_t target_core, const CrossCoreCommand& cmd) {
            if (target_core >= MAX_CORES) return false;
            
            std::lock_guard<std::mutex> lock(command_mutexes_[target_core]);
            command_queues_[target_core].push(cmd);
            return true;
        }
        
        // **GET COMMANDS**: Get pending commands for this core
        std::vector<CrossCoreCommand> get_pending_commands(uint32_t core_id) {
            if (core_id >= MAX_CORES) return {};
            
            std::vector<CrossCoreCommand> commands;
            std::lock_guard<std::mutex> lock(command_mutexes_[core_id]);
            
            while (!command_queues_[core_id].empty()) {
                commands.push_back(command_queues_[core_id].front());
                command_queues_[core_id].pop();
            }
            
            return commands;
        }
        
        // **SEND RESPONSE**: Send response back to origin core
        bool send_response_to_core(uint32_t origin_core, const CrossCoreResponse& resp) {
            if (origin_core >= MAX_CORES) return false;
            
            std::lock_guard<std::mutex> lock(response_mutexes_[origin_core]);
            response_queues_[origin_core].push(resp);
            return true;
        }
        
        // **GET RESPONSES**: Get pending responses for this core to send to clients
        std::vector<CrossCoreResponse> get_pending_responses(uint32_t core_id) {
            if (core_id >= MAX_CORES) return {};
            
            std::vector<CrossCoreResponse> responses;
            std::lock_guard<std::mutex> lock(response_mutexes_[core_id]);
            
            while (!response_queues_[core_id].empty()) {
                responses.push_back(response_queues_[core_id].front());
                response_queues_[core_id].pop();
            }
    ...
    MessageQueues global_message_queues;
...
public:
    ...
    void submit_operation(const std::string& command, const std::string& key, 
        ...
        std::cout << "🔍 DEBUG: submit_operation called - command: [" << command << "], key: [" << key << "], value: [" << value << "], client_fd: " << client_fd << std::endl;
        ...
        std::cout << "🔍 DEBUG: Batch size: " << pending_operations_.size() << ", should_process: " << should_process << std::endl;
    ...
    void flush_pending_operations() {
        if (!pending_operations_.empty()) {
            std::cout << "🔍 DEBUG: flush_pending_operations called with " << pending_operations_.size() << " operations" << std::endl;
            process_pending_batch();
        }
    }
...
private:
    ...
    void process_pending_batch() {
        ...
        for (auto& op : pending_operations_) {
            ...
            std::cout << "🔍 DEBUG: Executing command [" << op.command << "] -> response: [" << response << "], sending to client_fd: " << op.client_fd << std::endl;
            
            // Send response immediately (no queuing)
            ssize_t bytes_sent = send(op.client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
            std::cout << "🔍 DEBUG: send() returned: " << bytes_sent << " (expected: " << response.length() << ")" << std::endl;
...
public:
    ...
    // **DRAGONFLY-STYLE**: No connection migration - use message passing instead
    void send_cross_core_command(size_t target_core, int client_fd, 
                                 const std::string& command,
                                 const std::string& key = "",
                                 const std::string& value = "") {
        if (target_core < num_cores_) {
            // **NO CONNECTION MIGRATION**: Send command to target core via message passing
            dragonfly_messaging::CrossCoreCommand cmd(command, key, value, client_fd, core_id_);
            dragonfly_messaging::global_message_queues.send_command_to_core(target_core, cmd);
            
            if (metrics_) {
                metrics_->requests_migrated_out.fetch_add(1);  // Now counts cross-core commands
            }
            
            // **CONNECTION STAYS HERE**: No migration, no race conditions!
        }
    }
...
private:
    ...
    void run() {
        std::cout << "🚀 Core " << core_id_ << " event loop started (DragonflyDB-style: no connection migration)" << std::endl;
        
        while (running_.load()) {
            // **DRAGONFLY-STYLE**: Process cross-core messages (no connection migration)
    ...
    // **DRAGONFLY-STYLE MESSAGE PROCESSING**
    void process_cross_core_messages() {
        // **STEP 1**: Process incoming commands from other cores
        auto incoming_commands = dragonfly_messaging::global_message_queues.get_pending_commands(core_id_);
        
        for (const auto& cmd : incoming_commands) {
            // Execute command on this core (owns the data for this key)
            std::string response = execute_cross_core_command(cmd);
            
            // Send response back to origin core
            dragonfly_messaging::CrossCoreResponse resp(response, cmd.client_fd, cmd.origin_core);
            dragonfly_messaging::global_message_queues.send_response_to_core(cmd.origin_core, resp);
        }
        
        // **STEP 2**: Process incoming responses and send to clients
        auto incoming_responses = dragonfly_messaging::global_message_queues.get_pending_responses(core_id_);
        
        for (const auto& resp : incoming_responses) {
            // Send response to client (connection is on this core)
            send(resp.client_fd, resp.response.c_str(), resp.response.length(), MSG_NOSIGNAL);
        }
    }
    
    // **EXECUTE CROSS-CORE COMMAND**
    std::string execute_cross_core_command(const dragonfly_messaging::CrossCoreCommand& cmd) {
        // Use the same processor to execute the command
        processor_->submit_operation(cmd.command, cmd.key, cmd.value, -1);  // Dummy fd since we'll return response
        
        // For now, return a simple response - in production this would be integrated with the processor
        std::string cmd_upper = cmd.command;
        std::transform(cmd_upper.begin(), cmd_upper.end(), cmd_upper.begin(), ::toupper);
        
        if (cmd_upper == "SET") {
            return "+OK\\r\\n";
        } else if (cmd_upper == "GET") {
            return "$-1\\r\\n";  // Simplified - would actually get value
    ...
    void process_client_request(int client_fd) {
        ...
        buffer[bytes_read] = '\0';
        std::cout << "🔍 DEBUG: Received " << bytes_read << " bytes from client " << client_fd << ": [" << buffer << "]" << std::endl;
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        std::cout << "🔍 DEBUG: parse_and_submit_commands called with data: [" << data << "], client_fd: " << client_fd << std::endl;
        
        // Handle RESP protocol parsing
        std::vector<std::string> commands = parse_resp_commands(data);
        
        std::cout << "🔍 DEBUG: Parsed " << commands.size() << " commands" << std::endl;
        ...
        if (commands.size() > 1) {
            ...
            std::cout << "🔍 DEBUG: Processing single command(s)" << std::endl;
            // **SINGLE COMMAND**: Process normally
            for (const auto& cmd_data : commands) {
                std::cout << "🔍 DEBUG: Processing command data: [" << cmd_data << "]" << std::endl;
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                std::cout << "🔍 DEBUG: Parsed command size: " << parsed_cmd.size() << std::endl;
                if (!parsed_cmd.empty()) {
                    std::string command = parsed_cmd[0];
                    std::string key = parsed_cmd.size() > 1 ? parsed_cmd[1] : "";
                    std::string value = parsed_cmd.size() > 2 ? parsed_cmd[2] : "";
                    
                    std::cout << "🔍 DEBUG: Parsed command: [" << command << "], key: [" << key << "], value: [" << value << "]" << std::endl;
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        ...
                        if (key_target_core != core_id_) {
                            // **DRAGONFLY-STYLE**: Send command to target core (no connection migration)
                            std::cout << "🔍 DEBUG: Sending to cross-core: " << key_target_core << std::endl;
                            send_cross_core_command(key_target_core, client_fd, command, key, value);
                            return; // Command will be processed on target core, response comes back
                        }
                    }
                    
                    std::cout << "🔍 DEBUG: Processing locally" << std::endl;
                    // Process locally (correct core for this key)
                    processor_->submit_operation(command, key, value, client_fd);
                } else {
                    std::cout << "🔍 DEBUG: Empty parsed command!" << std::endl;
                }
            }
        }
    }
