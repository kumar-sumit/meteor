...
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands, size_t core_id, size_t total_shards) {
    ...
    // **CONSISTENCY FIX**: Use same core-based routing as working baseline single commands
    size_t current_core = core_id;
    
    // **KEY INSIGHT**: Use IDENTICAL routing logic as working baseline
    for (size_t cmd_idx = 0; cmd_idx < commands.size(); ++cmd_idx) {
        ...
        if (cmd_parts.size() >= 2) {
            ...
            // **IDENTICAL LOGIC**: Use same key→core mapping as baseline single commands
            size_t target_core = std::hash<std::string>{}(key) % 4; // Use num_cores directly
            
            if (target_core == current_core) {
                // **LOCAL FAST PATH**: Execute immediately with working baseline logic
                ...
                // **CROSS-CORE PATH**: For now, execute locally to ensure correctness
                // (Will optimize cross-core later)
                BatchOperation op(command, key, value, client_fd);
                std::string response = execute_single_operation(op);
                response_trackers.emplace_back(cmd_idx, true, response);
            }
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **CONSISTENT UNIFIED APPROACH**: Use same pipeline processing for single commands
            // This ensures identical cache access patterns and eliminates dual code paths
            std::vector<std::vector<std::string>> parsed_commands;
            ...
            if (!parsed_commands.empty()) {
                // **CONSISTENCY GUARANTEE**: Use same pipeline processing with fixed core routing
                processor_->process_pipeline_batch(client_fd, parsed_commands, core_id_, total_shards_);
            }
        }
    }
