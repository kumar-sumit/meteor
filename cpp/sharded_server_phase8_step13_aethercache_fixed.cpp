// **PHASE 8 STEP 13: AETHERCACHE OPTIMIZED CUCKOO HASH TABLE**
...
// - NO connection migration (true shared-nothing per core)
...
// **AETHERCACHE OPTIMIZED CUCKOO HASH TABLE**
// Based on NSDI '13 and EuroSys '14 research papers
namespace storage {
    class AetherCacheHashTable {
    private:
        // **AETHERCACHE CONSTANTS**
        static constexpr size_t SLOTS_PER_BUCKET = 4;     // 4 key/value pairs per bucket
        static constexpr size_t MAX_CUCKOO_PATH = 6;      // BFS guarantees short paths
        static constexpr size_t INITIAL_BUCKET_COUNT = 8192;  // Increased for lower load factor // Initial size
        static constexpr double MAX_LOAD_FACTOR = 0.95;   // Resize trigger
        
        // **PARTIAL HASH TAG** (8-bit tag for fast matching)
        using Tag = uint8_t;
        
        // **BUCKET SLOT** (key/value pair with tag)
        struct Slot {
            std::atomic<Tag> tag{0};                    // Partial hash for fast matching
            std::atomic<std::string*> key{nullptr};     // Pointer to key string
            std::atomic<std::string*> value{nullptr};   // Pointer to value string
            
            bool is_empty() const {
                return key.load(std::memory_order_acquire) == nullptr;
            }
            
            bool matches(Tag search_tag, const std::string& search_key) const {
                if (tag.load(std::memory_order_acquire) != search_tag) return false;
                auto* k = key.load(std::memory_order_acquire);
                return k && (*k == search_key);
            }
        };
        
        // **CUCKOO BUCKET** (4 slots + version counter for optimistic reads)
        struct Bucket {
            std::atomic<uint64_t> version{0};  // Version counter for optimistic reads
            Slot slots[SLOTS_PER_BUCKET];
            mutable std::mutex write_lock;     // Fine-grained lock for writes only
            
            // Optimistic read - completely lock-free
            std::optional<std::string> optimistic_find(Tag search_tag, const std::string& search_key) const {
                uint64_t v1, v2;
                std::string result;
                
                do {
                    v1 = version.load(std::memory_order_acquire);
                    if (v1 & 1) continue; // Odd version means write in progress
                    
                    // Scan slots for matching tag and key
                    for (const auto& slot : slots) {
                        if (slot.matches(search_tag, search_key)) {
                            auto* val = slot.value.load(std::memory_order_acquire);
                            if (val) result = *val;
                        }
                    }
            ...
            bool has_empty_slot() const {
                for (const auto& slot : slots) {
                    if (slot.is_empty()) return true;
                }
                return false;
            }
            
            // Find empty slot index (-1 if none)
        ...
        // **CUCKOO PATH NODE** (for BFS pathfinding with parent tracking)
        struct PathNode {
            size_t bucket_idx;
            int slot_idx;
            size_t depth;
            std::shared_ptr<PathNode> parent;  // For path reconstruction
            
            PathNode(size_t b, int s, size_t d, std::shared_ptr<PathNode> p = nullptr) 
                : bucket_idx(b), slot_idx(s), depth(d), parent(p) {}
        };
        
        // **HASH TABLE STATE**
        std::vector<std::unique_ptr<Bucket>> buckets_;
        std::atomic<size_t> bucket_count_{INITIAL_BUCKET_COUNT};
        std::atomic<size_t> item_count_{0};
        std::unique_ptr<memory::AdvancedMemoryPool> memory_pool_;
        
        // **HASH FUNCTIONS** (two independent high-quality hash functions)
        std::hash<std::string> hash1_;
        std::hash<std::string> hash2_;
        
        // Generate partial hash tag from full hash
        Tag make_tag(size_t full_hash) const {
            return static_cast<Tag>((full_hash >> 24) & 0xFF); // Use upper 8 bits
        }
        
        // Get two bucket indices for a key
        std::pair<size_t, size_t> get_bucket_indices(const std::string& key) const {
            size_t h1 = hash1_(key);
            size_t h2 = hash2_(key + "_salt"); // Add salt for independence
            size_t count = bucket_count_.load(std::memory_order_acquire);
            return {h1 % count, h2 % count};
        }
        
        // **BFS PATHFINDING** (find shortest displacement path with proper reconstruction)
        std::vector<PathNode> find_cuckoo_path(size_t start_bucket) {
            std::queue<std::shared_ptr<PathNode>> queue;
            std::unordered_set<size_t> visited;
            
            auto start_node = std::make_shared<PathNode>(start_bucket, -1, 0);
            queue.push(start_node);
            visited.insert(start_bucket);
            
            while (!queue.empty()) {
                auto current = queue.front();
                queue.pop();
                
                if (current->depth >= MAX_CUCKOO_PATH) continue;
                
                Bucket& bucket = *buckets_[current->bucket_idx];
                
                // Check if this bucket has an empty slot
                if (bucket.has_empty_slot()) {
                    // Found target! Reconstruct path from start to here
                ...
                // Try displacing each occupied slot to its alternative bucket
                for (int slot_idx = 0; slot_idx < SLOTS_PER_BUCKET; ++slot_idx) {
                    const auto& slot = bucket.slots[slot_idx];
                    if (slot.is_empty()) continue;
                    
                    auto* key_ptr = slot.key.load(std::memory_order_acquire);
                    if (!key_ptr) continue;
                    
                    auto [b1, b2] = get_bucket_indices(*key_ptr);
                    size_t alt_bucket = (b1 == current->bucket_idx) ? b2 : b1;
                    
                    if (visited.find(alt_bucket) == visited.end()) {
                        visited.insert(alt_bucket);
                        auto next_node = std::make_shared<PathNode>(alt_bucket, slot_idx, current->depth + 1, current);
    ...
    public:
        AetherCacheHashTable(size_t initial_bucket_count = INITIAL_BUCKET_COUNT)
            : memory_pool_(std::make_unique<memory::AdvancedMemoryPool>()) {
            bucket_count_.store(initial_bucket_count);
            buckets_.reserve(initial_bucket_count);
        ...
        bool set(const std::string& key, const std::string& value) {
            // **AETHERCACHE SET** - optimized cuckoo insertion with BFS pathfinding
            auto [b1, b2] = get_bucket_indices(key);
            Tag search_tag = make_tag(hash1_(key));
            
            // Try inserting in first bucket
            if (try_insert_in_bucket(b1, search_tag, key, value)) {
                return true;
            }
            
            // Try inserting in second bucket
            if (try_insert_in_bucket(b2, search_tag, key, value)) {
                return true;
            }
            
            // Both buckets full - use BFS to find cuckoo path
            return cuckoo_insert_with_path(b1, search_tag, key, value);
        }
        
        std::optional<std::string> get(const std::string& key) {
            // **AETHERCACHE GET** - optimistic lock-free lookup with version counters
            auto [b1, b2] = get_bucket_indices(key);
            Tag search_tag = make_tag(hash1_(key));
            
            // Try optimistic read from first bucket
            if (auto result = buckets_[b1]->optimistic_find(search_tag, key)) {
                return result;
            }
            
            // Try optimistic read from second bucket
            if (auto result = buckets_[b2]->optimistic_find(search_tag, key)) {
                return result;
            }
        ...
        bool del(const std::string& key) {
            // **AETHERCACHE DELETE** - fine-grained locking for removal
            auto [b1, b2] = get_bucket_indices(key);
            Tag search_tag = make_tag(hash1_(key));
            
            // Try deleting from first bucket
            if (try_delete_from_bucket(b1, search_tag, key)) {
                return true;
            }
            
            // Try deleting from second bucket
            return try_delete_from_bucket(b2, search_tag, key);
        }
        
        // **HELPER METHODS**
        
        // Try to insert key/value in specific bucket
        bool try_insert_in_bucket(size_t bucket_idx, Tag tag, const std::string& key, const std::string& value) {
            Bucket& bucket = *buckets_[bucket_idx];
            std::lock_guard<std::mutex> lock(bucket.write_lock);
            ...
            for (auto& slot : bucket.slots) {
                if (slot.matches(tag, key)) {
                    // Update existing value
                    auto* old_val = slot.value.load(std::memory_order_acquire);
                    auto* new_val = new std::string(value);
                    slot.value.store(new_val, std::memory_order_release);
                    delete old_val;
            ...
            // Find empty slot for new insertion
            int empty_slot = bucket.find_empty_slot();
            if (empty_slot >= 0) {
                auto& slot = bucket.slots[empty_slot];
                slot.key.store(new std::string(key), std::memory_order_release);
                slot.value.store(new std::string(value), std::memory_order_release);
                slot.tag.store(tag, std::memory_order_release);
                
                item_count_.fetch_add(1, std::memory_order_acq_rel);
        ...
        // Try to delete key from specific bucket
        bool try_delete_from_bucket(size_t bucket_idx, Tag tag, const std::string& key) {
            Bucket& bucket = *buckets_[bucket_idx];
            std::lock_guard<std::mutex> lock(bucket.write_lock);
            ...
            for (auto& slot : bucket.slots) {
                if (slot.matches(tag, key)) {
                    // Delete the entry
                    auto* old_key = slot.key.load(std::memory_order_acquire);
                    auto* old_val = slot.value.load(std::memory_order_acquire);
                    ...
                    item_count_.fetch_sub(1, std::memory_order_acq_rel);
        ...
        // **PROPER CUCKOO INSERTION** with BFS pathfinding and displacement execution
        bool cuckoo_insert_with_path(size_t start_bucket, Tag tag, const std::string& key, const std::string& value) {
            // **PHASE 1: BFS PATHFINDING (LOCK-FREE)**
            auto path = find_cuckoo_path(start_bucket);
            if (path.empty()) {
                // No path found - table is full or max depth reached
                return false;
            }
            
            // **PHASE 2: EXECUTE CUCKOO DISPLACEMENT CHAIN**
            // Collect all buckets in the path for ordered locking
            std::set<size_t> bucket_indices;
            for (const auto& node : path) {
                bucket_indices.insert(node.bucket_idx);
            }
            
            // Acquire locks in consistent order (by bucket index) to prevent deadlocks
            std::vector<std::unique_lock<std::mutex>> locks;
            std::vector<uint64_t> old_versions;
            
            for (size_t bucket_idx : bucket_indices) {
                locks.emplace_back(buckets_[bucket_idx]->write_lock);
                // Start write operation (increment to odd version)
                old_versions.push_back(buckets_[bucket_idx]->version.fetch_add(1, std::memory_order_acq_rel));
            }
            
            // **EXECUTE DISPLACEMENT CHAIN FROM END TO START**
            // Work backwards through the path to avoid overwriting data
            
            // First, insert our new key/value at the final empty slot
            const auto& final_node = path.back();
            Bucket& final_bucket = *buckets_[final_node.bucket_idx];
            int empty_slot = final_bucket.find_empty_slot();
            
            if (empty_slot < 0) {
                // Race condition - slot was filled, abort
                for (size_t i = 0; i < bucket_indices.size(); ++i) {
                    size_t bucket_idx = *std::next(bucket_indices.begin(), i);
                    buckets_[bucket_idx]->version.fetch_add(1, std::memory_order_acq_rel); // Complete write
                }
                return false;
            }
            
            // Store the item to be inserted
            std::string* current_key = new std::string(key);
            std::string* current_value = new std::string(value);
            Tag current_tag = tag;
            
            // Execute the displacement chain backwards
            for (int i = static_cast<int>(path.size()) - 2; i >= 0; --i) {
                const auto& node = path[i];
                Bucket& bucket = *buckets_[node.bucket_idx];
                
                if (node.slot_idx >= 0 && node.slot_idx < SLOTS_PER_BUCKET) {
                    auto& slot = bucket.slots[node.slot_idx];
                    
                    // Save the item being displaced
                    auto* displaced_key = slot.key.load(std::memory_order_acquire);
                    auto* displaced_value = slot.value.load(std::memory_order_acquire);
                    Tag displaced_tag = slot.tag.load(std::memory_order_acquire);
                    
                    // Move current item into this slot
                    slot.key.store(current_key, std::memory_order_release);
                    slot.value.store(current_value, std::memory_order_release);
                    slot.tag.store(current_tag, std::memory_order_release);
                    
                    // The displaced item becomes the current item for next iteration
                    current_key = displaced_key;
                    current_value = displaced_value;
                    current_tag = displaced_tag;
                }
            }
            
            // Finally, place the last displaced item in the empty slot
            auto& final_slot = final_bucket.slots[empty_slot];
            final_slot.key.store(current_key, std::memory_order_release);
            final_slot.value.store(current_value, std::memory_order_release);
            final_slot.tag.store(current_tag, std::memory_order_release);
            
            // Increment item count
            item_count_.fetch_add(1, std::memory_order_acq_rel);
            
            // Complete all write operations (increment to even versions)
            for (size_t i = 0; i < bucket_indices.size(); ++i) {
                size_t bucket_idx = *std::next(bucket_indices.begin(), i);
        ...
        size_t size() const {
            // **AETHERCACHE SIZE** - O(1) atomic size
        ...
        struct Stats {
            ...
            size_t bucket_count;
            double load_factor;
            size_t max_cuckoo_path;
        };
        
        Stats get_stats() const {
            Stats stats{};
            stats.total_entries = item_count_.load(std::memory_order_acquire);
            stats.bucket_count = bucket_count_.load(std::memory_order_acquire);
            stats.load_factor = static_cast<double>(stats.total_entries) / (stats.bucket_count * SLOTS_PER_BUCKET);
            stats.max_cuckoo_path = MAX_CUCKOO_PATH;
            return stats;
        }
    };
}
...
namespace cache {
    class HybridCache {
    private:
        std::unique_ptr<storage::AetherCacheHashTable> memory_cache_;
    ...
    public:
        HybridCache(size_t memory_shards, const std::string& ssd_path, size_t max_memory_mb = 256)
            ...
            memory_cache_ = std::make_unique<storage::AetherCacheHashTable>(1024);  // Initial size 1024 buckets for AetherCache
        ...
        struct Stats {
            storage::AetherCacheHashTable::Stats memory_stats;
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        // **LIBCUCKOO: ALWAYS PROCESS LOCALLY** - No connection migration
                        // Each core has its own libcuckoo hash table, process all commands locally
                        std::cout << "🔥 Core " << core_id_ << " processing command locally (fd=" << client_fd 
                                  << ") - LIBCUCKOO LOCAL PROCESSING" << std::endl;
                        processor_->submit_operation(command, key, value, client_fd);
                        
                        // Migration completely disabled for libcuckoo - all commands processed locally
                    } else {
                        processor_->submit_operation(command, key, value, client_fd);
                    }
                }
