...
private:
    ...
    void process_client_request(int client_fd) {
        // **PROFILING**: Time overall request flow to identify latency bottleneck
        ...
        auto after_recv = std::chrono::high_resolution_clock::now();
        ...
        if (commands.size() == 1 && !has_mget) {
            ...
            // **PROFILING**: Time complete single command path
            ...
            auto parse_time = std::chrono::duration_cast<std::chrono::microseconds>(after_parse - start_single).count();
            auto process_time = std::chrono::duration_cast<std::chrono::microseconds>(end_single - after_parse).count();
            auto total_time = std::chrono::duration_cast<std::chrono::microseconds>(end_single - start_single).count();
            
            // **PROFILING**: Log single command timing every 1000 commands
            static std::atomic<int> single_counter{0};
            if (single_counter.fetch_add(1) % 1000 == 0) {
                std::cout << "🔍 SINGLE CMD [" << single_counter.load() << "]: parse=" << parse_time 
                         << "μs, process=" << process_time << "μs, TOTAL=" << total_time << "μs" << std::endl;
            }
        } else {
            // **PIPELINE**: Use existing pipeline logic (includes MGET as multiple GETs)
            // **PROFILING**: Time pipeline command processing
            ...
            // **PROFILING**: Log pipeline timing every 1000 commands  
            static std::atomic<int> pipeline_counter{0};
            if (pipeline_counter.fetch_add(1) % 1000 == 0) {
                std::cout << "🔍 PIPELINE [" << pipeline_counter.load() << ", cmds=" << commands.size() 
                         << "]: TOTAL=" << pipeline_time << "μs" << std::endl;
            }
        }
        
        auto request_end = std::chrono::high_resolution_clock::now();
        
        // **PROFILING**: Log complete request flow timing every 1000 requests
        static std::atomic<int> request_counter{0};
        if (request_counter.fetch_add(1) % 1000 == 0) {
            auto recv_time = std::chrono::duration_cast<std::chrono::microseconds>(after_recv - request_start).count();
            auto processing_time = std::chrono::duration_cast<std::chrono::microseconds>(request_end - after_recv).count();
            auto total_time = std::chrono::duration_cast<std::chrono::microseconds>(request_end - request_start).count();
            
            std::cout << "🔍 REQUEST FLOW [" << request_counter.load() << "]: recv=" << recv_time 
                     << "μs, processing=" << processing_time << "μs, TOTAL=" << total_time 
                     << "μs, bytes=" << bytes_read << std::endl;
        }
