...
private:
    ...
    void process_received_command(int client_fd, const std::string& data) {
        ...
        // =========================
        // • Baseline single command: 693K ops/sec, excellent scaling (240 connections)
        // • Our pipeline commands: 7.67M QPS, zero connection errors
        // • Problem: Routing single commands through pipeline hurt performance
        //
        // HYBRID ARCHITECTURAL SOLUTION:
        // • Single commands (except MGET): Direct processing (baseline approach)
        // • Multiple commands + MGET: Pipeline processing (our stable approach)
        // • Performance: Get baseline's single command speed + our pipeline stability
        // • Correctness: Both paths use same cross-shard routing for consistency
        //
        // This combines the proven speed of baseline's direct processing with
        // the proven stability of our pipeline architecture.
        
        if (commands.size() == 1 && !has_mget) {
            // **SINGLE COMMAND OPTIMIZATION**: Direct processing without batching (baseline approach)
            auto parsed_cmd = parse_single_resp_command(commands[0]);
            if (!parsed_cmd.empty()) {
                process_single_command_direct(parsed_cmd, client_fd);
            }
        } else {
            // **PIPELINE**: Use existing pipeline logic for multiple commands and MGET (our stable approach)
