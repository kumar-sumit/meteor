...
namespace zero_copy {
    ...
    // **COMMAND-LEVEL TTL DETECTION** - for command processing layer
...
namespace storage {
    ...
    private:
        ...
        // **COMMAND-LEVEL TTL TRACKING** - separate from storage, checked at command processing level
        std::unordered_map<std::string, std::chrono::steady_clock::time_point> command_ttl_;
    ...
    public:
        ...
        bool del(const std::string& key) {
            // **TRUE SHARED-NOTHING**: Direct access to core's own data (no locking needed!)
            // **COMMAND-LEVEL TTL CLEANUP** - remove TTL tracking when key deleted
            command_ttl_.erase(key);
            return data_.erase(key) > 0;
        }
        
        // **COMMAND-LEVEL TTL METHODS** - for command processing layer only
        void set_command_ttl(const std::string& key, std::chrono::seconds ttl_seconds) {
            command_ttl_[key] = std::chrono::steady_clock::now() + ttl_seconds;
        }
        
        bool is_command_expired(const std::string& key) const {
            auto it = command_ttl_.find(key);
            if (it == command_ttl_.end()) {
                return false; // No TTL set
            }
            return std::chrono::steady_clock::now() >= it->second;
        }
        
        int get_command_ttl(const std::string& key) const {
            // Check if key exists in data first (baseline check)
...
namespace cache {
    ...
    public:
        ...
        // **COMMAND-LEVEL TTL METHODS** - for command processing layer
        void set_command_ttl(const std::string& key, std::chrono::seconds ttl_seconds) {
            memory_cache_->set_command_ttl(key, ttl_seconds);
        }
        
        bool is_command_expired(const std::string& key) const {
            return memory_cache_->is_command_expired(key);
        }
        
        int get_command_ttl(const std::string& key) const {
            return memory_cache_->get_command_ttl(key);
        }
...
public:
    ...
    ResponseInfo execute_single_operation_optimized(const BatchOperation& op) {
        ...
        // **FAST PATH: GET command** (most common in 1:3 ratio) - with command-level TTL
        if (zero_copy::is_get_command(op.command)) {
            // **COMMAND-LEVEL TTL CHECK FIRST** - before touching baseline storage
            if (cache_->is_command_expired(std::string(op.key))) {
                // Key expired at command level - return null without calling storage
                return ResponseInfo(ultra_optimized::ResponsePool::NULL_RESPONSE,
                                  ultra_optimized::ResponsePool::NULL_SIZE);
            }
            
            // **BASELINE STORAGE GET - COMPLETELY UNCHANGED**
        ...
        // **COMMAND-LEVEL TTL COMMAND** - separate from baseline commands
        if (zero_copy::is_ttl_command(op.command)) {
            int remaining = cache_->get_command_ttl(std::string(op.key));
            static thread_local char ttl_buffer[32];
            int len = snprintf(ttl_buffer, sizeof(ttl_buffer), ":%d\r\n", remaining);
