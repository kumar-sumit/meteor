...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **TRUE UNIFIED APPROACH**: Treat single commands as single-item pipelines
            // Route them through the SAME pipeline processing function that works correctly
            std::vector<std::vector<std::string>> parsed_commands;
            ...
            if (!parsed_commands.empty()) {
                // **KEY INSIGHT**: Use the EXACT same pipeline processing that works for multiple commands
                // This eliminates dual code paths and ensures identical behavior
                processor_->process_pipeline_batch(client_fd, parsed_commands, core_id_, total_shards_);
            }
        }
    }
