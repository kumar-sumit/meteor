// **PHASE 8 STEP 3: ADVANCED CONTENTION MITIGATION FOR SUB-1MS P99**
//
// Building on Step 2's success (140K RPS, 0.39ms P50, 0.69ms P95), now eliminating final P99 spikes:
// **MISSION**: Advanced contention mitigation to achieve true sub-1ms P99 latency
//
// PHASE 3 - Advanced Contention Mitigation:
// 1. **Enhanced Cuckoo Hashing**: Improved key distribution to reduce hotspot formation
// 2. **Advanced SCA Tuning**: Adaptive thresholds and intelligent backoff strategies
// 3. **Memory Pool Optimization**: Pre-allocated segments to eliminate expansion delays
// 4. **NUMA-Aware Placement**: Optimize atomic operations for cache coherence
// 5. **Hotspot Detection**: Proactive hotspot identification and mitigation
// 6. **Load Balancing**: Dynamic load redistribution for extreme scenarios
//
// Target Performance: 140K+ RPS with P99 < 1ms (eliminate 120ms spikes)
// Architecture: Advanced contention mitigation on top of VLL + SCA foundation
// Research: Based on advanced hash table optimization and NUMA best practices
//
// Previous Results: 139,580 RPS, P50: 0.39ms, P95: 0.69ms, P99: 120ms ⚠️
// Target Results:   140K+ RPS,     P50: <0.5ms,  P95: <1ms,   P99: <1ms ✅
...
// **PHASE 8 STEP 1: TRUE DRAGONFLY DASHTABLE IMPLEMENTATION**
namespace storage {
    ...
    private:
        ...
        // **VLL-Enabled DragonflyDB Slot with 2Q Cache Policy + SCA**
        struct alignas(64) Slot {  // Cache-line aligned for performance
            ...
            // **VLL (Very Lightweight Locking) Counters**
            mutable std::atomic<uint32_t> read_count{0};      // Active readers
            mutable std::atomic<uint32_t> write_count{0};     // Active writers (0 or 1)
            mutable std::atomic<uint32_t> contention_count{0}; // SCA: Track contention events
            
            // **PHASE 3: Advanced Contention Mitigation**
            mutable std::atomic<uint32_t> access_frequency{0}; // Hotspot detection
            mutable std::atomic<uint64_t> total_wait_time{0};  // Cumulative wait time for adaptive SCA
            mutable std::atomic<uint32_t> failed_attempts{0};  // Failed lock attempts for load balancing
            
            Slot() = default;
            
            // **VLL: Try to acquire read lock**
            bool try_read_lock() const {
                uint32_t writers = write_count.load(std::memory_order_acquire);
                if (writers > 0) return false;  // Writer active
                
                read_count.fetch_add(1, std::memory_order_acq_rel);
                
                // Double-check no writer started while we were acquiring
                if (write_count.load(std::memory_order_acquire) > 0) {
                    read_count.fetch_sub(1, std::memory_order_acq_rel);
                    contention_count.fetch_add(1, std::memory_order_relaxed);  // SCA: Track contention
                    return false;
                }
                return true;
            }
            
            // **VLL: Release read lock**
            void release_read_lock() const {
                read_count.fetch_sub(1, std::memory_order_acq_rel);
            }
            
            // **VLL: Try to acquire write lock**
            bool try_write_lock() const {
                uint32_t expected_writers = 0;
                if (!write_count.compare_exchange_strong(expected_writers, 1, std::memory_order_acq_rel)) {
                    contention_count.fetch_add(1, std::memory_order_relaxed);  // SCA: Track contention
                    return false;  // Another writer active
                }
                
                // Wait for all readers to finish (spin briefly, then backoff)
                uint32_t readers = read_count.load(std::memory_order_acquire);
                if (readers > 0) {
                    write_count.store(0, std::memory_order_release);  // Release write lock
                    contention_count.fetch_add(1, std::memory_order_relaxed);  // SCA: Track contention
                    return false;
                }
                return true;
            ...
            // **VLL: Release write lock**
            void release_write_lock() const {
                write_count.store(0, std::memory_order_release);
            }
            
            // **PHASE 3: Advanced SCA with Adaptive Thresholds**
            bool is_high_contention() const {
                uint32_t contentions = contention_count.load(std::memory_order_relaxed);
                uint32_t accesses = access_frequency.load(std::memory_order_relaxed);
                
                // Adaptive threshold: more accesses = higher tolerance for contention
                uint32_t threshold = std::max(5U, std::min(50U, accesses / 10));
                return contentions > threshold;
            }
            
            // **PHASE 3: Hotspot Detection**
            bool is_hotspot() const {
                uint32_t accesses = access_frequency.load(std::memory_order_relaxed);
                uint32_t failed = failed_attempts.load(std::memory_order_relaxed);
                
                // Hotspot: high access frequency with significant failures
                return accesses > 100 && failed > (accesses / 4);  // >25% failure rate
            }
            
            // **PHASE 3: Advanced Backoff Strategy**
            uint32_t calculate_backoff_us(uint32_t attempt) const {
                uint64_t avg_wait = total_wait_time.load(std::memory_order_relaxed);
                uint32_t contentions = contention_count.load(std::memory_order_relaxed);
                
                // Base exponential backoff
                uint32_t base_backoff = 1U << std::min(attempt / 5, 8U);  // 1μs to 256μs
                
                // Adaptive multiplier based on historical wait times
                if (contentions > 0) {
                    uint32_t avg_wait_us = static_cast<uint32_t>(avg_wait / contentions);
                    uint32_t multiplier = std::min(4U, avg_wait_us / 100);  // Max 4x multiplier
                    base_backoff *= (1 + multiplier);
                }
                
                // Cap at 1ms for extreme cases
                return std::min(base_backoff, 1000U);
            }
            
            // **PHASE 3: Track Access for Hotspot Detection**
            void track_access() const {
                access_frequency.fetch_add(1, std::memory_order_relaxed);
            }
            
            // **PHASE 3: Track Failed Attempt**
            void track_failed_attempt(uint64_t wait_time_us) const {
                failed_attempts.fetch_add(1, std::memory_order_relaxed);
                total_wait_time.fetch_add(wait_time_us, std::memory_order_relaxed);
            }
            
            // **2Q Cache Policy: Promote slot on access (VLL-aware)**
            void promote() {
        ...
        struct alignas(64) Bucket {  // Cache-line aligned
            ...
            // **VLL-Enabled Cuckoo Hashing: Insert with dual hash probing**
            bool insert(const K& key, const V& value, uint32_t primary_hash, uint32_t secondary_hash, bool is_stash = false) {
                // **PHASE 3: Enhanced key lookup with hotspot detection**
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].occupied.load(std::memory_order_acquire) && slots[i].key == key) {
                        slots[i].track_access();  // Track access for hotspot detection
                        
                        // **PHASE 3: Advanced VLL with adaptive backoff**
                        auto start_time = std::chrono::steady_clock::now();
                        uint32_t attempts = 0;
                        
                        while (!slots[i].try_write_lock() && attempts < 200) {  // Increased max attempts
                            attempts++;
                            
                            // **PHASE 3: Advanced adaptive backoff**
                            uint32_t backoff_us = slots[i].calculate_backoff_us(attempts);
                            std::this_thread::sleep_for(std::chrono::microseconds(backoff_us));
                        }
                        
                        if (attempts >= 200) {
                            // **PHASE 3: Track failed attempt with wait time**
                            auto end_time = std::chrono::steady_clock::now();
                            uint64_t wait_time_us = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
                            slots[i].track_failed_attempt(wait_time_us);
                            return false;  // Failed to acquire lock
                        }
                        
                        slots[i].value = value;
                        slots[i].promote();  // 2Q: promote on access
                        slots[i].release_write_lock();  // VLL: Release write lock
                        return true;
                    }
                }
                
                // **PHASE 3: Enhanced slot insertion with load balancing**
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (!slots[i].occupied.load(std::memory_order_acquire)) {
                        // **PHASE 3: Advanced VLL with adaptive backoff for insertion**
                        auto start_time = std::chrono::steady_clock::now();
                        uint32_t attempts = 0;
                        
                        while (!slots[i].try_write_lock() && attempts < 150) {  // Balanced max attempts
                            attempts++;
                        ...
                        if (attempts >= 150) {
                            // **PHASE 3: Track failed attempt and try next slot**
                            auto end_time = std::chrono::steady_clock::now();
                            uint64_t wait_time_us = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
                            slots[i].track_failed_attempt(wait_time_us);
                        ...
                        // Double-check slot is still empty
                        if (slots[i].occupied.load(std::memory_order_acquire)) {
                        ...
                        slots[i].track_access();  // Track initial access
                        slots[i].release_write_lock();  // VLL: Release write lock
            ...
            // **PHASE 3: Enhanced 2Q Cache Policy Find with Advanced SCA**
            bool find(const K& key, V& result) {
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (slots[i].occupied.load(std::memory_order_acquire) && slots[i].key == key) {
                        slots[i].track_access();  // Track access for hotspot detection
                        
                        // **PHASE 3: Advanced VLL with adaptive backoff for reads**
                        auto start_time = std::chrono::steady_clock::now();
                        uint32_t attempts = 0;
                        
                        while (!slots[i].try_read_lock() && attempts < 100) {  // Increased attempts for reads
                            attempts++;
                            
                            // **PHASE 3: Advanced adaptive backoff (lighter for reads)**
                            uint32_t backoff_us = std::max(1U, slots[i].calculate_backoff_us(attempts) / 4);  // 1/4 backoff for reads
                            std::this_thread::sleep_for(std::chrono::microseconds(backoff_us));
                        }
                        
                        if (attempts >= 100) {
                            // **PHASE 3: Track failed read attempt**
                            auto end_time = std::chrono::steady_clock::now();
                            uint64_t wait_time_us = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
                            slots[i].track_failed_attempt(wait_time_us);
                            continue;  // Try next slot
                        ...
                        // Double-check key still matches after acquiring lock
                        if (slots[i].occupied.load(std::memory_order_acquire) && slots[i].key == key) {
                            result = slots[i].value;
                            slots[i].promote();  // 2Q: promote on access
                            slots[i].release_read_lock();  // VLL: Release read lock
                            return true;
                        }
        ...
        // **VLL-Enabled DragonflyDB Segment with Stash Buckets**
        struct alignas(64) Segment {
            std::array<Bucket, TOTAL_BUCKETS> buckets;
            std::atomic<size_t> total_entries{0};
            // **VLL: No segment-level mutex - all locking is per-slot**
            
            // **PHASE 3: Enhanced Cuckoo Hash Routing with Hotspot Mitigation**
            std::pair<size_t, size_t> get_home_buckets(uint32_t primary_hash, uint32_t secondary_hash) const {
                // **PHASE 3: Improved hash distribution to reduce clustering**
                // Use different modulo bases to spread load more evenly
                size_t bucket1 = (primary_hash * 0x9E3779B9) % REGULAR_BUCKETS;  // Golden ratio multiplier
                size_t bucket2 = (secondary_hash * 0x85EBCA6B) % REGULAR_BUCKETS; // Different prime multiplier
                
                // Ensure buckets are different to avoid degenerate cases
                if (bucket1 == bucket2) {
            ...
            // **PHASE 3: Advanced Hash Generation for Better Distribution**
            std::pair<uint32_t, uint32_t> generate_enhanced_hashes(const K& key) const {
                std::hash<K> hasher;
                uint64_t base_hash = hasher(key);
                
                // **PHASE 3: Use different hash functions to minimize correlation**
                uint32_t primary_hash = static_cast<uint32_t>(base_hash);
                uint32_t secondary_hash = static_cast<uint32_t>((base_hash >> 32) ^ (base_hash * 0x9E3779B9));
                
                // **PHASE 3: Additional mixing to reduce hotspots**
                primary_hash ^= primary_hash >> 16;
                secondary_hash ^= secondary_hash >> 16;
                
                return {primary_hash, secondary_hash};
            }
            
            // **PHASE 3: Enhanced VLL Insert with Advanced Cuckoo Hashing**
            bool insert(const K& key, const V& value) {
                // **VLL: No segment-level lock - buckets handle their own VLL locking**
                
                // **PHASE 3: Use enhanced hash generation for better distribution**
                auto [primary_hash, secondary_hash] = generate_enhanced_hashes(key);
                auto [bucket1, bucket2] = get_home_buckets(primary_hash, secondary_hash);
                
                // **PHASE 3: Smart bucket selection with hotspot avoidance**
                // Check if primary bucket is a hotspot and try secondary first if so
                bool primary_is_hotspot = is_bucket_hotspot(bucket1);
                bool secondary_is_hotspot = is_bucket_hotspot(bucket2);
                
                // Try less contentious bucket first
                if (primary_is_hotspot && !secondary_is_hotspot) {
                    // Try secondary first
                ...
                // **PHASE 3: Enhanced stash selection with load balancing**
                // Try stash buckets in order of least contention
                std::vector<size_t> stash_order;
                for (size_t i = REGULAR_BUCKETS; i < TOTAL_BUCKETS; ++i) {
                    stash_order.push_back(i);
                }
                
                // Sort stash buckets by contention level (least contentious first)
                std::sort(stash_order.begin(), stash_order.end(), [this](size_t a, size_t b) {
                    return get_bucket_contention_score(a) < get_bucket_contention_score(b);
                });
                
                for (size_t bucket_idx : stash_order) {
                    if (buckets[bucket_idx].insert(key, value, primary_hash, secondary_hash, true)) {
                        total_entries.fetch_add(1, std::memory_order_relaxed);
                        return true;
                    }
                }
            ...
            // **PHASE 3: Hotspot Detection for Buckets**
            bool is_bucket_hotspot(size_t bucket_idx) const {
                if (bucket_idx >= TOTAL_BUCKETS) return false;
                
                // Check if any slot in the bucket is a hotspot
                const auto& bucket = buckets[bucket_idx];
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (bucket.slots[i].occupied.load(std::memory_order_acquire) && 
                        bucket.slots[i].is_hotspot()) {
                        return true;
                    }
                }
                return false;
            ...
            // **PHASE 3: Calculate Bucket Contention Score**
            uint32_t get_bucket_contention_score(size_t bucket_idx) const {
                if (bucket_idx >= TOTAL_BUCKETS) return UINT32_MAX;
                ...
                for (size_t i = 0; i < SLOTS_PER_BUCKET; ++i) {
                    if (bucket.slots[i].occupied.load(std::memory_order_acquire)) {
                        total_score += bucket.slots[i].contention_count.load(std::memory_order_relaxed);
                        total_score += bucket.slots[i].failed_attempts.load(std::memory_order_relaxed) * 2; // Weight failures more
                    }
                }
            ...
            // **PHASE 3: Enhanced VLL Find with Hotspot-Aware Routing**
            bool find(const K& key, V& result) {
                // **VLL: No segment-level lock - buckets handle their own VLL locking**
                
                // **PHASE 3: Use enhanced hash generation for consistency**
                auto [primary_hash, secondary_hash] = generate_enhanced_hashes(key);
                auto [bucket1, bucket2] = get_home_buckets(primary_hash, secondary_hash);
                
                // **PHASE 3: Smart bucket search order based on contention**
                bool primary_is_hotspot = is_bucket_hotspot(bucket1);
                bool secondary_is_hotspot = is_bucket_hotspot(bucket2);
                
                // Search less contentious bucket first for better performance
                if (primary_is_hotspot && !secondary_is_hotspot) {
                    if (buckets[bucket2].find(key, result) || buckets[bucket1].find(key, result)) {
                        return true;
                    }
                } else {
                ...
                // **PHASE 3: Enhanced stash search with contention ordering**
                std::vector<size_t> stash_order;
                for (size_t i = REGULAR_BUCKETS; i < TOTAL_BUCKETS; ++i) {
                    stash_order.push_back(i);
                }
                
                // Sort by contention (least contentious first)
                std::sort(stash_order.begin(), stash_order.end(), [this](size_t a, size_t b) {
                    return get_bucket_contention_score(a) < get_bucket_contention_score(b);
                });
                
                for (size_t bucket_idx : stash_order) {
                    if (buckets[bucket_idx].find(key, result)) {
                        return true;
                    }
                }
                return false;
        ...
        // **VLL-Enabled DragonflyDB Dynamic Segment Expansion**
        void expand_segments() {
            // **VLL: No global mutex - expansion is lock-free**
    ...
    public:
        ...
        // **VLL-Enabled DragonflyDB Find with 2Q Promotion**
        ...
        Statistics get_statistics() const {
            // **VLL: No global mutex - just read atomic counters**
            size_t segments = segment_count_.load(std::memory_order_relaxed);
            size_t entries = total_entries_.load(std::memory_order_relaxed);
            size_t capacity = segments * TOTAL_BUCKETS * SLOTS_PER_BUCKET;
    ...
    private:
        struct Shard {
            DragonflyDashtable<std::string, std::string> data;  // **VLL-ENABLED: DragonflyDashtable with per-slot VLL**
            // **VLL: No shard-level mutex - DragonflyDashtable handles all locking internally**
            std::atomic<size_t> operation_count{0};
            std::atomic<size_t> lock_contention{0};  // Now tracks VLL contention events
        };
    ...
    public:
        ...
        bool set(const std::string& key, const std::string& value) {
            ...
            // **VLL: No shard-level mutex - DragonflyDashtable handles all locking internally**
            bool result = shard.data.insert(key, value);  // **VLL-ENABLED: DragonflyDashtable with per-slot VLL**
        ...
        std::optional<std::string> get(const std::string& key) {
            ...
            // **VLL: No shard-level mutex - DragonflyDashtable handles all locking internally**
            auto result = shard.data.find(key);  // **VLL-ENABLED: DragonflyDashtable with per-slot VLL**
            if (result) {
                shard.operation_count.fetch_add(1);
                return result;
            }
        ...
        bool del(const std::string& key) {
            ...
            // **VLL: No shard-level mutex - DragonflyDashtable handles all locking internally**
            // **NOTE**: DragonflyDashtable doesn't implement delete yet (Phase 2 focus on insert/find with VLL)
            bool erased = false;  // **TODO**: Implement delete in Phase 3 with VLL
            if (erased) {
                shard.operation_count.fetch_add(1);
            }
            return erased;
        ...
        size_t size() const {
            ...
            for (const auto& shard_ptr : shards_) {
                ...
                // **VLL: No shard-level mutex - DragonflyDashtable handles all locking internally**
                auto stats = shard.data.get_statistics();  // **VLL-ENABLED: DragonflyDashtable with per-slot VLL**
        ...
        Stats get_stats() const {
            ...
            for (const auto& shard_ptr : shards_) {
                ...
                auto dashtable_stats = shard.data.get_statistics();  // **VLL-ENABLED: DragonflyDashtable with per-slot VLL**
                stats.total_entries += dashtable_stats.total_entries;
                stats.total_lock_contention += shard.lock_contention.load();  // Now tracks VLL contention events
            }
...
private:
    ...
    // **PHASE 8 STEP 1: DragonflyDB-Style Pipeline Processing - Process ALL commands locally**
    std::string execute_single_operation(const BatchOperation& op) {
        ...
        if (cmd_upper == "SET") {
            // Use HybridCache which now contains DragonflyDB Dashtable - process all keys locally
            bool success = cache_->set(op.key, op.value);
            return success ? "+OK\r\n" : "-ERR failed to set\r\n";
        }
        
        if (cmd_upper == "GET") {
            // HybridCache now uses DragonflyDB Dashtable internally
        ...
        if (cmd_upper == "DEL") {
            // Delete using HybridCache which now contains DragonflyDB Dashtable
        ...
        // **PHASE 8 STEP 1: Enhanced Command Support**
        if (cmd_upper == "INFO") {
            auto cache_stats = cache_->get_stats();
            std::ostringstream info;
            info << "# DragonflyDB Dashtable Stats (via HybridCache)\r\n";
            info << "memory_utilization:" << cache_stats.memory_utilization << "%\r\n";
            info << "current_memory_usage:" << cache_stats.current_memory_usage << "\r\n";
            info << "max_memory_size:" << cache_stats.max_memory_size << "\r\n";
            info << "access_patterns_tracked:" << cache_stats.access_patterns_tracked << "\r\n";
            info << "memory_total_entries:" << cache_stats.memory_stats.total_entries << "\r\n";
            std::string info_str = info.str();
            return "$" + std::to_string(info_str.length()) + "\r\n" + info_str + "\r\n";
        }
...
private:
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **PHASE 8 STEP 1: PIPELINE CONNECTION AFFINITY FIX (from Phase 7 Step 3)**
            // Always process pipeline commands locally to prevent connection migration issues
            // This eliminates the 99% pipeline performance regression - DragonflyDB style local processing
