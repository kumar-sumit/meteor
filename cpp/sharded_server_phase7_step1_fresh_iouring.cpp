// **PHASE 7 STEP 1: IO_URING HIGH-PERFORMANCE ASYNC I/O IMPLEMENTATION**
// Building on Phase 6 Step 3 (working pipeline batch processing) with:
// - io_uring async I/O for 3x performance improvement
// - Batched async operations (accept/recv/send)
// - Zero-copy buffer management
// - Advanced completion queue processing
// - Reduced syscall overhead and context switching
// Target: 2.4M+ RPS (3x improvement over Phase 6 Step 3's 800K RPS)
// Preserving all previous optimizations: pipeline processing, multi-accept, CPU affinity
...
// **PHASE 7 STEP 1: io_uring includes**
#include <liburing.h>
#include <sys/mman.h>
...
private:
    ...
    // **PHASE 7 STEP 1: io_uring response sender**
...
public:
    ...
    DirectOperationProcessor(size_t memory_shards, const std::string& ssd_path, size_t max_memory_mb = 256, 
                           monitoring::CoreMetrics* metrics = nullptr, ResponseSender* sender = nullptr) 
        : metrics_(metrics), response_sender_(sender), hot_key_cache_(1024) {
...
private:
    ...
    void process_pending_batch() {
        ...
        for (auto& op : pending_operations_) {
            ...
            // **PHASE 7 STEP 1: Use io_uring send operation**
            if (response_sender_) {
...
bool DirectOperationProcessor::execute_pipeline_batch(std::shared_ptr<ConnectionState> conn_state) {
    ...
    // **PHASE 7 STEP 1: Single io_uring send for entire pipeline response**
    bool success = false;
    if (!conn_state->response_buffer.empty()) {
        if (response_sender_) {
            response_sender_->send_response(conn_state->client_fd, conn_state->response_buffer);
            success = true; // Assume success for async operation
        } else {
...
// **PHASE 7 STEP 1: io_uring operation structures**
#if HAS_IO_URING
...
struct IoUringOperation {
    ...
    IoUringOperation(IoUringOpType op_type, int fd = -1)
        : type(op_type), client_fd(fd), buffer(nullptr), buffer_size(0),
          start_time(std::chrono::steady_clock::now()), accept_len(sizeof(accept_addr)) {}
};

// **PHASE 7 STEP 1: Zero-copy buffer pool for io_uring**
...
public:
    IoUringBufferPool() {
        ...
        for (size_t i = 0; i < POOL_SIZE; ++i) {
            ...
            available_buffers_.push(raw_ptr);
        }
        std::cout << "🚀 io_uring buffer pool initialized with " << POOL_SIZE 
                  << " buffers of " << BUFFER_SIZE << " bytes each" << std::endl;
    }
...
private:
    ...
    // **PHASE 7 STEP 1: io_uring event loop**
#if HAS_IO_URING
    ...
    std::mutex uring_mutex_;
    std::atomic<uint64_t> operation_counter_{0};
    
    // **PHASE 7 STEP 1 FIX: Deferred submission queue for recv operations**
    std::queue<int> pending_recv_submissions_;
...
public:
    CoreThread(int core_id, size_t num_cores, size_t total_shards, const std::string& ssd_path, size_t memory_mb,
        ...
        processor_ = std::make_unique<DirectOperationProcessor>(
            owned_shards_.size(), core_ssd_path, memory_mb / num_cores_, metrics_, this);
        ...
        // **PHASE 7 STEP 1: Initialize io_uring**
#if HAS_IO_URING
        if (io_uring_queue_init(QUEUE_DEPTH, &ring_, 0) < 0) {
            std::cerr << "⚠️  Failed to initialize io_uring for core " << core_id_ 
                      << ", falling back to epoll" << std::endl;
        } else {
            buffer_pool_ = std::make_unique<IoUringBufferPool>();
            std::cout << "🚀 Core " << core_id_ << " io_uring initialized with queue depth " 
    ...
    ~CoreThread() {
        ...
        // **PHASE 7 STEP 1: Cleanup io_uring**
#if HAS_IO_URING
        if (buffer_pool_) {
    ...
    // **PHASE 7 STEP 1: Enhanced dedicated accept with io_uring support**
    void run_dedicated_accept() {
        std::cout << "🔌 Core " << core_id_ << " dedicated accept thread started" << std::endl;
        
#if HAS_IO_URING
        if (buffer_pool_) {
            // Use io_uring for accept operations
            std::cout << "🚀 Core " << core_id_ << " using io_uring for accept operations" << std::endl;
            submit_accept_operation(); // Submit initial accept
            
            // **PHASE 7 STEP 1: Keep accept thread alive for io_uring**
            // The accept thread should stay alive to monitor the dedicated socket
            // even though actual accepts are handled in the main event loop
            while (running_.load()) {
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
            }
            std::cout << "🔌 Core " << core_id_ << " io_uring accept thread stopped" << std::endl;
            return;
        }
#endif
        
        // Fallback to traditional accept loop
        while (running_.load()) {
...
private:
    ...
    void run() {
        ...
        while (running_.load()) {
            ...
            // **PHASE 7 STEP 1: Use io_uring when available, fallback to epoll**
#if HAS_IO_URING
            if (buffer_pool_) {
                try {
                    process_events_iouring();
                } catch (const std::exception& e) {
                    std::cerr << "🚨 Core " << core_id_ << " io_uring error: " << e.what() << ", falling back to epoll" << std::endl;
#ifdef HAS_LINUX_EPOLL
                    process_events_linux();
#endif
                }
            } else {
                std::cerr << "🔄 Core " << core_id_ << " using epoll (no io_uring buffer pool)" << std::endl;
#ifdef HAS_LINUX_EPOLL
                process_events_linux();
#endif
            }
#elif defined(HAS_LINUX_EPOLL)
            process_events_linux();
#elif defined(HAS_MACOS_KQUEUE)
    ...
    // **PHASE 7 STEP 1: io_uring event processing**
#if HAS_IO_URING
    void process_events_iouring() {
        // **PHASE 7 STEP 1 FIX: Always process deferred operations first**
        std::cout << "🔍 Core " << core_id_ << " checking for pending recv submissions..." << std::endl;
        process_pending_recv_submissions();
        
        // **PHASE 7 STEP 1: Submit any pending operations**
        int submitted = io_uring_submit(&ring_);
        
        // **PHASE 7 STEP 1: Process completions with very short timeout**
        struct __kernel_timespec ts;
        ts.tv_sec = 0;
        ts.tv_nsec = 100000; // 0.1ms timeout - very short to keep event loop responsive
        ...
        if (ret < 0) {
            ...
            // **PHASE 7 STEP 1: Continue processing even on timeout**
        } else {
            // **PHASE 7 STEP 1: Process all available completions efficiently**
            unsigned head;
            unsigned count = 0;
            io_uring_for_each_cqe(&ring_, head, cqe) {
                process_iouring_completion(cqe);
                count++;
            }
            
            if (count > 0) {
                io_uring_cq_advance(&ring_, count);
                std::cout << "🔄 Core " << core_id_ << " processed " << count << " io_uring completions" << std::endl;
            }
        }
        
        // **PHASE 7 STEP 1: Always flush operations from processor**
        processor_->flush_pending_operations();
        
        // **PHASE 7 STEP 1: Submit any new operations that were queued during processing**
        int new_submitted = io_uring_submit(&ring_);
        if (new_submitted > 0) {
            std::cout << "🔄 Core " << core_id_ << " submitted " << new_submitted << " new operations" << std::endl;
        }
    }
    
    // **PHASE 7 STEP 1 FIX: Process pending recv submissions in batches**
    void process_pending_recv_submissions() {
        std::queue<int> local_queue;
        size_t queue_size = 0;
        
        // Move pending submissions to local queue to minimize lock time
        {
            std::lock_guard<std::mutex> lock(recv_queue_mutex_);
            queue_size = pending_recv_submissions_.size();
            if (queue_size > 0) {
                std::cout << "🔍 Core " << core_id_ << " processing " << queue_size << " pending recv submissions" << std::endl;
                local_queue = std::move(pending_recv_submissions_);
                pending_recv_submissions_ = std::queue<int>{}; // Reset the queue
            }
        }
        ...
        // Process all pending recv submissions
        size_t processed = 0;
        while (!local_queue.empty()) {
            int client_fd = local_queue.front();
            local_queue.pop();
            processed++;
            
            std::cout << "🔄 Core " << core_id_ << " processing deferred recv submission " << processed << "/" << queue_size << " for fd=" << client_fd << std::endl;
            submit_recv_operation(client_fd);
        }
        
        std::cout << "✅ Core " << core_id_ << " completed processing " << processed << " deferred recv submissions" << std::endl;
    }
    
    void process_iouring_completion(struct io_uring_cqe *cqe) {
        uint64_t op_id = reinterpret_cast<uint64_t>(io_uring_cqe_get_data(cqe));
        int result = cqe->res;
        
        std::lock_guard<std::mutex> lock(uring_mutex_);
        auto it = pending_operations_.find(op_id);
        if (it == pending_operations_.end()) {
            return; // Operation not found, might have been cancelled
        }
        
        auto& op = it->second;
        
        switch (op->type) {
            case IoUringOpType::ACCEPT: {
                if (result > 0) {
                    int client_fd = result;
                    std::cout << "✅ Core " << core_id_ << " accepted connection fd=" << client_fd 
                              << " via io_uring" << std::endl;
                    
                    // **PHASE 7 STEP 1: Process accept completion immediately**
                    add_client_connection_iouring(client_fd);
                    
                    // **PHASE 7 STEP 1: Submit next accept with slight delay to avoid resource contention**
                    std::this_thread::sleep_for(std::chrono::microseconds(10));
                    submit_accept_operation();
                    
                    std::cout << "🔄 Core " << core_id_ << " accept completion processed for fd=" << client_fd << std::endl;
                } else if (result < 0) {
                    std::cerr << "❌ Core " << core_id_ << " accept failed: " << strerror(-result) << std::endl;
                    // Still submit next accept to keep accepting
                    submit_accept_operation();
                }
                break;
            }
            case IoUringOpType::RECV: {
                if (result > 0) {
                    std::string data(static_cast<char*>(op->buffer), result);
                    std::cout << "📥 Core " << core_id_ << " received " << result << " bytes from fd=" 
                              << op->client_fd << " data: " << data << std::endl;
                    parse_and_submit_commands(data, op->client_fd);
                    
                    // Return buffer to pool
                    buffer_pool_->return_buffer(static_cast<char*>(op->buffer));
                    
                    // Submit next recv for this client
                    submit_recv_operation(op->client_fd);
                } else if (result == 0 || result == -ECONNRESET) {
                    // Client disconnected
                    std::cout << "🔌 Core " << core_id_ << " client fd=" << op->client_fd 
                              << " disconnected (result=" << result << ")" << std::endl;
                    buffer_pool_->return_buffer(static_cast<char*>(op->buffer));
                    close_client_connection(op->client_fd);
                } else if (result < 0) {
                    std::cerr << "❌ Core " << core_id_ << " recv failed for fd=" << op->client_fd 
            ...
            case IoUringOpType::SEND: {
                ...
                if (result > 0) {
                    std::cout << "📤 Core " << core_id_ << " sent " << result << " bytes to fd=" 
                              << op->client_fd << std::endl;
                } else if (result < 0) {
                    std::cerr << "❌ Core " << core_id_ << " send failed for fd=" << op->client_fd 
                              << ": " << strerror(-result) << std::endl;
                }
                break;
            }
            case IoUringOpType::CLOSE: {
                // Close completed
                break;
            }
        }
    ...
    void submit_accept_operation() {
        ...
        auto op = std::make_unique<IoUringOperation>(IoUringOpType::ACCEPT);
        
        // **PHASE 7 STEP 1: Use persistent storage in the operation**
        ...
        {
            std::lock_guard<std::mutex> lock(uring_mutex_);
            pending_operations_[op_id] = std::move(op);
        }
        
        // **PHASE 7 STEP 1: Submit immediately to ensure operation is queued**
        io_uring_submit(&ring_);
        std::cout << "🔄 Core " << core_id_ << " submitted accept operation (op_id=" << op_id << ")" << std::endl;
    }
    
    void submit_recv_operation(int client_fd) {
        // **PHASE 7 STEP 1: Enhanced recv submission with retry logic**
        const int MAX_RETRIES = 3;
        
        std::cout << "🔍 Core " << core_id_ << " submit_recv_operation called for fd=" << client_fd << std::endl;
        
        for (int retry = 0; retry < MAX_RETRIES; retry++) {
            std::cout << "🔄 Core " << core_id_ << " recv submission attempt " << (retry + 1) << "/" << MAX_RETRIES << " for fd=" << client_fd << std::endl;
            
            if (!buffer_pool_) {
                std::cerr << "❌ Core " << core_id_ << " submit_recv_operation: no buffer pool!" << std::endl;
                return;
            }
            
            // **FIX: Ensure we have space in the submission queue**
            struct io_uring_sqe *sqe = io_uring_get_sqe(&ring_);
            if (!sqe) {
                if (retry < MAX_RETRIES - 1) {
                    std::cout << "⚠️  Core " << core_id_ << " SQE full, submitting pending and retrying..." << std::endl;
                    io_uring_submit(&ring_); // Submit pending operations to free up SQEs
                    std::this_thread::sleep_for(std::chrono::microseconds(100));
                    continue;
                } else {
                    std::cerr << "❌ Core " << core_id_ << " submit_recv_operation: no SQE available after retries!" << std::endl;
                    return;
                }
            }
            ...
            if (!buffer) {
                if (retry < MAX_RETRIES - 1) {
                    std::cout << "⚠️  Core " << core_id_ << " buffer pool empty, retrying..." << std::endl;
                    std::this_thread::sleep_for(std::chrono::microseconds(100));
                    continue;
                } else {
                    std::cerr << "❌ Core " << core_id_ << " submit_recv_operation: no buffer available after retries!" << std::endl;
                    return;
                }
            }
            
            uint64_t op_id = operation_counter_.fetch_add(1);
            auto op = std::make_unique<IoUringOperation>(IoUringOpType::RECV, client_fd);
            op->buffer = buffer;
            op->buffer_size = IoUringBufferPool::buffer_size();
            
            io_uring_prep_recv(sqe, client_fd, buffer, IoUringBufferPool::buffer_size(), 0);
            io_uring_sqe_set_data(sqe, reinterpret_cast<void*>(op_id));
            ...
            // **PHASE 7 STEP 1 FIX: Don't submit immediately - let main event loop handle it**
            std::cout << "🔄 Core " << core_id_ << " queued recv operation for fd=" << client_fd 
                      << " (op_id=" << op_id << ") - will be submitted by event loop" << std::endl;
            
            // Success - exit retry loop
            return;
        }
        
        // **PHASE 7 STEP 1: If we get here, all retries failed**
        std::cerr << "💥 Core " << core_id_ << " CRITICAL: submit_recv_operation FAILED after " << MAX_RETRIES << " retries for fd=" << client_fd << std::endl;
    }
    ...
    void submit_send_operation(int client_fd, const void* data, size_t size) {
        ...
        uint64_t op_id = operation_counter_.fetch_add(1);
        auto op = std::make_unique<IoUringOperation>(IoUringOpType::SEND, client_fd);
        op->buffer = buffer;
        op->buffer_size = size;
        ...
        // **PHASE 7 STEP 1: Submit immediately to ensure operation is queued**
        io_uring_submit(&ring_);
        std::cout << "🔄 Core " << core_id_ << " submitted send operation for fd=" << client_fd 
                  << " size=" << size << " (op_id=" << op_id << ")" << std::endl;
    }
    
    void add_client_connection_iouring(int client_fd) {
        // Set non-blocking
        int flags = fcntl(client_fd, F_GETFL, 0);
        fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
        ...
        // **PHASE 7 STEP 1 FIX: Queue recv operation for deferred submission**
        std::cout << "🔄 Core " << core_id_ << " queuing recv operation for fd=" << client_fd << std::endl;
        {
            std::lock_guard<std::mutex> lock(recv_queue_mutex_);
            pending_recv_submissions_.push(client_fd);
        }
        std::cout << "✅ Core " << core_id_ << " recv operation queued for fd=" << client_fd << std::endl;
        
        std::cout << "✅ Core " << core_id_ << " accepted connection fd=" << client_fd 
                  << " (io_uring async)" << std::endl;
    }
#endif
