...
private:
    ...
    // **SIMPLE ROUTING FIX**: Core and shard info for correct routing
    int core_id_;
...
bool DirectOperationProcessor::process_pipeline_batch(int client_fd, const std::vector<std::vector<std::string>>& commands) {
    ...
    size_t total_shards = total_shards_;  // Use actual total_shards from constructor
    // **ROUTING FIX**: Calculate correct shard for this core
    ...
    for (const auto& cmd_parts : commands) {
        if (cmd_parts.size() >= 2) {
            ...
            if (target_shard == current_shard) {
                ...
                // **SIMPLE FIX**: For now, process locally (should not happen in 4C 4S)
                // In 4C 4S config, each key should map to its owning core, so this is a fallback
                BatchOperation op(command, key, value, client_fd);
                std::string response = execute_single_operation(op);
                local_responses.push_back(response);
            }
        ...
        if (commands.size() > 1) {
            ...
            for (const auto& cmd_data : commands) {
                ...
                if (!parsed_cmd.empty()) {
                    ...
                    // **ROUTING FIX**: Apply same routing logic as pipelines for single commands
                    if ((cmd_upper == "GET" || cmd_upper == "SET" || cmd_upper == "DEL") && !key.empty()) {
                        size_t target_shard = std::hash<std::string>{}(key) % total_shards_;
                        size_t current_shard = core_id_ % total_shards_;
                        
                        if (target_shard == current_shard) {
                            // **CORRECT CORE**: This key belongs to this core
                            std::cout << "✅ Core " << core_id_ << ": " << cmd_upper << " " << key << " (correct core, shard " << current_shard << ")" << std::endl;
                            processor_->submit_operation(command, key, value, client_fd);
                        } else {
                            // **WRONG CORE**: Command came to wrong core - this is the root problem!
                            std::cout << "❌ Core " << core_id_ << ": " << cmd_upper << " " << key << " (WRONG core! target_shard=" << target_shard << ", current_shard=" << current_shard << ")" << std::endl;
                            
                            // **SIMPLE CROSS-SHARD**: Send a simple error for now to confirm this is the issue
                            std::string error_response = "-ERR key routed to wrong core\r\n";
                            send(client_fd, error_response.c_str(), error_response.length(), MSG_NOSIGNAL);
                        }
                    } else {
                        // **NON-DATA COMMANDS**: Process locally (PING, etc.)
                        processor_->submit_operation(command, key, value, client_fd);
                    }
                }
            }
