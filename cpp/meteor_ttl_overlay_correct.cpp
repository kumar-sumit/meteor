// **METEOR v8.0: TTL OVERLAY - REDIS-STYLE TRANSPARENT TTL**
//
// **🚀 v8.0 TTL OVERLAY IMPLEMENTATION - REDIS-STYLE TRANSPARENT TTL**:
// ✅ ZERO MODIFICATION to baseline storage (Entry struct unchanged)
// ✅ SEPARATE TTL TRACKING table (Redis approach - key -> expiration time)
// ✅ PASSIVE EXPIRATION during GET (transparent to application)  
// ✅ ACTIVE EXPIRATION via periodic cleanup (background thread)
// ✅ REDIS-COMPATIBLE COMMANDS: EXPIRE, TTL, PERSIST
// ✅ BASELINE FUNCTIONALITY PRESERVED: SET/GET/pipeline unchanged
//
// **ARCHITECTURE**: TTL as transparent overlay on working baseline
//
...
namespace storage {
    ...
    private:
        ...
        // **REDIS-STYLE TTL OVERLAY**: Separate expiration tracking (doesn't modify core storage)
        std::unordered_map<std::string, std::chrono::steady_clock::time_point> expiration_table_;
        mutable std::chrono::steady_clock::time_point last_cleanup_;
        static constexpr auto CLEANUP_INTERVAL = std::chrono::minutes(1);
        
        // **REDIS-STYLE PASSIVE EXPIRATION**: Check during operations
        bool is_expired(const std::string& key) const {
            auto exp_it = expiration_table_.find(key);
            if (exp_it == expiration_table_.end()) {
                return false; // No expiration set
            }
            return std::chrono::steady_clock::now() >= exp_it->second;
        }
        
        // **REDIS-STYLE ACTIVE EXPIRATION**: Periodic cleanup
        void active_expiration() const {
            auto now = std::chrono::steady_clock::now();
            if (now - last_cleanup_ < CLEANUP_INTERVAL) return;
            
            // Remove expired keys from both tables
            auto& mutable_data = const_cast<std::unordered_map<std::string, Entry>&>(data_);
            auto& mutable_exp = const_cast<std::unordered_map<std::string, std::chrono::steady_clock::time_point>&>(expiration_table_);
            
            for (auto exp_it = mutable_exp.begin(); exp_it != mutable_exp.end();) {
                if (now >= exp_it->second) {
                    // Key expired - remove from both tables
                    mutable_data.erase(exp_it->first);
                    exp_it = mutable_exp.erase(exp_it);
                } else {
                    ++exp_it;
                }
            }
    ...
    public:
        ...
        bool set(const std::string& key, const std::string& value) {
            // **BASELINE UNCHANGED**: Exactly same logic as working v7.0
            data_[key] = Entry(key, value);
            // Note: SET doesn't affect expiration (Redis behavior)
            return true;
        }
        
        std::optional<std::string> get(const std::string& key) {
            // **REDIS-STYLE PASSIVE EXPIRATION**: Check TTL transparently
            active_expiration(); // Periodic cleanup
            
            if (is_expired(key)) {
                // Remove expired key from both tables  
                data_.erase(key);
                expiration_table_.erase(key);
                return std::nullopt;
            }
        ...
        bool del(const std::string& key) {
            // **BASELINE UNCHANGED**: Exactly same logic as working v7.0
            expiration_table_.erase(key); // Also remove from TTL table
            return data_.erase(key) > 0;
        }
        
        // **NEW TTL OVERLAY METHODS** (Redis-compatible)
        bool expire(const std::string& key, std::chrono::seconds ttl) {
            // Check if key exists in core storage (unchanged)
            if (data_.find(key) != data_.end() && !is_expired(key)) {
                // Set expiration in separate table
                expiration_table_[key] = std::chrono::steady_clock::now() + ttl;
                return true;
            }
            return false;
        }
        ...
        bool persist(const std::string& key) {
            // Check if key exists and not expired
            if (data_.find(key) != data_.end() && !is_expired(key)) {
                expiration_table_.erase(key); // Remove TTL
                return true;
            }
            return false;
        }
...
namespace cache {
    ...
    public:
        ...
        bool set(const std::string& key, const std::string& value) {
            // **BASELINE UNCHANGED**: Direct access to VLLHashTable (no tracking overhead)
        ...
        // **TTL OVERLAY METHODS** (pass through to VLLHashTable)
        bool expire(const std::string& key, std::chrono::seconds ttl) {
