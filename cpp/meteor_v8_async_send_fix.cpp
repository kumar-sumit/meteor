...
private:
    ...
    // 
    // **🎉 ROOT CAUSE FIX APPLIED - ASYNC SEND TRANSFORMATION**:
    // ========================================================
    // 
    // CRITICAL DISCOVERY: The 60x latency difference was caused by:
    // • Single commands: Used send_response_async() → FAST (async I/O)
    // • Pipeline commands: Used send() → SLOW (synchronous blocking)
    // 
    // After architectural routing fix, ALL commands go through pipeline path,
    // so pipeline=1 was using slow synchronous send() causing 10.36ms p99.9.
    // 
    // SOLUTION: Replace ALL send() calls with send_response_async() calls
    // 
    // EXPECTED RESULT: Pipeline=1 → 0.175ms p99.9 (60x improvement!)
    //
