// **METEOR PHASE 1.2B: SYSCALL REDUCTION & FULL CPU UTILIZATION**
// 
...
//
// **PHASE 1.2B SYSCALL REDUCTION OPTIMIZATIONS**:
...
// **GLOBAL SHARED CHANNELS DEFINITION**: Define the static variables outside namespace
namespace dragonfly_cross_shard {
    extern std::vector<std::shared_ptr<boost::fibers::buffered_channel<std::unique_ptr<CrossShardCommand>>>> global_shard_channels_;
    extern std::mutex global_channels_mutex_;
    extern bool global_channels_initialized_;
}

std::vector<std::shared_ptr<boost::fibers::buffered_channel<std::unique_ptr<dragonfly_cross_shard::CrossShardCommand>>>> dragonfly_cross_shard::global_shard_channels_;
std::mutex dragonfly_cross_shard::global_channels_mutex_;
bool dragonfly_cross_shard::global_channels_initialized_ = false;
...
private:
    ...
    // Initialize global shared channels (called once)
    void initialize_global_channels() {
        std::lock_guard<std::mutex> lock(global_channels_mutex_);
        if (!global_channels_initialized_) {
            global_shard_channels_.reserve(num_shards_);
            for (size_t i = 0; i < num_shards_; ++i) {
                global_shard_channels_.emplace_back(
                    std::make_shared<boost::fibers::buffered_channel<std::unique_ptr<CrossShardCommand>>>(1024)
                );
            }
            global_channels_initialized_ = true;
        }
    }
...
public:
    CrossShardCoordinator(size_t num_shards, size_t current_shard) 
        ...
        // Initialize global shared channels if not already done
        initialize_global_channels();
    }
    ...
    boost::fibers::future<std::string> send_cross_shard_command(
        ...
        // Send to target shard's GLOBAL SHARED channel
        if (target_shard < global_shard_channels_.size()) {
            try {
                global_shard_channels_[target_shard]->push(std::move(cmd));
    ...
    // **DRAGONFLY PATTERN**: Start dedicated fiber for processing cross-shard commands
    void start_shard_processor(size_t shard_id, std::function<std::string(const CrossShardCommand&)> executor) {
        if (shard_id >= global_shard_channels_.size()) return;
        
        // **BOOST.FIBERS**: Create processing fiber for this shard (DragonflyDB pattern)
        boost::fibers::fiber([shard_id, executor]() {
            auto& channel = *global_shard_channels_[shard_id];
            std::unique_ptr<CrossShardCommand> cmd;
            
            std::cout << "🔥 Started cross-shard processor for shard " << shard_id << std::endl;
            
            while (true) {
                try {
                    // **BLOCKING WAIT**: Wait for commands from channel
                    if (channel.pop(cmd) == boost::fibers::channel_op_status::success && cmd) {
                        try {
                            // **EXECUTE LOCALLY**: Execute command and fulfill promise
                        ...
                        // Channel closed or error, exit fiber
                        break;
                    }
                } catch (const std::exception& e) {
                    std::cerr << "Cross-shard processor error: " << e.what() << std::endl;
                    break;
                }
            }
            
            std::cout << "🛑 Cross-shard processor for shard " << shard_id << " stopped" << std::endl;
        }).detach();
    }
    ...
    void shutdown() {
        std::lock_guard<std::mutex> lock(global_channels_mutex_);
        for (auto& channel : global_shard_channels_) {
            if (channel) {
                channel->close();
            }
        }
...
// **INTER-CORE COMMAND ROUTING**: Simple command passing between cores
struct InterCoreCommand {
    ...
    InterCoreCommand() : client_fd(-1), source_core(0) {}
};
...
struct ConnectionMigrationMessage {
    ...
    // Default constructor for dequeue operations
    ConnectionMigrationMessage() : client_fd(-1), source_core(0), timestamp(std::chrono::steady_clock::now()) {}
...
public:
    ...
    void set_core_references(const std::vector<CoreThread*>& cores) {
        all_cores_ = cores;
        std::cout << "🔗 Core " << core_id_ << " has references to " << all_cores_.size() << " cores for routing" << std::endl;
    }
...
private:
    // **DRAGONFLY CROSS-SHARD**: Commands processed by dedicated fibers (started in constructor)
    // No polling needed - fibers automatically handle cross-shard commands via channels
    ...
    void process_connection_migrations() {
        ConnectionMigrationMessage migration_msg;
        while (incoming_migrations_.dequeue(migration_msg)) {
            // Add the migrated connection to this core's event loop
            add_migrated_client_connection(migration_msg.client_fd);
            
            // Process the pending command that triggered the migration
            if (!migration_msg.pending_command.empty()) {
                processor_->submit_operation(migration_msg.pending_command, 
                                           migration_msg.pending_key, 
                                           migration_msg.pending_value,
                                           migration_msg.client_fd);
            }
        }
    }
    ...
    void run() {
        ...
        while (running_.load()) {
            // **OPTIMIZATION**: Work stealing removed for performance testing
            
            // **DIRECT ROUTING**: No connection migration needed - removed overhead
            
            // **DRAGONFLY CROSS-SHARD**: Commands automatically processed by dedicated fibers
            ...
            // **OPTIMIZATION**: Heavy work distribution removed for performance testing
        }
    ...
    void parse_and_submit_commands(const std::string& data, int client_fd) {
        ...
        if (commands.size() > 1) {
            ...
            // **SINGLE COMMAND**: DragonflyDB cross-shard approach (no connection migration)
            for (const auto& cmd_data : commands) {
                auto parsed_cmd = parse_single_resp_command(cmd_data);
                if (!parsed_cmd.empty()) {
                    ...
                    // **PERFORMANCE FIX**: Eliminate routing overhead - process all locally
                    
                    // Process locally (all commands end up here)
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "✅ Configuration: " << num_cores << " cores, " << num_shards << " shards (perfect 1:1 mapping)" << std::endl;
