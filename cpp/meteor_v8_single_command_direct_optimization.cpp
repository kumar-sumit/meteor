...
private:
    ...
    void process_pipeline_with_mget_support(int client_fd, std::vector<std::vector<std::string>>& parsed_commands) {
        // **SENIOR ARCHITECT OPTIMIZATION**: Single Command Direct Path
        //
        // PERFORMANCE ANALYSIS ROOT CAUSE:
        // =================================
        // Pipeline=1: 10.55ms p99.9 (string accumulation overhead)
        // Pipeline=2: 0.19ms p99.9 (efficient batched processing)
        //
        // SOLUTION: Bypass string accumulation for single commands
        // Use direct response sending to eliminate allocation overhead
        
        if (parsed_commands.size() == 1) {
            // **SINGLE COMMAND DIRECT OPTIMIZATION**: Skip string accumulation entirely
            ...
            if (!cmd.empty()) {
                ...
                if (cmd_upper == "MGET" && cmd.size() > 1) {
                    // Handle MGET with direct response (same logic but direct send)
                    ...
                    std::string response = mget_response.str();
                    send(client_fd, response.c_str(), response.length(), MSG_NOSIGNAL);
                    return;
                } else {
                    // **DIRECT SINGLE COMMAND PROCESSING**: Most efficient path
                    std::string key = cmd.size() > 1 ? cmd[1] : "";
                    std::string value = cmd.size() > 2 ? cmd[2] : "";
                    
                    // Use the optimized version directly to avoid string conversion overhead
                    BatchOperation op(cmd[0], key, value, client_fd);
                    auto response_info = processor_->execute_single_operation_optimized(op);
                    
                    // Direct send without string allocation
                    send(client_fd, response_info.data, response_info.size, MSG_NOSIGNAL);
                    return;
                }
            }
            return; // Single command processed
        }
