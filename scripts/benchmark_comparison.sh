...
# Comprehensive benchmark script for Redis vs Meteor server
# This script will test both systems with various workloads
...
# Test parameters
TEST_DURATION=30
SET_OPERATIONS=10000
GET_OPERATIONS=10000
CONCURRENT_CLIENTS=10
VALUE_SIZE=1024

echo "================================================================"
echo "          REDIS vs METEOR SERVER BENCHMARK COMPARISON"
echo "================================================================"
...
test_server_performance() {
    ...
    # Generate test data
    local test_value=$(generate_value $VALUE_SIZE)
    
    # Test 1: SET Performance
    echo "1. SET Performance Test..."
    local start_time=$(date +%s%N)
    for i in $(seq 1 $SET_OPERATIONS); do
        redis-cli -h $host -p $port set "key_$i" "$test_value" > /dev/null 2>&1
    done
    ...
    for i in $(seq 1 1000); do
        ...
        for j in $(seq 1 3); do
            ...
            redis-cli -h $host -p $port set "key_$key_id" "$test_value" > /dev/null 2>&1
        done
    done
    local end_time=$(date +%s%N)
    local duration_ms=$(( ($end_time - $start_time) / 1000000 ))
    local mixed_ops_per_sec=$(( 10000 * 1000 / $duration_ms ))
    
    echo "   Mixed Operations: 10000 (7000 GET, 3000 SET)"
    echo "   Duration: ${duration_ms}ms"
    echo "   Throughput: ${mixed_ops_per_sec} ops/sec"
    echo "   Average Latency: $(( $duration_ms * 1000 / 10000 ))μs"
    
    # Test 4: Latency Test (100 operations with timing)
    ...
    local p50=${sorted_latencies[49]}
    local p95=${sorted_latencies[94]}
    local p99=${sorted_latencies[98]}
    local max_latency=${sorted_latencies[99]}
...
run_concurrent_test() {
    ...
    echo
    echo "5. Concurrent Test ($CONCURRENT_CLIENTS clients)..."
    
    # Create a temporary script for each client
    cat > /tmp/concurrent_client.sh << 'EOF'
#!/bin/bash
HOST=$1
PORT=$2
CLIENT_ID=$3
...
for i in $(seq 1 $OPERATIONS); do
    redis-cli -h $HOST -p $PORT set "concurrent_${CLIENT_ID}_$i" "value_$i" > /dev/null 2>&1
    redis-cli -h $HOST -p $PORT get "concurrent_${CLIENT_ID}_$i" > /dev/null 2>&1
done
EOF
    chmod +x /tmp/concurrent_client.sh
    
    local start_time=$(date +%s%N)
    
    # Start concurrent clients
    for i in $(seq 1 $CONCURRENT_CLIENTS); do
        /tmp/concurrent_client.sh $host $port $i &
    done
    ...
    local end_time=$(date +%s%N)
    local duration_ms=$(( ($end_time - $start_time) / 1000000 ))
    local total_ops=$(( $CONCURRENT_CLIENTS * 2000 ))  # 1000 SET + 1000 GET per client
    local concurrent_ops_per_sec=$(( $total_ops * 1000 / $duration_ms ))
    
    echo "   Concurrent Clients: $CONCURRENT_CLIENTS"
    echo "   Total Operations: $total_ops"
    echo "   Duration: ${duration_ms}ms"
    echo "   Throughput: ${concurrent_ops_per_sec} ops/sec"
    echo "   Average Latency: $(( $duration_ms * 1000 / $total_ops ))μs"
    
    # Store concurrent results
    if [ "$server_name" == "Redis" ]; then
        REDIS_CONCURRENT_OPS=$concurrent_ops_per_sec
    else
        METEOR_CONCURRENT_OPS=$concurrent_ops_per_sec
    fi
    
    # Cleanup
    rm -f /tmp/concurrent_client.sh
}
...
if [ $? -eq 0 ]; then
    run_concurrent_test "Redis" $REDIS_HOST $REDIS_PORT
fi
...
if [ $? -eq 0 ]; then
    run_concurrent_test "Meteor" $METEOR_HOST $METEOR_PORT
fi
...
printf "%-20s %-15s %-15s %-15s\n" "Concurrent ops/sec" "$REDIS_CONCURRENT_OPS" "$METEOR_CONCURRENT_OPS" "$(( $METEOR_CONCURRENT_OPS * 100 / $REDIS_CONCURRENT_OPS - 100 ))%"
echo "----------------------------------------------------------------"
